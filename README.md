...
- **🗑 移除空间（R-3）**：空间行「⚙」→ 设置底部「危险区」可删除工作空间——展开即拉取**影响预检**（只读：任务/编队/模型/执行请求/技能/目标/编排/会话/消息/日程/成员 11 类计数 + 在办任务清单），需 **type-to-confirm** 输入 `delete-space:<id>` 精确匹配才可确认；删除级联清空该空间全部数据表（`POST /api/spaces/delete`，审计保留 `space:delete` 历史），**software/default 受保护不可删**；删的是当前空间时界面自动切回「全部空间」。磁盘残留（本地文件/绑定目录）需按提示由运维清理，接口不删磁盘。
...
- **🤖 对话 AI 回复（R-4）**：各空间默认开启 AI 直答——发消息后守护（scrum-worker responder）从 `GET /api/chat/replies` 拉 awaiting 消息，用**无仓库工具**轻量子代理（`settings.model ?? 该空间默认模型`）按会话历史直答，回复以 `author=<scope>-assistant` 落为第二条消息（`POST /api/chat/replies/answer`，≤120s 超时自动 failed）；气泡三态：⏳ 等待回复… / ✅ replied 正文（🤖 + 模型徽标）/ ❌ failed 附原因 + **重试**按钮；身份泛化：general = 我（右），其余 author（含 assistant）一律对方侧。每空间开关/身份在 `POST /api/chat/reply-settings`（默认开，关后零出站）。
...
- **📜 规范中心**（`RulesPanel`，分层项目规范 R-2）：两个明确分区——①**全局层**（rules 表，跨空间统一规范，本面板编辑保存 `POST /api/rules`，≤3000 字，审计 `rules:update`）②**空间层文件族**（`LEGION.md` / `AGENTS.md` / `agent.md`，在空间绑定仓库内维护，守护扫单自动读取注入）；全局层/空间层按预算分层截断注入派工提示词，空间层后置 + 优先声明。职责总纲见 §3.8.1。
- 左侧 QuickTools「文件浏览 / 浏览网页」与侧栏模块入口指向同一面板（`active` 状态一致）；**📅 日程日历 / 🔔 通知中心**（P1 切片 S6/S7 已交付，见 §3.9 三中心扩展）随空间使用。
- **🧩 技能中心（跨空间共享 R-1）**：注册技能 → 将军发布/驳回 → 按成员或 `scope:xxx` **跨空间授权**（general 门禁；撤销即时生效）；目标空间视角自动含已发布共享技能（来源标注「来自空间 X · 只读」）；「全部空间」视图只含已发布，草稿/pending 由服务端收口不外泄（`include=pending` 仅 general 复审视角）。
- **右侧「实时动态」**：SSE 审计流；顶部 KPI：目标/完成/进行中/AI 员工数。
- **底部命令栏**：⏸ 全局暂停（v1）/ 🗓 任务调度 / 🎯 发布目标 / ⚙️ 模型配置 / ＋ 新建任务 / 📤 导出日报。

### 3.8.1 规范载体职责总纲（分层 · R-2，按优先级注入）

团队规范分四类载体，各自维护入口不同、注入时机不同，冲突时**空间层优先于全局层**：

| 载体 | 内容 | 维护入口 | 注入时机/优先级 |
| --- | --- | --- | --- | --- |
| **空间层文件族**（`LEGION.md` / `AGENTS.md` / `agent.md`） | 仓库规则（空间/项目专属；LEGION.md 默认最优先，其余按序并入） | 空间绑定仓库根目录（文件中心/仓库编辑器） | 每次派工注入，作为空间层段**后置 + 「优先于全局层」声明** |
| **全局层**（team-hub `rules` 表，`scope=global`） | 跨空间统一规范/质量标准 | 指挥台「📜 规范中心」全局层编辑器（`POST /api/rules`） | 每次派工注入，作为全局层段（位于空间层段之前） |
| **skills（技能中心）** | 技能内容（任务怎么做：检查项/步骤/输出格式） | 「🧩 技能中心」注册 + 将军发布 + 按成员/`scope:xxx` 授权 | 按任务的 scope/授权命中注入（已发布，士兵只读） |
| **roles.json `stage.prompt` / `stage-standards`** | 岗位职责与阶段交付模板（做什么/不做什么） | 仓库 `plugins/roles.json`（角色配置） | 任务拆解/派工流水线按岗位模板注入 |

空间层与全局层超出预算（全局 3000 / 空间 4000 / 合计 7000 字）时按段边界截断并附真实字数提示，代码围栏不半截；空间层超限保留、全局层完整。
