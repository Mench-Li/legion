...
日常主入口是 **workbench 军团指挥台**，数据与 API 中枢是 **team-hub v2**。Scrum 的用户界面已经合入 workbench 的“任务中心”，且已收敛为**单一 Scrum 泳道视图**：按状态泳道展示、可拖拽迁移，顶部“视角”（工作中/待我决定/待办/已完成）只过滤卡片、不改变泳道结构。仓库中的 `scrum/` 保留 v1 协议引擎与本地自托管脚本（用于契约对照与调试），其 v1 任务库已归档、`:4820` 独立页面已退役，不再是入口。

> 需要逐项操作说明时，请阅读 [功能使用手册](docs/FEATURES.md)；部署、验证和回滚以 [部署手册](docs/DEPLOY.md) 为准；**“现在是什么状态”（拓扑、测试基线、已知限制）以 [当前状态入口](docs/STATUS.md) 为准**——各 `*-evidence/` 目录是带基线 commit 的历史快照，不作为当前依据。
...
- workbench 默认消费 v2；“任务中心”的 Scrum 泳道视图消费 v2，不等同于继续使用 v1 `tasks.json`。
...
- 任务中心为**单一 Scrum 泳道视图**（原“指挥总览/Scrum 看板”双 Tab 已合并）：按状态泳道展示，中枢模式下可拖拽迁移；
  顶部“📊 视角”按工作中、待我决定、待办、已完成过滤卡片，泳道结构与拖拽目标保持完整。
...
# 宿主插件加载诊断（纯函数单测无需 DSH；主套件需要 $DSH_CHECKOUT）
...
4. `test`：对话、技能、日历、文件、浏览器、契约、白板、真实宿主插件注入（`p13-host-injection`，含插件导入失败诊断）与真实浏览器端到端（`e2e-browser`，无浏览器时 SKIP）测试；
