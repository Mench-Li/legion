...
日常主入口是 **workbench 军团指挥台**，数据与 API 中枢是 **team-hub v2**。Scrum 的用户界面已经合入 workbench 的“任务中心”：其中既有按状态聚合的“指挥总览”，也有可拖拽的“Scrum 看板”。仓库中的 `scrum/` 保留 v1 协议引擎与本地自托管脚本（用于契约对照与调试），其 v1 任务库已归档、`:4820` 独立页面已退役，不再是入口。
