...
### 15.5 ★★★ 订正：**这一条上一轮已经裁过，而且已经施工完了**（2026-09-24 当天自纠）

★ 写 §15 的时候我读的是 §14 与 §5 索引表的"待裁决"，**没读到台账 §2 与提交 `d955dac`**。事实是：

| 时间 | 发生了什么 |
| --- | --- |
| **2026-09-23**（第 118 轮第七轮，`d955dac`） | 已裁定「**丙的机制 + 目录锚在既有配置量上**」，**并四处一起落**：`product/process-manifest.mjs` 的 team-hub `envNames` 加 `LEGION_DATA_DIR`、`product/config-schema.mjs` 派生行、`product/launcher/launcher.mjs` 的 `derivedValuesFor()`、`team-hub/server.mjs` 的 30s 收账 tick。带端到端读数（真 hub 进程 + 临时库 → 45s 内那一行落进 `tool_calls`）。 |
| 2026-09-23（第 118 轮第八轮） | 写入侧接进生产（`root-row.mjs:674` 的 `createToolCallSpoolWriter`），runId **按事件**取 ⇒ 并发的两个 Run 各写各的。 |
| **2026-09-24**（今天，业主复述） | 对 §5 第 18 / 28 条答"**丙** + 同族一次裁完" —— 与 `d955dac` **同一方向**，不是新决定。 |

⇒ 所以 §15 的"**接线尚未开始**""`decisionSourceRecorded` 依然没有产出者"**是错的**，已就地订正。
**真正剩下的只有一条**：runId 要绑在**按 Run 安装**的那个缝（台账 §2.1 #4，PRT-214 已跑通两遍的形状）——
`root-row.mjs` 是**进程级单例**，在那儿绑死会让整个进程只往**第一个** Run 的账本里写。

★★ 复跑读数（2026-09-24，本机）：车道五套 **54 例全绿** ——
`spool` 14 / `toolcall-drain` 13 / `spool-writer` 14 / `spool-writer-wiring` 6 / `toolcall-sweep` 7。

> ★★★ 这一处的形状值得单独记：**"待裁决"这张索引表会滞后于施工。**
> 台账知道第 28 条已裁定，§5 索引表还写着"待裁决"，而那条分档判据（`decisionStateViolations`）
> 只核**索引 ↔ §5 正文 ↔ 合计**三者**彼此**一致 —— 它对"**这个状态与代码/提交是否一致**"一句话都说不出来。
> ⇒ 一个只能自证的清单，与一个恒真的清单，在"它能不能发现过期"上是同一个东西。
> ★ 而且这一次**正是它把我送进了一次多余的裁决提问**：我把"待裁决"当成现状，
> 让业主重新裁了一条 2026-09-23 就已经裁过并施工完的事。
> （处置：立一条判据 —— **标"待裁决"的行里若已经写着带日期的裁决记录，就判不一致**。）
