...
> ⚠️ **上面这一节（§1.1）是 2026-09-16 的快照**，其中的阶段判读此后已被后续工作取代多处。
> 当前状态一律以 `docs/MULTI-AGENT-FEATURE-STATUS.md` 为准。§1.2 把**可机械核对**的那部分列出来。

## 1.2 状态注记校准（2026-09-18 第 29 轮）

本文 §4 的每个功能标题后面括着**写作当时**的状态注记（如「收口中」「待补」）。
它们是**设计期的快照**，**不是当前状态**。

第 29 轮逐条核对了一遍：**8 条带注记的标题里，7 条说"还没做完"，而状态表里那 7 条已经全是 ✅**
（只有 F-04 的注记仍然成立，它今天还是 🟡）。这两份文档放在一起，
**读者信哪一份取决于他先打开哪一份**——本文是目标点名的输入文档，所以先把这件事说清。

★ **注记一个字都没有删**：它是设计期的快照，删掉等于篡改历史。
下面是**取代关系**，不是对原文的修正：

| 功能 | 标题里的原注记（写作当时） | 当前状态 | 依据（可核对落点） |
|---|---|---|---|
| F-01 | 基础已落地，收口中 | ✅ | PRT-101～109；套件 `runtime-contract`（64 例）、`contract.test.mjs` |
| F-02 | 主体已落地，生产接线收口中 | ✅ | PRT-201～206；套件 `dsh-adapter`（98 例） |
| F-03 | 规格已定义，产品级闭环待补 | ✅ | `RUNTIME_STATES` / `CLAIM_POLICY` / `runtimeStatusReport()`；PRT-711 认领闸门 |
| F-05 | 事件流已落地，投递状态机待补 | ✅ | `event-delivery.test.mjs`（31 例）+ `event-delivery-wiring.test.mjs`（7 例真 HTTP） |
| F-07 | 部分基础已存在，完整 Run 快照待收口 | ✅ | PRT-401～409；套件 `context-store`、`context-export`、`context-retention` |
| F-10 | 控制面基础已落地，DSH 工具全量接线待收口 | ✅ | PRT-601～607 |
| F-12 | 产品级 Launcher 待完成 | ✅ | PRT-701～713 |

**没有列进来的：F-04（尚未完成大规模提取）**——它的注记**仍然成立**：
该行今天还是 🟡（日期闸门，最早 2026-09-24 可开工）。一条只列"标题说没做完"的表，
如果连仍然没做完的也列进去，那张表就不再回答"哪些已经被取代"了。

★★ **2026-09-24（第 118 轮第四十二轮）补记：上面那个"最早 2026-09-24 可开工"的日期到了，闸门也开了 —— 所以本段有一半要改读数。**
业主在 2026-09-24 就 §13 的 Program 级停止条件按**乙**读出（逐片推进 + 把「编排提取」另行立项）；
「编排提取」**已立**为 **PRT-1007**（`docs/superpowers/prt/PRT-PROGRESS.md` 阶段 3），
**片 1 已交付**（`81a8756`：岗位文档契约那族纯函数逐字搬出 `plugins/src/index.ts`，
新增 `plugins/src/docContract.ts`，并立了逐字对拍判据 `scripts/probes/probe-slice-verbatim.mjs`）。
⇒ 本段里 **"F-04 尚未完成大规模提取"仍然成立**（`index.ts` 里还压着编排逻辑，逐片搬完为止），
但 **"还没开工"这一读法已被取代**：它现在是**在制**，载体是 PRT-1007。
★ 一条"注记仍然成立"的判断里，**"仍然没做完"与"仍然没开工"是两件事** ——
日期一到，后者的有效期就结束了，而它写在同一个句子里。

**这张表由判据 `scripts/prt/spec-status-calibration.mjs` 机械核对**（套件同目录）：
① 凡"注记说没做完、而状态表已全 ✅"的功能，**必须**在上表里；
② 上表写的状态，必须**等于**状态表当前的状态（哪天 F-01 退回 🟡，这张表会红，
而不是继续宣称 ✅）；③ 上表不许出现"注记还没被取代"的条目。

★ **诚实边界**：这条判据只覆盖 §4 的**标题注记**（那是可机械解析的形状）。
**§1.1 那张阶段表不在这条判据的覆盖内**——里面的"阶段 2.5 尚未形成闭环"、
"阶段 4～10 按未完成处理"同样已被后续工作取代，但它们与 F-NN 不是一一对应，
机械判不了，只能由人在这里说明。

## 1.3 正文级的偏离（判据**判不到**的那一半）

§1.2 校准的是**标题注记**。**§4 的正文**里还有一类偏离，判据覆盖不到，
所以在这里逐条写明。**判不到的原因不是没做，是这类"偏离"没有可机械解析的形状**：
它长得和一句正确的规格一模一样。

### 1.3.1 F-01 的「环境变量白名单」：契约里有、校验有、**消费者没有**

§4.1 F-01 的正文写着（本文件 `:146`）：
...
| 问题 | 实测读数 |
|---|---|
| `env` 在 `RUN_REQUEST_REQUIRED`（15 个必填字段）里吗 | **不在**——它不是必填 |
| 给了会不会校验形状 | **会**：`env` 非数组 ⇒ `env 必须是数组（白名单）`（`run.mjs:218-222`） |
| 不给会怎样 | **静默补成 `[]`**：`value.env = Array.isArray(req.env) ? [...req.env] : []`（`run.mjs:227`） |
| 谁**读**它 | **一个都没有**。全仓 `req.env` / `request.env` 零命中；`runtime/adapters/**` 里没有任何一处碰它 |
...
那里选的是"**给了就必须解释得通，不给就如实缺席**"（三态：`absent`/`installed`/`refused`）。
而 `env` 走的是**另一条路**：缺席折成 `[]`，于是"**没声明**"与"**声明了一个都不许**"
在读数上是**同一个值**。两者都是本仓写下来过的形状，而它们对同一个问题给了相反的答案。

**这一条要不要改，是一个决定**（已记为对照表 §5 第 29 条）：
要么给 `env` 接一个真消费者（那它才是一条真的白名单），
要么承认环境的作用域是**进程级**的（`product/process-manifest.mjs` 的 `envNames` +
`buildChildEnv()`——今天真正在起作用的那一层），并把本文这句"必须包含"改掉。

★ 今天**不作恶**的原因只有一个：这一格**本来就没被消费**。
"它没伤人"与"它是对的"是两件事。
