...
- **`permitsTool` 的输入词汇表没有改**：§4.4 那条教义仍然逐字成立，
  本模块是**中间那一层**，不是对它的推翻。
- **本机没有真实员工许可可配**：投递路径（五把键 + 清单 + `childEnvNames`）
  已通，`scan --check` 与 `config.test.mjs` 的 53 例里**只剩一条**外部阻塞
  （见 §5 第 30 条）。
...
| 17 | PRT-707 的两份实现用两个不同的模型密钥引用名 | **未标注** | 产品 + 项目主 |
...
| 26 | 外部 API 授权表要不要管 scheme | **未标注** | 产品 + 架构 |
| 27 | 岗位清单说的是「Legion 能力名」还是「执行面工具名」 | **已解开**（第 112 轮，见 §4.5） | 产品 + 架构 |
| 28 | 那条"出站车道"的文件该放在哪个目录 | 待裁决 | 产品 + 架构 |
| 29 | RunRequest.env | **未标注** | 产品 + 架构 |

⇒ **合计**：已裁决 **4** · 待施工 **0** · 待裁决 **5** · **未标注 20**（共 29 行）。
