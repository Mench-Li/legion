...
| 4 | **PRT-214 G4** | 产品 | win32 `sandbox-enforcement` 的"完整 vs 部分"判据 | 强制面自检的通过标准没有权威定义，自检通过与否不可复核 ★★★ 2026-09-24 业主裁定（本会话）：**定义 win32 `sandbox-enforcement` 的"完整 vs 部分"口径** —— 哪几项算完整、"部分"即**永久平台边界**（与第 11 条①同类） ⇒ **已裁决**。 |
| 5 | **PRT-253 G2/G3/G4/G6** | 产品 + 项目主 | 能力来源判据、win32 沙箱口径、**真实模型黄金跑** | 单员工黄金任务只能"代码就绪"，不能宣称跑通 ★★★ 2026-09-24 业主裁定（本会话）：**除"真实模型黄金跑"外三项本轮定口径**；黄金跑**挂到第 7 条名下**（真实凭据 / 机器），不单独追 ⇒ **已裁决**。 |
