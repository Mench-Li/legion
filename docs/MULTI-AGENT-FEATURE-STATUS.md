...
| F-03 | Runtime Manager | ✅ | `product/runtime-state.mjs` | `RUNTIME_STATES` / `CLAIM_POLICY` / `runtimeStatusReport()`；PRT-711 认领闸门 | ★ **[2026-09-24 未核（T5）]** 本行**没有任何可复跑的判据**（普查 `scripts/probes/feature-row-judges.mjs`：它既没有被任何**点名这个 F-id** 的套件覆盖，也没有在行内引用可复跑的套件名或判据脚本）。★ "✅"说的是**已交付**，不是"**有东西在核**" —— 这两件事在本表里长得一样，区别只在于后者能被人复跑。补判据（推荐：给 `run-ci.mjs` 加一个点名本 F-id 的套件，或在"依据"格里引一条可复跑的判据脚本）后请**同时撤掉本标记**（门禁会核"两份名单是否一致"）。 |
| F-04 | Orchestrator Core | 🟡 | `orchestrator/worker/*`、`team-hub/run-store.mjs`、`orchestrator/{acceptance,pipeline,workspace}/` | PRT-301～316。★ **不在这里复制那份计数**——逐条状态见 [`superpowers/prt/PRT-PROGRESS.md`](./superpowers/prt/PRT-PROGRESS.md) 台账（那是权威）。此处曾写着一份手抄的四列合计（已完成 143、部分 19、未开始 1、需外部输入 2），而台账当时的真实读数是 **145 行 = 已完成 138、部分 4、未开始 1、需外部输入 2**：**它对不上**，且**没有任何门禁会去核对它**（`check-docs` 只管 `README.md` 与 `docs/FEATURES.md`）。一个手抄的计数与一份会漂移的计数是同一个东西，只不过前者的读者会以为它被核对过——现已改为不复制，并由 `scripts/prt/progress-check.test.mjs` 用例 ⑥ 钉住 | `plugins/src/index.ts` 仍是主要编排热点（§1.1 的判断）★★★ **2026-09-24 订正 + 业主裁决（第 9 条，选乙：逐片推进 + 把「编排提取」另行立项）**：本格此前写的「**PRT-316 是台账里唯一的 ⬜**」**是过期的**，而且过期得比它自己写的还彻底 —— 台账**今天一条 ⬜ 都没有**（机器读数：145 行 = **141 ✅ / 4 ⏸ / 0 🟡 / 0 ⬜**，`node scripts/prt/progress-check.mjs`）。PRT-316 **已于 2026-09-21 收口为 ✅**（`2967119`，"PRT-316 收口 —— 本项 🟡 → ✅"，运行面仓储新建在独立模块 `team-hub/run-store.mjs`）。⇒ 那条"排期规则最早 2026-09-24 可启动"的账**已经结清**（今天就是 2026-09-24；churn 门禁在裁决当日重量：HEAD `4009179`）；**③ §13 的 Program 级停止条件**由业主 2026-09-24 按**乙**读出：逐片推进 + 把「编排提取」**另行立项**。★ 于是本格 🟡 的剩余**只剩一件事**：`plugins/src/index.ts` 里还压着编排逻辑（它就是"逐片"要搬的东西），而"另行立项"这件事本轮**刚裁定、还没立**——下一轮的第一件施工项 ★★★ **2026-09-24（第 118 轮第四十二轮）订正：本格上面那两句都已经过期。**① 它引的是一份**陈旧的台账副本** —— 而本页自己的规矩就是**不在这里复制那份计数**（逐格读数见台账；`scripts/prt/boundary-facts.test.mjs` 的 ⑰ 会强制**每一处抄写都登记**，它正是靠这条抓到我这处新写的副本的）。**要点只有一句**：今天的台账比它多一条 🟡，多出来的那一行就是本格所说的「编排提取」立项 —— **PRT-1007**。② 它末尾写「**另行立项**这件事本轮刚裁定、**还没立**」—— **已经立了**：`docs/superpowers/prt/PRT-PROGRESS.md` 阶段 3 里已有 **PRT-1007「编排提取」** 一行（🟡），而且**片 1 已交付**（`81a8756`：岗位文档契约那族纯函数逐字搬出 `plugins/src/index.ts`，新增 `plugins/src/docContract.ts`，并立了逐字对拍判据 `scripts/probes/probe-slice-verbatim.mjs`）。⇒ **本格 🟡 的剩余没有变**（`index.ts` 里还压着编排逻辑，逐片搬完为止），但它现在的**载体**是 PRT-1007，不再是"还没立"。★★ **被取代的陈述**：上面那句 `145 行 = …` 与「还没立」保留原样作历史。 ★★ **2026-09-24（第 118 轮第四十二轮）补：PRT-1007 片 2 已交付** —— `childLogFile` + `statusFileNames`（多空间/子实例的**文件命名**族）逐字搬出 `plugins/src/index.ts`，落在新模块 `plugins/src/spacePaths.ts`（判据 `scripts/probes/probe-slice-verbatim.mjs` 现已登记**两片**、四问全过）。★ 这一刀的位置有个讲究：回引写在**被删掉的那两块的原位置**，而不是文件头 —— 写在文件头会让其下**所有行号 +3**，而 `boundary-facts` 的 `source-original-citations-on-line` 登记的正是"指向本文件的行号引文"的读数（`runtime/adapters/dsh/{port,index,schema,parity}.mjs` 与仓里若干 `.ts` 都在引），读数一变判据就红 —— 第 42 轮为此回滚过两次。⇒ 剩余仍是同一件事：`index.ts` 里还压着编排逻辑，逐片搬完为止。 |
...
| F-06 | Run 状态机 / 验收 / 交接 | ✅ | `team-hub/run-store.mjs`、`orchestrator/acceptance/`、`orchestrator/pipeline/` | PRT-307/308/309/310/311/312 | ★ **[2026-09-24 未核（T5）]** 本行**没有任何可复跑的判据**（普查 `scripts/probes/feature-row-judges.mjs`：它既没有被任何**点名这个 F-id** 的套件覆盖，也没有在行内引用可复跑的套件名或判据脚本）。★ "✅"说的是**已交付**，不是"**有东西在核**" —— 这两件事在本表里长得一样，区别只在于后者能被人复跑。补判据（推荐：给 `run-ci.mjs` 加一个点名本 F-id 的套件，或在"依据"格里引一条可复跑的判据脚本）后请**同时撤掉本标记**（门禁会核"两份名单是否一致"）。 |
| F-07 | 上下文快照 | ✅ | `runtime/context/*`、`team-hub/context-store.mjs` | PRT-401～409；套件 `context-store`、`context-export`、`context-retention` | — |
| F-08 | 模型档案与绑定 | ✅ | `team-hub/model-store.mjs`、`binding-store.mjs`、`probe-service.mjs` | PRT-501/502/506/507 | ★ **[2026-09-24 未核（T5）]** 本行**没有任何可复跑的判据**（普查 `scripts/probes/feature-row-judges.mjs`：它既没有被任何**点名这个 F-id** 的套件覆盖，也没有在行内引用可复跑的套件名或判据脚本）。★ "✅"说的是**已交付**，不是"**有东西在核**" —— 这两件事在本表里长得一样，区别只在于后者能被人复跑。补判据（推荐：给 `run-ci.mjs` 加一个点名本 F-id 的套件，或在"依据"格里引一条可复跑的判据脚本）后请**同时撤掉本标记**（门禁会核"两份名单是否一致"）。 |
| F-09 | 密钥库与 Run 凭证 | 🟡→⏸ | `security/secrets/*`、`product/secrets.mjs`、`product/launcher/{secrets-check,run-credential*,secrets-acl-runner}.mjs` | PRT-505/509；`secrets-acl-runner.test.mjs`（12 例，含真 `whoami`+`icacls`） | 见 §2「本轮已关掉的」；**C1～C4 是产品裁决**，属 ⏸ ★ **2026-09-24 业主裁决：C1～C4（凭证轮换、多档案、失败姿态、运维出口）归入「按真实客户需求推进」，暂缓** ⇒ 本格整行转 ⏸。★ 注意转 ⏸ **不是**说这一条没做：B1/B2 已关（真 runner/owner、`inspectSecretsAcl` 接上），win32 `0600` 是**已裁定的永久平台边界**；⏸ 只针对 C 系列那四个**产品形态**问题 |
| F-10 | 权限、审批与审计 | ✅ | `team-hub/permission-engine.mjs`、`approval-*.mjs`、`context-plan-store.mjs` | PRT-601～607 | ★ **[2026-09-24 未核（T5）]** 本行**没有任何可复跑的判据**（普查 `scripts/probes/feature-row-judges.mjs`：它既没有被任何**点名这个 F-id** 的套件覆盖，也没有在行内引用可复跑的套件名或判据脚本）。★ "✅"说的是**已交付**，不是"**有东西在核**" —— 这两件事在本表里长得一样，区别只在于后者能被人复跑。补判据（推荐：给 `run-ci.mjs` 加一个点名本 F-id 的套件，或在"依据"格里引一条可复跑的判据脚本）后请**同时撤掉本标记**（门禁会核"两份名单是否一致"）。 |
| F-11 | DSH 强制面（hard floor / sandbox / preset） | 🟡 | `runtime/dsh-composition/*` | PRT-213/214/253 的一部分 | 见 §2；G1/G4、G2/G3/G4/G6 需人裁决。★★ **另有一条本轮新发现（§5.2）：三道范围检查在生产里从未被注入**——`enforcementSurfaces()` 实测 `pathScope:false` / `whitelist:false`，`execution-scope`/`external-api-scope` 连端口都没有。这不是"F-11 没做"，而是"做了但没装上"；裁决见 §5 第 14 条 ★★★ **[2026-09-18 订正·逐句] 上面这段有三处要按新读数读**：① 「三道范围检查在生产里**从未**被注入」**对第一道已不成立**——`pathScope`（PRT-604）**已经接进生产组合根**（`40d2d60`）：范围表从**部署配置**来（`LEGION_PATH_SCOPE`，登记在 `runtime/config-schema.mjs`），经 `scope-port.mjs` 变成桥要的那个函数，在 `root-row.mjs` 接上；`production-scope-wiring.test.mjs` ①b 钉住"配了翻成 true"。② 所以那个 `pathScope:false` 读数**不是**"没接线"的证据，而是"**这次装配没有配范围表**"的读数——**没配 ≠ 接了个空的**，两者在读数上分得开了（这正是 `scope-port.mjs` 存在的理由）。③ ★ 「`whitelist` 连端口都没有」**是错的**：`createEnforcementBridge` 的入参表里**有它**（`runtime/dsh-composition/tool-request.mjs:517`，注释 `PRT-603 的岗位白名单`），`assembleEnforcement` 也照传（`assemble.mjs:135`/`:168`）——**位置在、没人给它值**。真正连位置都没有的只有 `execution-scope`（PRT-605）/`external-api-scope`（PRT-606）两道。★ 一句话：`pathScope` 已接（**配了才拦**）、`whitelist` **有位无值**、另外两道**无位**  ★★★ **2026-09-18 第 21 轮再订正一句：上面那处「`whitelist` **有位无值**」要读成「位置在、**值给不进去**」。** 第 21 轮把 `DECISION-RUNREQUEST-EXECUTION-PLANE.md` §11 留下的两处"我没有量"量掉了：唯一那个产出者（`permitsTool`）与桥的输入**词汇表不相交**（产出者认 **Legion 能力名**，桥交进来的是**执行面 DSH 名**——同一份 permit 喂前者放行、喂后者一个都不放行，抬 `maxRisk` 也救不了）⇒ 这一道**不是"等人配一个值"**，配了也只会得到一个**全拒**的强制面。已从第 14 条移出、立为第 **27** 条。详见 §4.4 与 §5.2 第 21 轮更新 ★★★ **2026-09-24（第 118 轮第三十八轮）再补一句：本行前面那句「真正连位置都没有的只有 `execution-scope`（PRT-605）/`external-api-scope`（PRT-606）两道」也已被取代** —— 这两道**都接上了**（第 19/20 轮，见 §5.2 的日期化更新），连同此前的 `pathScope`（PRT-604）与 `whitelist`（PRT-603），**四道全在**。⇒ 本行 🟡 的剩余**只剩人工裁决**：G1/G4（上位授权身份、win32 沙箱的"完整 vs 部分"判据）与 G2/G3/G4/G6（能力来源判据、沙箱口径、真实模型黄金跑），即 §5 第 3/4/5 条 —— **不是**"还有一道没接线"。⚠️ 判据：`node scripts/prt/spec-status-calibration.mjs` |
| F-12 | Product Launcher | ✅ | `product/launcher/*` | PRT-701～713 | ★ **[2026-09-24 未核（T5）]** 本行**没有任何可复跑的判据**（普查 `scripts/probes/feature-row-judges.mjs`：它既没有被任何**点名这个 F-id** 的套件覆盖，也没有在行内引用可复跑的套件名或判据脚本）。★ "✅"说的是**已交付**，不是"**有东西在核**" —— 这两件事在本表里长得一样，区别只在于后者能被人复跑。补判据（推荐：给 `run-ci.mjs` 加一个点名本 F-id 的套件，或在"依据"格里引一条可复跑的判据脚本）后请**同时撤掉本标记**（门禁会核"两份名单是否一致"）。 |
| F-13 | 配置与诊断 | ✅ | `product/config-schema.mjs`、`scripts/config/scan.mjs` | `scan --check`（1065 个疑似字面量）、`config sync`、`encoding-check` | — |
| F-14 | 安装、升级、回滚 | ✅ | `product/launcher/runtime-install.mjs`、`runtime/packs/*` | PRT-801～813、PRT-1001～1006 | ★ **[2026-09-24 未核（T5）]** 本行**没有任何可复跑的判据**（普查 `scripts/probes/feature-row-judges.mjs`：它既没有被任何**点名这个 F-id** 的套件覆盖，也没有在行内引用可复跑的套件名或判据脚本）。★ "✅"说的是**已交付**，不是"**有东西在核**" —— 这两件事在本表里长得一样，区别只在于后者能被人复跑。补判据（推荐：给 `run-ci.mjs` 加一个点名本 F-id 的套件，或在"依据"格里引一条可复跑的判据脚本）后请**同时撤掉本标记**（门禁会核"两份名单是否一致"）。 |
...
| F-15 | 闸门齐全而**汇总不存在**（grep `evidenceAggregate`/`rollup` 零命中）：`budget-ledger` 记了 `usage_records`，但"按员工花了多少"没有一个读出口 | `usage-rollup.test.mjs` 12 例 + `usage-rollup-http.test.mjs` 9 例 ★★ 2026-09-24 业主裁决：**第 2 条（PRT-009 执行期外读数）记永久环境边界** —— 不在本阶段采集执行期外读数，与本条①同类处置 ⇒ 本行的 `🟡→✅` **不再挂任何账**（不需要"等一台执行期外的机器"）。 |
...
| F-19 缺口① | 七类版本**没有落点**：`EmployeeManifest` 只是上下文来源，没有任何模块承载"这个岗位被冻结成哪一版" | `role-pack.test.mjs` 42 例（含对账两种漂移、嵌套强制面、连接器凭证），6 处守卫已**变异验证** |
...
> ⚠️ **被取代（2026-09-24）**：上面这段"**"连接器形状"这件事今天判不了**"已被**§5 第 23 条的裁决 + W-23 的施工取代**。
> 裁决采 ①：**新增 `connectorShape` 谓词端口**，判据是**名字形状**（`mcp__` 前缀，`isMcpPublicName`）+ "有没有已声明的命名空间与它匹配"，而不是本段主张的**来源**。
> 代价与残留如实记下：① 判形状**不拆 `__`**（嵌套命名空间不可判），所以理由里**不点名**那个命名空间，只点名"没有任何已声明连接器的命名空间与它匹配"；
> ② 一个**非 MCP 形状**的未声明名字（`totally-made-up`）仍然走政策门（"认不出就拒掉一切"不许），这一条由 decision-port ⑪ 钉住；
> ③ 本段下面那句"净效果仍是拒绝——但功劳在政策门"描述的是**当时**的读数，现在这一格的功劳在**登记表侧**（连接器层具名拒）。
> 判据：`runtime/connectors/decision-port.test.mjs` ⑩～⑮、`runtime/dsh-composition/plugins/root-row.test.mjs`「归属的**边界**」④。
...
- `LEGION_EXECUTION_SCOPE` 与 `LEGION_PATH_SCOPE`、`LEGION_CONNECTOR_DECLARATIONS`
  ...
  补那一个数组时三把键一起通。
  ★★★ **2026-09-24 订正：这一条已经关了**（原文保留：它记的是当时的读数与"三把键一起通"这个判断，
  而那个判断**是对的** —— 2026-09-20 补数组时四把键确实一起通了）。现在：
  四道范围表 + 岗位许可五把键全在 `envNames` 里；可复跑读数见上面的
  `node scripts/probes/_probe-r113-permit-delivery.mjs`。
...
- `LEGION_EXTERNAL_API_SCOPE` 同样**不在** `product/process-manifest.mjs` 的
  ...
  ★★★ **2026-09-24 订正：这个数组已经修了。** 四道范围表四把键（`LEGION_PATH_SCOPE` /
  `LEGION_CONNECTOR_DECLARATIONS` / `LEGION_EXECUTION_SCOPE` / `LEGION_EXTERNAL_API_SCOPE`）
  2026-09-20 一并进了 `runtime` 的 `envNames`（`product/process-manifest.mjs:266-311`），
  于是「每接一道就多一个受害者」这条**停在了第四道**。可复跑读数：
  `node scripts/probes/_probe-r113-permit-delivery.mjs`（五把键逐把「✅ 到了」）＋
  `node --test scripts/config/config.test.mjs`（schema `fields` ↔ 清单 `envNames` 必须对得上）。
  ⚠️ 上面那段「连续两轮都在涨」保留原文：它描述的是**修之前**的走势，那正是它值得记的理由。
...
| 族 | 代表模块 | 已在 §5 的哪一条 |
| --- | --- | --- |
| **生命周期 / 阶段 9 产品动作** | `product/lifecycle/{data-export,retention,uninstall}.mjs`（★★ **已清**：第 118 轮**第十六轮**第二刀接上——`--export-plan` / `--retention-plan` / `--uninstall-plan` 三面，走 `product/lifecycle/plan-cli.mjs`，而它们**共同的入参**由新模块 `product/lifecycle/store-scan.mjs` 从磁盘读出来。★ 这三个是**直接** import 的，不是连带） | 第 16 条 |
| **发布与合规** | —（★ 第 118 轮第十四轮**已清**：`product/release/{checklist,privacy}.mjs` 与 `product/support/runbook.mjs` 由 `legion --report=<kind>` 接进生产） | 第 16 条 |
...
| 15 | ★★ **PRT-610 执行面的落账点** | 项目主 + 产品 | `tool_calls` 的**表、读面、就绪证据的产出点**已经接上（本轮），而**写入方仍然是 0**：要接 `recordToolCall`/`markDispatched` 得先定"一次工具调用在哪一层落账"——`tools/pre-execute` 是**判定**点，而 `markDispatched` 必须在**真的派发之前**落库，那个位置比判定点更靠下 | 不接则这笔账永远是空的：表建好了、三条路由能读、就绪判据能回答"在不在记"，而**一条记录都不会有**。★ 这与第 14 条是**同一个决定**（执行面的载荷里今天没有这些字段），不是代码量问题。★★ 另：`release-gate.mjs` 的 `evaluateReadiness` **本身也零生产调用方**，所以"证据有了产出点"之后仍没有人在**发布决策**里读它——本轮只补齐了能被代码单独关闭的那一半 ★★★ 2026-09-24 业主裁定（本会话）：**落账层问题已移入第 28 条并已裁定（丙）**；工程残余 = 同一条 runId 缝 ⇒ **已裁决**。 |
| 16 | ★★ **阶段 9 产品动作的 CLI 面（PRT-903/904/905/908/909、PRT-712、PRT-707）** | 产品 | 这批模块（发布检查清单、隐私说明、保留策略、数据分类、数据导出、卸载、支持手册、首次运行向导、指标数据源、崩溃报告、★ **第 39 轮新增的 §7 指标口径与生产者**：`product/metrics-spec7.mjs` + `product/metrics-spec7-source.mjs`）**全部自带用例、全部 ✅、全部零生产入口**（系统盘点见 §5.3）。要决定的是：**这些"产品级动作"由谁触发**——`legion` 的子命令？一个独立的发布/运维 CLI？还是只作为发布流程里人工跑的一次性脚本？ | 不决定则它们**对用户不存在**。台账自己为 PRT-509 写过这句话：「一个功能没有入口，与这个功能不存在，对用户来说是同一件事」。★ 其中 `metrics-source.mjs` 的台账行已如实写了"还没有界面/CLI 消费者"，另外六个没有写。★★ **第 39 轮的补充**：§7 那六格指标的数据**已经在库里**、读出口**也写好了**（17 例断言、13/13 变异），于是这条决定的代价从"要不要做"变成了**"做好了给谁看"**——并且它**不能**用"顺手接进远程心跳"来绕过：那份载荷走**允许名单**，加一个键就是加一次**数据外流**，那是本条之外的另一个点头。★★ 另：PRT-801～813 的升级执行链**看起来**也在这个清单里，但它**不是**缺口——`runtime-install.mjs` 的注释写明 Launcher **刻意不 import** 它（进程卫生：Launcher 要在那些依赖起来之前先把进程看好）。**不要**给 Launcher 补这个 import，见 §5.3。它的执行者同样取决于第 1 条（PRT-011 DSH 分发形态） ★★★ **第 118 轮第十四轮：本条已裁决（业主 2026-09-23 裁定：做），第一刀已落。** 形式**不是**新造一个发布/运维 CLI，而是 **`legion` 的旗标**——沿用本仓已有三处**同一形状**的先例（`--log-policy` / `--runtime-install-plan` / `--diagnostics=`）。第一刀只切**三份纯读数**（`--report=checklist\|privacy\|runbook`，handler 在 `product/report-cli.mjs`）：入参**都是模块自己的默认**、零副作用、不碰数据，所以它**不必**先解决"输入从哪来"那道更难的题。可达性 41 → **36**（`gap` 20 → **15**）—— ★ 比预期**多两个**：`data-classes.mjs` 与 `crash-report.mjs` 是**被连带**接上的（谁都没有直接去接它们；一次接线让几个模块同时可达，而账只会报出"你记着的那几条"）。剩下 **6 个**（`data-export` / `retention` / `uninstall` / `metrics-source` / `metrics-spec7` / `metrics-spec7-source`）要目录、要模式、要库连接 ⇒ 各自需要那次裁决。 ★★ **第 118 轮第十六轮（第二刀）：那 6 个里清了 3 个** —— 而这一刀的**形式与第一刀不同**：它补的**不是 CLI，是一个读法**。`planExport` / `planRetention` / `planUninstall` 的主入口都要一份"磁盘上实际存在的落点"，而在这之前**仓里没有任何一处产生它**（`init.mjs:268` 的 `directorySize()` 只回一个总量）。⇒ 新增 `product/lifecycle/store-scan.mjs`（只读、有上限且**到上限要说出来**、不跟符号链接出根、**认不出就不猜一个类别**），再由 `product/lifecycle/plan-cli.mjs` 接三面：`--uninstall-plan=<mode>` / `--export-plan` / `--retention-plan`。可达性 36 → **33**（`gap` 15 → **12**）。★ 两条必须记下来的：① **`--diff` 报"过期 3 条"，而人读表里只记着 2 条**——第三条 `uninstall.mjs` 只在基线 JSON 里，没人记得（与上一轮同形，又对了一次）；② **差点走错的那条路**：这三个模块各自都导出**静态表**（类别表 / `DEFAULT_RETENTION` / `EXPORT_CLASS_POLICY`），"给 `--report` 加四个目录打印"能让 4 个模块**今天就变成可达**而**用户一个动作都按不到**——那正是 `reachability.test.mjs` ④ 自己写着的判据（"一个模块可达的读数与一条链真的跑了的读数，在只看探针汇总时是同一个东西"）。**没做**。剩下 **3 个**全是 metrics 那一支（要一个库句柄） |
...
| 2 | PRT-009 峰值资源采样 | 已裁决 | 需要一台执行期外的机器 |
| 3 | PRT-214 G1 | 已裁决 | 项目主 |
| 4 | PRT-214 G4 | 已裁决 | 产品 |
| 5 | PRT-253 G2/G3/G4/G6 | 已裁决 | 产品 + 项目主 |
| 6 | PRT-509 C1～C4 | 已裁决 | 产品 |
| 7 | 真实凭据 / 另一台机器 | **未标注** | 项目方 |
| 8 | F-23 / F-25 是否要做 | 已裁决 | 产品 |
| 9 | PRT-316 的开工资格 | 已裁决 | 项目主 |
| 10 | F-22 / F-24 是否按客户需求推进 | 已裁决 | 产品 |
| 12 | F-19 Role Pack 的"七类版本"范围确认 | 已裁决 | 产品 |
| 13 | F-21 判定面的接线资格 | 已裁决 | 项目主 + 产品 |
| 14 | 三道范围检查的生产接线 | 已裁决 | 产品 + 项目主 |
| 15 | PRT-610 执行面的落账点 | 已裁决 | 项目主 + 产品 |
| 16 | 阶段 9 产品动作的 CLI 面 | 已裁决 | 产品 |
| 17 | PRT-707 的两份实现用两个不同的模型密钥引用名 | 已裁决 | 产品 + 项目主 |
| 18 | F-18 / F-19 的"执行面一半"要谁来调用 | 已裁决 | 产品 + 项目主 |
| 19 | 第 13/14/15/18 条其实是一条决定，而且执行面拿不到控制面凭证 | 已裁决 |  |
| 20 | Runtime 契约服务端那一行要不要进补丁层 | 已裁决 | 产品 + 项目主 |
| 21 | 两条 ⏸ 要的不是机器、也不是凭据 | **未标注** | 项目方 |
| 22 | 连接器声明里的工具名该写「公开名」还是「裸名」 | 已裁决 | 产品 + 项目主 |
| 23 | 命名空间"认不出来"的那一类要不要按教义拒 | 已裁决 | 产品 + 项目主 |
| 24 | 政策门要不要从连接器声明里读「能力」 | 已裁决 | 产品 + 架构 |
| 25 | 「这个岗位能调哪些 MCP 工具」该由哪张表说了算 | 已裁决 | 产品 + 架构 |
| 26 | 外部 API 授权表要不要管 scheme | 已裁决 | 产品 + 架构 |
| 27 | 岗位清单说的是「Legion 能力名」还是「执行面工具名」 | 已裁决 | 产品 + 架构 |
| 28 | 那条"出站车道"的文件该放在哪个目录 | 已裁决 | 产品 + 架构 |
| 29 | RunRequest.env | 已裁决 | 产品 + 架构 |

⇒ **合计**：已裁决 **26** · 待施工 **0** · 待裁决 **0** · **未标注 2**（共 28 行）。

> ★ 这一句是**派生值**（`reachability.mjs` 的 `decisionStateTally()`），
> 而它此前写着 `已裁决 7 · 待施工 0 · 待裁决 4 · 未标注 18` ——
> **与它正上方那 29 行逐行相加的结果不符**（逐行是 8 / 0 / 3 / 18）。
> 两处矛盾而判据只盯逐行 ⇒ **它报 0 条**（第三十八轮实测）。
> 现在判据连这一句一起核（`TALLY_STALE` / `TALLY_MISSING`），
> 量具：`node scripts/probes/probe-decision-tally.mjs`。
> ★ 同日业主一次性裁了 **#6 / #8 / #9 / #10 / #29** 五行 ⇒ 8/0/3/18 → **13/0/2/14**
> （这一跳正是新判据第一次真的派上用场的地方）。

> 一张"状态写在散文里"的清单，与一张"状态写在列里"的清单，
> 在**读者从头读到尾**的时候是同一个东西 ——
> 而清单存在的理由，正是**没有人会从头读到尾**。

★★ **"未标注 20"是第 56 轮当时的读数**：那一轮的交付里说"需要人工介入的只有三件事"，
而那张表上**有 20 行没有写明自己还需要谁**。
（第三十八轮实测：**未标注 14** —— #16 / #26 / #27 此后各自拿到了显式裁决，
#6 / #8 / #10 / #29 由业主同日裁掉、#9 从「待裁决」转「已裁决」。
★ 这处数字与上面那句合计**不是**同一条判据管的：它是**那一轮的叙述**，不是派生值；
写在这里是保留历史，读者请以合计那一句为准。）
...
| **产品入口已定形**（第 118 轮第十四轮第一刀） | `product/release/{checklist,privacy}.mjs`、`product/support/runbook.mjs` —— 由 `legion --report=checklist\|privacy\|runbook`（`product/report-cli.mjs`）接进生产 | **不再是缺口**。★ 剩下没接的是 `product/lifecycle/{retention,data-export,uninstall}.mjs` 与 `product/metrics-source.mjs`（+ `metrics-spec7*.mjs`）：它们的输入要目录 / 模式 / 库连接，**碰数据** ⇒ 各有各的那一次裁决，**没有**在第十四轮顺手做掉。 ★★ **第 118 轮第十六轮补正**：前三个**已经接上了**（`--export-plan` / `--retention-plan` / `--uninstall-plan`，共同入参由 `store-scan.mjs` 从磁盘读）——它们要的"目录"其实**就是布局自己那几个根**（`resolveLayout` 给的，一个默认值都不用发明），所以它们**不需要**另一次业主裁决；真正要裁决的只剩 **metrics 那一支的库句柄**（只读打开 hub 的 `team.db`，那是"谁可以读运行中的库"那类问题） |
...
`decisionSourceRecorded` 在全仓**没有产出者**，因此永远判否。 ★★ **2026-09-24 订正**：这句读的是 `d955dac`（2026-09-23）**之前**的状态 —— 收账侧接进生产之后它**已有产出者**（`orchestrator/worker/toolcall-drain.mjs:285` 导出这个判据名，`release-gate.mjs:193` 消费它），且 `d955dac` 带端到端读数。**保留原样作历史，不再是当前读数。**
...
★ **2026-09-24 补**：这一段保留的是「脏树不影响那次读数」这个**方法论**结论（它仍然成立）；
而"条目 A"本身（那 4 把键不在 `envNames` 里）此后**已被取代** —— 2026-09-20 那五把键补了进去，
见本节 F-21 行与本页 §4.6 的订正。
...
（补丁层五行：hard-floor / root / pre-execute / approval-answerer / permission-presets）、（**订正 2026-09-24**：`pre-execute` 与 `approval-answerer` 今天标着"只能进程内挂载"，**不进** `legion-host.patch.yml`；这份名单与清单的权威是 `host-plane.manifest.json`）
...
## 5.29 口径落地：win32 sandbox 的「完整 vs 部分」与 PRT-253 G2/G3/G4/G6（W-4 / W-5）

> 本节回应 §5 第 4 条（win32 `sandbox-enforcement` 的"完整 vs 部分"）与第 5 条
> （PRT-253 G2/G3/G4/G6）。两条裁决都写着"**定口径**"，而口径与"待外部输入"
> 必须**分开记**（第 5 行原话）——混成一句时，"我们还缺一个真实凭据"会把
> "标准是什么"一起拖住。
>
> ★ 编号说明：本节用 `## 5.29`，是在**两级**编号（`## 5.N` 与 §5.23 里面的 `### 5.N`）
> 里取的最大号 +1 —— 我第一次写的是 `## 5.24`（被第 41 轮的 `### 5.24` 占了），
> 第二次写的是 `## 5.25`（被第 42 轮的 `### 5.25` 占了），**两次都被插入脚本的"已占用"检查拦下**。
> ★ 两个不同主题共用一个编号，读者会以为它们是同一件事的两半。

### ① win32 `sandbox-enforcement`：口径 = 八条拒绝理由的**闭集**（W-4）

「完整」不是一个形容词，是**八条**判据全部通过；任何一条不过 ⇒ `effective: false`，
自检不通过、worker 一侧拒绝自动执行。口径表（源码：`runtime/dsh-composition/selfcheck.mjs`
的 `probeSandbox`；判据：`composition.test.mjs` ⑳，53/53）：

| # | 分组 | 拒绝理由（源码逐字首段） | 这一条在防什么 | 性质 |
|---|---|---|---|---|
| ① | 服务面 | `未挂载沙箱服务：ctx.sandbox.confine 不可用（仅有配置名不算生效）` | "配了个沙箱名字"被当成"沙箱生效" | 可修（配置/装配） |
| ② | 服务面 | `confine() 拒绝为探针提供管制（fail closed）` | 后端明确拒绝时**不许**读成"没沙箱所以放行" | 可修（后端选择） |
| ③ | 服务面 | `confine() 未返回对象（收到 …）` | 契约被违反时不许猜管制程度 | 可修（契约） |
| ④ | 管制强度 | `confine() 未报告 enforcement` | "没报告" ≠ "已完全管制" | 可修（契约） |
| ⑤ | 管制强度 | `沙箱仅提供 partial 级管制：存在未被管制的执行路径…` | ★ **本行就是那个平台边界** | ★ **永久平台边界** |
| ⑥ | 包装证据 | `confine() 未返回管制后的 argv 数组` | 拿不到"管制后长什么样"就没法说它管过 | 可修（契约） |
| ⑦ | 包装证据 | `confine() 原样返回了输入 argv：未做任何包装` | "配了沙箱但没生效"最直接的证据 | 可修（实现） |
| ⑧ | 拒绝可识别 | `confine() 未提供非空 denialSignatures` | 拒绝无法被识别时会退化成普通失败 | 可修（契约） |

★ **与 §5 第 11 条① 一次收口**：第 11 条① 是"win32 上 `0600` **本机无法证明**"，
与第 4 条的 ⑤（`partial`）是**同一类**东西 —— **已裁定的永久平台边界**（本机不可测 /
平台只能做到部分管制）。两条的处置口径一致：**如实记成边界** —— 不因为它"没到 full"
就把它写成待办（那是把平台限制记成待修缺陷），也不因为"反正是平台限制"就把它从口径里
划掉（那会让"只有 partial"与"已完全管制"在读数上同形）。

★★ **我自己第一版把口径写成了"四项"**（服务 / full / 包装 / 拒绝签名）。
是 ⑳ 的**闭集断言**当场把它抓出来的：源码里有 **8** 条拒绝理由，我只读了 5 条
（`reasons.push` 那 5 条），把另外 3 条**早退分支**（`reasons: [...]`）整批漏了。
那一版若就这样落表，"⑥⑦⑧ 三条判据坏了"在表里**没有位置** ——
而"口径漏了一格"与"口径就是这四项"，在只看这条判据的用例里是同一个绿。

### ② PRT-253 G2/G3/G4/G6：**按条目读，不按编号读**（W-5）

§5 第 5 行给了 **4 个编号**（G2/G3/G4/G6）却只列了 **3 个条目**（能力来源判据 /
win32 沙箱口径 / 真实模型黄金跑）；整仓（含 PRT-214 / PRT-253 的文档）里**没有**
G 编号的定义表，同一个 `G4` 在第 3/4 行（PRT-214）与第 5 行（PRT-253）指的不是同一件事。
⇒ 本节的读法是：**按条目读、不按编号读**；编号与条目的对应关系**不发明**，
作为一处"说不上"如实留在这里（这比补一张猜出来的对应表安全：猜出来的表会被后来的人当成权威）。

| 条目 | 口径（本轮定） | 状态 | 证据（可复跑） |
|---|---|---|---|
| **能力来源判据** | 一个工具的能力 = **静态事实 ∪ 声明**；声明**只许抬升** —— 风险取 `max`、审批只做或运算、方向不许退回 `read`、不认识的能力 ⇒ `critical` | ✅ 已裁 + 已施工 | §5 第 24 条；`resolveToolWithDeclaration`（`tool-capability.mjs`）；判据 ⑩～⑭（44/44）+ 变异 `_probe-ruling24-declaration-raise.mjs` |
| **win32 沙箱口径** | 见上面 ①：八条闭集；`partial` 是永久平台边界 | ✅ 已裁 + 本批落表 | §5 第 4 条；`composition.test.mjs` ⑳（53/53）+ 变异 `_probe-w4-sandbox-caliber.mjs` |
| **真实模型黄金跑** | **不在这里定口径** —— 它缺的不是定义，是**外部输入**（能跑 DPAPI 的 Windows 机器 + 真实模型 API key） | ⏸ **挂 §5 第 7 条名下**（本批不改它状态、不单独追） | §5 第 7 条 |

★ **口径与"待外部输入"分开记**（第 5 行那句"不许混成一句"的要求）：上面两行是
**标准**（今天就复核得了、判据可复跑）；最后一行是**输入**（没有它，"单员工黄金任务跑通"
这句话只能停在"代码就绪"）。把两类写进同一句，会让"标准还没定"与"机器还没有"
看起来是**同一个**阻塞 —— 于是拿到机器的人以为还要等口径，定口径的人以为还要等机器。
