...
- **`未标注`**：该行**没有**任何显式状态标记。★ 我**不替业主判定**这一列 —— 那是裁决，不是抄写。
...
| 27 | 岗位清单说的是「Legion 能力名」还是「执行面工具名」 | 待裁决 | 产品 + 架构 |
| 28 | 那条"出站车道"的文件该放在哪个目录 | **未标注** | 产品 + 架构 |
| 29 | RunRequest.env | **未标注** | 产品 + 架构 |

⇒ **合计**：已裁决 **4** · 待施工 **0** · 待裁决 **4** · **未标注 21**（共 29 行）。

> 一张"状态写在散文里"的清单，与一张"状态写在列里"的清单，
> 在**读者从头读到尾**的时候是同一个东西 ——
> 而清单存在的理由，正是**没有人会从头读到尾**。

★★ **"未标注 21"就是这一轮的读数**：交付里说"需要人工介入的只有三件事"，
而这张表上**有 21 行没有写明自己还需要谁**。两者不可能同时成立 ——
要么那些条目已经解决（那就该写明"已裁决"），要么它们还开着（那"三件"就远远不够）。

★ 本表由 `scripts/prt/reachability.mjs` 的 `decisionStateRows()` **派生**，
判据 `reachability.test.mjs` ⑭/⑭b/⑭c 盯着"索引 = 今天派生出来的值"。
