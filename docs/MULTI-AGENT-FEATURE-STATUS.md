...
| 16 | 阶段 9 产品动作的 CLI 面 | 已裁决 | 产品（★ 第 118 轮第十四/十五轮：**第一刀已落** —— `legion --report=<kind>` 接上 3 份只读报告，`gap` 20 → 15、不可达 41 → 36；剩 6 个要目录/模式/库连接） |
| 17 | PRT-707 的两份实现用两个不同的模型密钥引用名 | 已裁决 | 产品 + 项目主 |
| 18 | F-18 / F-19 的"执行面一半"要谁来调用 | 已裁决 | 产品 + 项目主（★ 2026-09-24 业主裁定：**与第 28 条同一裁决** —— 控制面按 Run 投喂数据、执行面在 Run 结束时算；★ **接线尚未开始**，见本行详述） |
...
| 28 | 那条"出站车道"的文件该放在哪个目录 | 已裁决 | 产品 + 架构（**已裁决** —— ★ 2026-09-23 `d955dac` 裁定「丙的机制 + 目录锚在既有配置量上」**并已施工**；2026-09-24 业主复述 **丙**（按 Run 交付目录），与 `d955dac` 同一方向）；★★ **索引此前写"待裁决"是过期的**（★ 2026-09-23 `d955dac` 就已裁定「**丙的机制 + 目录锚在既有配置量上**」并**已施工**：目录逐 Run 一份、锚在 `layout.dataDir`；2026-09-24 业主复述为 **丙**，与 `d955dac` 同一方向 ⇒ 本条**不需要新裁决**）。★ **仍未落的一条**：runId 绑在**按 Run 安装**的缝（台账 §2.1 #4） |
| 29 | RunRequest.env | 已裁决 | 产品 + 架构 |

⇒ **合计**：已裁决 **15** · 待施工 **0** · 待裁决 **0** · **未标注 14**（共 29 行）。
> ★ **2026-09-24 业主对第 18 / 28 条一并裁定**（丙 + 同族一次裁完）⇒ 13/0/2/14 → **15/0/0/14**。
> ⚠️ **被取代的陈述**（★ 2026-09-24 补全，含**我自己当天写错的两句**）：第 28 条正文的"⚠️ 但生产里还没有人调它们"（第 22 轮状态）、"**位置未决**""**本轮不替业主选**""**待裁决**"，以及**我当天写进去的**"接线尚未开始""`decisionSourceRecorded` 依然没有产出者" ——这些读的都是**施工/裁定之前**的状态；
> 它们在本节**保留原样**（历史），但**不再是当前读数**——当前读数以索引表与本行合计为准。
