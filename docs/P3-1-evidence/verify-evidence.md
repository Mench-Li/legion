...
6. **前端为判定层测试**：房间/角色/错误文案的纯逻辑有 12 例单测，但**没有**浏览器自动化
   （不引入 jsdom/Playwright）；只读态按钮禁用、房间切换、复制链接等 DOM 行为未自动断言，靠人工核对。
7. **真实浏览器多房间协同未做跨机器验证**：端到端用的是进程内 Node `WebSocket` 客户端
   （与浏览器同为 RFC6455，但不等价于浏览器行为验证）。
8. **本轮未重启生产服务**：白板（:8080）未在本机生产实例上重启，P3-1 治理端点在生产上尚未生效；
   本机 :5173 生产实例未受影响（只重建了 dist 供 CI smoke 使用）。
...
- `whiteboard/.gitignore` 忽略 `apps/server/data/`（房间 DB、审计 JSONL 都在其下）与
  ...
  新增共享模块后**必须跑 `node scripts/build.mjs`**，否则浏览器端加载不到（本切片已跑，新增 `room.mjs` 已拷贝）。
