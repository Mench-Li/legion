...
| `whiteboard/apps/server/test/audit-archive.test.mjs`（新，**14 例**） | 10 条纯函数用例 + 4 条**真实进程**端到端用例（含真重启、含写入量量级） |
...
## 5. **自查发现的第二处自伤：逐条 presence 审计把限流用例打红了**

> 这一节记录的是**我自己引入的缺陷**，由**既有**用例抓到。如实写明，因为它比原候选更能说明本切片的纪律。

### 5.1 现象

P4-5 首次合入 `main` 后，冻结点门禁的 `test` 阶段 **FAIL**：

```
FAIL whiteboard: exit=1 tests=197 pass=196 fail=1
  ✖ 频率超限：先丢弃并回 warning，持续超限才断开
    Error: waitFor timeout; inbox=[{"type":"leave","clientId":"…"}]
```

该用例是 **P3-1 既有**用例（本切片未改动它）：连上房间后连发 260 条 presence，
期望超出突发额度（240）的部分被丢弃并回 `rate_limited` 告警。

### 5.2 先查清「是用例太紧，还是真回归」

单跑该用例 6/6 通过——所以不能据此判定用例脆弱。用门禁同款负载（白板全量并行）复跑：

| 版本 | 白板全量连跑 | 失败次数 |
| --- | --- | --- |
| P4-5 之前（`ee5ee03`） | 4 次 | **0** |
| P4-5 首次合入（逐条 presence 审计） | 3 次 | **1** |

于是写探针直接量因果变量（真实服务 + 真实审计目录 + 260 条 presence，跑 2 次取读数）：

| 变体 | 告警到达 | 审计行数 | 其中 `presence` 行 | `rateLimitDrops` | `presenceUpdates` |
| --- | --- | --- | --- | --- | --- |
| A：逐条写（首次合入的版本） | 78 / 82 ms | **262** | **249** | **11** | 249 |
| B：仅首次写（修复版） | 11 / 12 ms | 22 | **1** | **19** | 241 |
| C：P4-5 之前（无 presence 审计） | 14 / 13 ms | 21 | 0 | **19** | 241 |

**结论：这是真实回归，用例是对的。** 机制是可算的：`audit` 写文件是**同步** `appendFileSync`，
逐条 presence 审计把 249 次同步落盘塞进消息热路径；限流器令牌按 120/s 回填，
处理变慢就等于**多回填**——本该丢的 8 条被放行（19 → 11）。余量只剩 11 条时，
并行门禁负载稍微再慢一点就会让 `drops` 归零，告警永不出现 → `waitFor` 超时。
修复版 B 的行为与基线 C **完全一致**（19 drops / 241 updates / 告警 11ms），只多 1 条 presence 审计。

顺带确认了 `WB_AUDIT_DIR=''` 的既有语义陷阱：它被配置层当作「未提供」→ 回落到默认目录
`apps/server/data`，所以 e2e helper 的 `WB_AUDIT_DIR: ''` **并不会**关掉文件审计——
这也解释了为什么这段 I/O 会真的发生。

### 5.3 修法与锚定

- **修法**：presence 只在**该连接首次发时**记一条（`conn.presenceAudited`），
  条数上界 = 连接数，与鼠标移动次数无关；取证价值（「这个连接在房间里活跃过」）保留，
  逐次移动属行为画像，不进审计。
- **锚定**：新增 2 条真实服务用例，把「审计写入量必须与**消息数**同阶，不能与
  **更新次数/载荷大小**同阶」这条不变量本身钉住——200 条 presence 只应留 **1** 条；
  一条消息带 200 个 op 也只应留 **1** 条（摘要里体现 `count`/`kinds`）。
- **负向对照**：把两条新用例打在逐条版本上 → `AssertionError: 200 条 presence 只应留 1 条审计（实际 200）`。
- **复核**：修复后白板全量连跑 **5 次全绿**（对照：修复前 3 次里 1 次红）。
- **纪律沉淀**：`AUDIT_TYPES` 上加了「写入量纪律」注释，并要求新增类型时确认其写入量上界。
...
| 运行 | 结果 |
| --- | --- |
| `node --test apps/server/test/audit-archive.test.mjs` | **14/14 PASS** |
| 既有 `metrics-audit.test.mjs`（13 例，未改动） | **13/13 PASS**（既有语义零回归） |
| `npm test`（白板全量） | **199/199 PASS**（185 → **199**，+14）；修复后连跑 5 次全绿 |
...
9. **审计写文件仍是同步的（P3-1 的既有设计），未改成异步队列**：本切片只把「写入量」收敛到
   与消息数同阶（§5），**没有**消除单次写入的阻塞。热路径对同步落盘的敏感度已有实测
   （§5.2：249 次多余落盘即让限流行为偏移），因此**任何新增审计类型都必须先确认写入量上界**；
   若将来 ops 频率上到与 presence 同阶，需要先做异步批量落盘（含退出时 flush 的持久性权衡），
   那是独立切片。
10. **`presence` 只记「首次活跃」**，不记之后的任何更新，也**不记**「变为不活跃」——
    离开由既有的 `disconnect` 覆盖。所以审计能回答「活跃过」，**不能**回答「活跃了多久 / 什么时候停的」。

## 8. 复跑命令

```bash
# 新用例（纯函数 + 真实进程重启 + 写入量量级）
