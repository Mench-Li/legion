...
- ~~**未验证跨版本恢复**（老代码读新库、新代码读老库）。~~ ★ **2026-09-21 补：这一项已关闭**，
  见 `scripts/prt/backup-restore-cross-version.test.mjs`（套件 `prt-xver`，11 例，已进 `run-ci.mjs`）。
  两个方向各给一组读数：
  - **新代码读老库（前滚）**：夹具形状取自**真实历史**——`f53404e`（2026-08-25）那一版
    `team-hub/server.mjs` 只有 `tasks`/`members`/`audit` 三张表，今天有 50 张。老数据逐行对拍不丢；
    新表被建、新列被补；**补上来的每一列都可空或有默认值**；唯一一处**破坏性迁移**
    （`goal` 表 DROP+重建）保住行数、升级形状，且**幂等**（第二次打开不重建）。
  - **老代码读新库（回滚）**：老代码的 INSERT **从历史源码里抠出来真的跑了一遍**
    （不是重写一份），并断言老代码写过的每一列今天都还在、老表上没有被挂触发器。
    ⚠️ **边界如实保留**：这一向**没有真的去跑那一版 `server.mjs`**——它没有 `isMain` 守卫，
