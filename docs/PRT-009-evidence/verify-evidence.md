...
> ★★★ **2026-09-20 第 44 轮：整条链**第一次**被一次真读数走通；而"每次 Run 印出一行"那句仍然成立，因为**主动停止那条路把它跳掉了**。**
>
> 两件事，分开说。
>
...
>
>   于是"一台真进程 → 采样 → `status()` → launcher 那句映射 → 磁盘记录"**整条链
>   从来没有被一次真读数走通过**。第 44 轮把它走了一次（起一台真占内存的子进程）：
>
>   ```
...
>             磁盘上的 peakResource = 194.5 MiB，cpuMs=47（与内存里的读数**逐字段相等**）
>   ```
>
>   ⇒ 三格全绿：① 真进程 → `status()` 有真读数；② 退出印出**恰好一条**；
>   ③ 同一读数**原样**落到磁盘。
>
...
>
>   ⚠️ **这条读数覆盖的是机制，不是产品基线**：那台子进程是探针造的（占 140MB 好让采样看得见），
>   **不是**黄金任务那一次 Run。所以"资源基线**数值**"那一半**仍然**卡在 §4 第 15 条
>   （本机不会发生一次真的自动执行）。这次关掉的是"**接线都在，而从没被观测过**"那一半。
>
> **（二）而"每次 Run 真的印出一行"这句，当时还**不成立**——因为主动停止那条路把它跳掉了。**
>
> `product/launcher/supervisor.mjs` 的 `handleExit` 里（修之前）：
>
> ```js
...
> }
> ...
> reportPeakResource()         // ← 于是只在**非主动退出**时才走到
> ```
>
...
> | B 非主动退出 | 64 MiB | **1 条** |
>
> ⇒ 读数是**反的**：**崩溃那一次**会印，**正常那一次**不印。
>
> > 只在崩溃时印出峰值、在正常结束时把它丢掉，
> > 与"只测量出问题的那一次运行"是同一个东西——
> > 而资源基线要的恰恰是**正常那次**。
>
> 而同一条路上还有第二件事：`launcher.mjs` 的 `forgetRunRecord()` 在**正常停止**后
> 删掉落盘记录（五处调用，注释写明"它是**这次**运行的状态，不是历史"——那条是**有意的**）。
> ⇒ 两处叠加，**一次成功 Run 的峰值读数两处都不留**。
>
> **修法**：在主动停止那条分支里补一句，判据是 `!disposed`——
> `dispose()` 那条路上调用方很可能已经关掉 sink（`dispose()` 自己的注释就写着这件事），
...
>
> 唯一仍阻塞的那一项，原因**不是**「需要一次真实执行」——那个理由已经被用掉了。> 清单长期挂着同一个理由，读的人会默认它没变，清单就成了噪音；
