...
## 7b. ★★★ 提交之后才发现：对拍这套东西，**在干净克隆上本来就会坏**

提交完、九道门禁全绿之后，我再跑了一遍 `pair-routes` —— 它报了 `✖ 对拍未通过`。

失败信息是这样的：

```
=== 族：task-lifecycle（前缀 /api/claim；搬走前 ref=54c254d）===
  旧 handle 里 1 条；模块声明 11 条
✖ #0 POST exact /api/claim 体不同：
```

「旧 handle 里 **1** 条」——它**没有**用这一族的路径表，而是退回了一个前缀，
只圈住了 `POST /api/claim` 一条，然后拿这一条去比 11 条。

根因不在被测代码，在**取族函数的换行敏感**：

```js
const mp = /^\s+path: '([^']+)',$/.exec(line)   // ← 行尾必须是 `,` 然后结束
```

而本仓 `core.autocrlf=true`。我在排查破验残留时跑过一次
`git checkout -- team-hub/routes/task-lifecycle.mjs` —— git 按它的约定
把工作区写成了 **CRLF**，于是 `split('\n')` 每行末尾多一个 `\r`，
`',$` 匹配不上、`modulePathsOf` **静默返回 `null`**、对拍**退回前缀口径**。

★★ 关键在这里：**这不是本片引入的问题**。全仓 32 个族模块里有 **8 个**工作区是 CRLF ——
另外 7 个（`automation` `calendar` `compaction` `config` `experience` `packs` `secrets`）
**恰好都是前缀族**，退回的那个前缀正好是对的，所以一点症状都没有。
`task-lifecycle` 是**第一个工作区为 CRLF 的路径表族**，一撞上就现形。

★★★ 而 `autocrlf=true` 意味着：**一次干净克隆就会把全部 32 个模块都写成 CRLF**。
到那时，22 个路径表族会**一起**退回错误的前缀口径、**一起**报 ✖ ——
而失败信息长得像「体不同」，**读起来完全像搬错了内容**。

> *一个「内容我一行一行比过、都一样」的印象，
> 与一个「它是被 `\r` 挡住、压根没取到」的事实，
> 在我没有去看那个 `null` 是从哪来的时候是同一个东西。*

修法：`modulePathsOf` 在匹配前先剥掉行尾的 `\r`；工作区那份恢复成 git 在本仓期望的形态。
`HEAD` 里的 blob 一直是 **LF**（286 行、CRLF 0），与其余 31 个模块一致 —— 平台契约没被动过。

## 7c. ★★★ 同一轮还发现：一道核验**会在提交那一刻自己退化成永远通过**

`verify34.mjs` 的「改前」那一侧，第一版取的是 `HEAD:team-hub/server.mjs`。
提交**之前**跑，HEAD 还是「没搬」的那份，差集恰好 11 条、真的拦得住东西；
提交**之后**再跑，HEAD 已经变成「搬完了」的那份 —— 前后两侧成了**同一份内容**，
差集恒为空、这道核验**当场变成永远通过**，而且输出干干净净，看不出任何异常。

> *一个「提交前它拦下过一次真问题」的印象，
> 与一个「提交后它再也拦不下任何东西」的事实，
> 在我没有在**提交之后**再跑它一遍的时候是同一个东西。*

已改成取本族搬走前的 ref（`54c254d`），提交后复跑仍然 ✅。

## 7d. ★★★ 一次**过程事故**：`git commit --amend` 顶掉了同行的提交

补记上面这两件事时，我给证据文档与台账追加了一段，然后用 `git commit --amend`。
在那个瞬间，HEAD **已经不是**我的切片提交了 —— 同行在这之间提交了
`8cc39a7`（第 74 轮）。于是 amend 改的是**他们的提交**：它被顶掉、他们的改动
被折进我的提交里、还挂上了我的提交信息。

修法：`git reset --soft 8cc39a7` —— 只移动分支，**索引与工作区一个字节都不动**。
同行的提交连同作者与信息原样回来，他们的改动一个字没改。
**没有用 `git reset --hard`**：那会把同行大量**未提交**的工作一起抹掉。

> *一个「`--amend` 改的是我自己那个提交」的印象，
> 与一个「它改的是**当下的 HEAD**、而 HEAD 早已不是我的」的事实，
> 在我没有在 amend **之前**先看一眼 HEAD 是谁的时候是同一个东西。*

补记因此改成**普通提交**（`0174a2f`），并在事后核对过：
两个提交各自只含自己的文件，同行的 `8cc39a7` 原样在链上。
