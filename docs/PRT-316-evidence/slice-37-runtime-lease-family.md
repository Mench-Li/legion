...
累计：**188 = 154 已搬 + 34 剩余**（81.9%）；`sources` 53。

### ★★★ 补记：这里我先写成了 **152**，而它是错的

本片落地后的机器读数（`extractRoutes` 与 Σ`extractDeclaredRoutes`）是 **154 已搬 + 34 剩余**，
而我在台账与本文档里写的都是 **152**。

原因是**数错了哪一段**：本族搬走的是 **9 条**（`145 + 9 = 154`），
而我脑子里用的是**第二段**那 **7 条**（`145 + 7 = 152`）——
**我刚为它写了七条反向判据，那个"7"还在眼前。**

> 一个「这一族有 7 条」的印象（它确实有，但那是**不属于本片**的另一段），
> 与一个「本片搬走的是 9 条」的事实，
> 在我把「已搬」当成**手算**而不是当成**机器读数**的时候是同一个东西。

★★ 更值得注意的是：**这个数没有任何一道判据看着它。**
`baseline-snapshot --check` 盯的是 `httpRoutes` 的**清单**、不是"已搬/剩余"的**除法**；
`progress-check` 盯的是台账的派生计数（`140/145` 那一层），不是这一句散文里的数；
九道门禁**一道都不红**。它是**我写进散文里的一个自算数**，
只有靠事后拿机器读数去比才看得见。

★ 修法（本片就做）：台账与文档里的这个数改成**机器读数** `154 / 81.9%`，
并在这两处都留下这段补记 —— 因为"散文里的自算数没人看着"这件事本身才是根因。
