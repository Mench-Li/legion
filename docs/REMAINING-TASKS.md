# Legion 剩余待办清单
  ...
  **状态：本清单（P1-1～P3-3 共 14 项）已清空**，且**清单外候选已全部收口**——

  已完成 **9 项**（#1/#2/#3/#4/#6/#7/#8/#9/#10 → P3-4/P4-1～P4-7），**无待办候选**。
  ...
  候选 #7（指标与审计为进程内、审计不读历史文件）已于 2026-09-11 修复（**P4-5**），证据 `docs/P4-5-evidence/verify-evidence.md`；

  候选 #5（白板多实例共享存储未实现）已于 2026-09-11 定性并加固（**P4-6**）：ADR-0008 已否决多实例，本切片把「单实例」从注释升级为可执行目录锁，证据 `docs/P4-6-evidence/verify-evidence.md`；

  候选 #3（`notify` 套件曾超时一次、未复现）已于 2026-09-11 定位到机制并修复（**P4-7**）：主因是就绪轮询用无上界的 `fetch`（undici 默认 300s = 套件硬上限），证据 `docs/P4-7-evidence/verify-evidence.md`。

  全量基线：**39 套件 / 1049 用例，`run-ci --only test` PASS**（以 `docs/STATUS.md` 记录的数字为准）。
...
3. ~~**`notify` 套件曾超时一次、未复现**~~ → **已于 2026-09-11 定位到机制并修复（P4-7）**。先纠正原登记的一个错误前提：

   `node --test` 的文件是**并行**执行的，且一个文件的结果在它的**用例跑完时**就输出、**不等该文件进程退出**

   （三文件哨兵实验：hang 文件永不退出时其结果**始终没有**被打印，而 fast 的已打印）。所以「hub-smoke 已打印

   2 个用例通过」推不出「它已退出」——真实情况恰好相反：**用例全绿，卡住的就是 hub-smoke 自己**。

   机制级定位（三条缺陷都能独立造成该症状，括号内为实测读数）：

   ① **就绪轮询在就绪后不停**：`boot()` 的递归 `setTimeout` 在每个 tick 的 `finally` 里只要没到 15s 就再排一个 tick，

   于是连上之后仍每 120ms 打 HTTP 到期限（哨兵实测 **41 次 tick**，`afterAll` 时仍有 **4 个 pending Timeout**）。

   ② **轮询的请求没有上界（主因）**：旧实现直接 `fetch(...)`，而 undici 默认 `headersTimeout` = **300s**，

   **与套件级硬上限同值**。若 hub 已 accept 却迟迟不回响应（重负载下事件循环阻塞、或进程在错误时刻被杀），

   该请求会挂到 ~300s；它是 fire-and-forget 的后台轮询，所以**测试照常全绿**，进程却因挂着一个未结请求无法退出

   ——与记录的「用例全绿 / 此后无输出 / 恰好 300s」三个细节全部吻合。实测：黑洞服务端下裸 `fetch` **20s 未收敛**，

   加 `AbortSignal.timeout(150)` 则 **159ms** 被截断。

   ③ **SSE 订阅的 `off()` 不在 `finally`**：中途断言失败/用例被 abort 时永不执行，`MiniEventSource` 每 300ms

   **无上限**重连。实测真泄漏版 **卡死 >75s**，三处关闭齐全则 5s 正常退出。

   **修复**：可取消的有界轮询（就绪即停）+ **每个探测请求带 `AbortSignal.timeout`**（不可能挂到 300s）+

   订阅在 `finally` 关闭（含 `cleanup` 扫描与 `afterAll` 断言，共三道闸）+ 三类句柄登记表与兜底自检

   （把「静默卡死」变成带名字的失败）。

   **验证**：新增 2 条确定性回归用例（就绪即停 / 黑洞服务端下按时收敛）；用例 15 → **17**；

   单跑耗时 19s → **10–11s**（连跑 6 次一致），这正是「不再空转到 15s 期限」的直接体现。

   **证据**：`docs/P4-7-evidence/verify-evidence.md`（含 7 条未覆盖边界）。**注意**：原历史实例无法重放，

   本切片给出的是**机制级**定位与可复现的等价条件，不是对当年那一刻的现场复原。
...
5. ~~**白板多实例共享存储未实现**~~ → **已于 2026-09-11 定性并加固（P4-6）**：ADR-0008 **明确否决**了多实例

   （「满足任一触发条件即重新评估，不提前做」），三条触发条件当前**一条都不成立**，因此本切片**没有**去实现

   多实例（那需要外部 DB/Redis，与仓库「零第三方依赖」冲突）。转而实测了那个必然要问的相邻问题 ——

   **误把两个实例指向同一 `WB_ROOMS_DIR` 会怎样**：真起两个进程、各画一笔，得到的是**静默的数据分裂**

   （实例1 视图 1→2、实例2 视图 2→3，真值 4，**两侧日志一个字都没有**）；根因在应用层：

   各持一份内存 doc，且 `snapshot()` 的 `DELETE FROM ops` 会删掉对方尚未读到的 op（永久丢失）。

   旧代码里这只是一句注释（`rooms.mjs` 第 7 行「本模块假设唯一进程持有这些文件」）。

   **加固**：把单实例约束从注释升级为**可执行守卫** —— 房间目录独占锁 `.whiteboard.lock`

   （`wx` 原子创建、pid/端口/起始时间、**陈旧锁可接管**否则一次 SIGKILL 就永久锁死、

   release 只删自己的锁），第二个实例**启动即失败**并给出三条处置建议；`/healthz`、`/readyz`、

   `/metrics` 暴露守卫状态；接管陈旧锁时留启动告警 + 审计 `lock_takeover`。

   **验证**：真实双进程读数（第二个 `up=false` 非零退出、第一个不受影响；强杀后新实例接管并正常服务）；

   用例 **12 例**，负向对照（把锁换成「永远允许」）**9 条 FAIL**；白板全量 199 → **211**。

   **证据**：`docs/P4-6-evidence/verify-evidence.md`（含 8 条未覆盖边界）。**注意**：锁是**建议性**的，

   只防误操作；多实例的「正确」实现仍未设计（ADR-0008 被否方案 A 已补记实测读数与「接口不足」的结论）。
...
7. ~~**指标与审计为进程内**~~ → **已于 2026-09-11 修复（P4-5）**：侦察先把候选拆准——`/metrics`
   ...
   **验证**：真实进程**真杀真启** A/B；用例 **14 例**（10 纯函数 + 4 真实进程），白板全量 185 → **199**；
   两组负向对照（未改代码导入即失败；新版 audit + 旧版 index 报 `应含 ops：connect,room_open`）。
   **自查中还抓到一处自伤**：逐条 presence 审计把既有「频率超限」用例打红（实测 249 次多余同步落盘 → 丢弃数 19→11、告警 12ms→80ms）；已改为按连接记一条，并把「审计写入量须与消息数同阶」钉成 2 条断言（负向对照：打在旧版本上 `实际 200`）。
...
8. ~~**`serve.mjs` 对未知资源路径仍回退 SPA 200 HTML**~~ → **已于 2026-09-10 修复（P4-4）**：
   ...
   新增判定纯函数 `staticRequestKind(req, pathname)`——只有**导航**请求（路径段无扩展名 / `.html`/`.htm` /

   `Accept` 含 `text/html`）才回退 SPA 入口；其余按**静态资源**处理，缺失即 **404 + 可读 JSON 体**。

   旧行为把「文件不存在」伪装成 200 整页 HTML：前端 `fetch` 缺失 JSON 只会看到

   `JSON 解析失败: Unexpected token '<'`，浏览器取缺失 `.js` 只会看到 MIME 报错；真实触发场景是

   **发版后仍持旧 `index.html` 缓存的浏览器去请求上一版哈希产物**（vite 产物为 `assets/index-<hash .js`）。

   **验证**：真实浏览器 A/B（`200 text/html` + 解析失败 → `404` + `{"error":"未找到静态资源…"}`）、

   用例 6 → **16 例**（3 条真实服务契约 + 7 条判定纯函数，含负向锚定），负向对照在未修代码上 **9 FAIL**。

   **证据**：`docs/P4-4-evidence/verify-evidence.md`。
...
10. ~~**白板「重连窗口内的绘制会被静默丢弃」**~~ → **已于 2026-09-10 修复（P4-3）**：`main.mjs` 的

    `send()` 原先只在 `ws.readyState === OPEN` 时发送、其余情况静默 return，于是「连接尚未建立的窗口里

    画的东西」①服务端从未收到、②`welcome` 把 `doc` 换成服务端文档后**屏上也一起消失**——是双重静默丢失。

    修法：未就绪时 op **入队**（有界，超限丢最旧并报出丢弃条数）+ **可见提示**；`welcome` 后

    `flushPendingOps()` 补发（按 `welcome.limits.maxOpsPerMessage` **分块**——超限会被服务端以 1008 关连接；

    并在本地重放一次，因为服务端广播带 `except`、不回显给发送者）；切房间丢弃旧房间暂存但**如实提示**；

    只读角色与 presence 分别「不发送但提示」「不入队」；提示条按**优先级**占用（治理类   连接状态类，

    抽成纯函数 `shared/notice.mjs`）——这条规则来自本轮自造的一次回归：队列提示把「操作过于频繁」顶掉了，

    被**既有**的 P4-1 限流 e2e 用例抓到（证明那条断言确实在干活）。

    **验证方式**：A/B 现场读数（同一探针只换 `main.mjs`）——修复前：重连后服务端元素 **0**、本地画布墨迹 **0**；

    修复后：服务端 **1**、本地墨迹 **1516** 像素。浏览器 E2E 由 7 → **10 例**，新增 ⑧「连接抖动窗口」与

    ⑨「切房间后立刻绘制」两条竞态用例与 ⑩「切房间只建一个连接」，并做了**负向对照**（用例打在未修/停用守卫的代码上必 FAIL）；

    另有 27 例纯函数单测（队列/补发 19 + 提示优先级 8）。**证据**：`docs/P4-3-evidence/verify-evidence.md`；手册：`docs/E2E.md` §6.1。

    **未覆盖（诚实登记）**：入队 op 沿用原 stamp（并发冲突按 LWW 取舍）；队列有界（500）极端离线仍会丢最旧；

    「已写 socket、未到服务端」的在途 op 仍需 ack/重传协议才能救；真实网络故障（TCP 半开）下前端拿不到

    `onclose`，行为未验证。**顺带修掉两条追查中发现的真实缺陷**：① 补发原先「`drain()` 后不看 `send` 返回值」——写不进去就静默丢，

    等于在修复里重演缺陷，已改为 `sendInChunks` 只把**真正写进 socket** 的算发出去、其余回队重试；

    ② 切房间时旧 socket 的 `onclose` **无条件重连**，同一个房间连出两个 socket（实测「3 个 socket、2 个同时 OPEN」），

    已加陈旧连接守卫（只有当前 socket 的事件才算数）。
