...
> 全量基线：**39 套件 / 1002 用例，`run-ci --only test` PASS**（以 `docs/STATUS.md` 记录的数字为准）。
...
10. ~~**白板「重连窗口内的绘制会被静默丢弃」**~~ → **已于 2026-09-10 修复（P4-3）**：`main.mjs` 的
    ...
    **验证方式**：A/B 现场读数（同一探针只换 `main.mjs`）——修复前：重连后服务端元素 **0**、本地画布墨迹 **0**；
    修复后：服务端 **1**、本地墨迹 **1516** 像素。浏览器 E2E 由 7 → **9 例**，新增 ⑧「连接抖动窗口」与
    ⑨「切房间后立刻绘制」两条竞态用例，并做了**负向对照**（用例打在未修代码上必 FAIL，失败文本即症状）；
    另有 19 例纯函数单测（队列 11 + 提示优先级 8）。**证据**：`docs/P4-3-evidence/verify-evidence.md`；手册：`docs/E2E.md` §6.1。
