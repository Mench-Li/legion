...
2. **`dual-write` 有一次未定性的偶发失败**（2026-09-10 全量运行中观察到 1 次；此后单跑 5 次、8 进程负载下 2 次、
   两次全量运行均未复现）：该套件断言双进程写同库时 `audit.seq` / task id 全唯一，是 P1-1 的回归锚点，
   **不能含糊**。当时失败原文丢失（已补「失败套件原始输出落盘」，路径 `.ci/<run>/suites/<套件>.log`）——
   下次复现应立即用该产物定性：若确为 `audit.seq` 撞号，则是 P1-1 修复未覆盖的并发窗口（真实缺陷）；
   若为 `SQLITE_BUSY` 超时，则应评估 `busy_timeout=5000` 在负载下是否偏小。
3. **插件配置面未统一**（P3-2 已登记边界）：`plugins/` 读取 `CHAT_CTX_BUDGET_CHARS` /
...
4. **白板多实例共享存储未实现**（P3-1 已登记）：当前是单实例多房间；ADR-0008 记录了被否方案与
   「转 v2」的触发条件（真正需要横向扩展时再启动）。
5. **前端仍无浏览器自动化**（P3-1 边界）：房间/角色/限流相关的前端行为靠判定层测试 +
   静态契约测试覆盖，DOM 行为不自动断言；现场行为以人工走查为准。
6. **指标与审计为进程内**（P3-1 边界）：重启归零、无长期归档；`/api/rooms/<id>/audit` 只返回当前进程事件。
7. **`serve.mjs` 对未知资源路径仍回退 SPA 200 HTML**（`docs/STATUS.md` 限制 #12）：既是 SPA 需要，
   也让「资源不存在」的探测更费解，属于可继续收敛的小项。
8. **宿主插件导入失败的诊断可读性**：本次排查中，`team-hub/lib` 缺失在宿主侧表现为「60s 未就绪」
   或「插件路由 404」两种形态，定位只能靠翻宿主子进程 stderr。若在 p13 fixture 层面把
   「插件条目导入失败」直接翻成明确的断言失败（而不是超时/404 间接症状），同类问题的定位会快得多。
