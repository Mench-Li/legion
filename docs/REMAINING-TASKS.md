...
> 全量基线：**39 套件 / 994 用例，`run-ci --only test` PASS**（以 `docs/STATUS.md` 记录的数字为准）。
...
10. ~~**白板「重连窗口内的绘制会被静默丢弃」**~~ → **已于 2026-09-10 修复（P4-3）**：`main.mjs` 的
    ...
    **验证方式**：A/B 现场读数（同一探针只换 `main.mjs`）——修复前：重连后服务端元素 **0**、本地画布墨迹 **0**；
    ...
    另有 11 例队列纯函数单测。**证据**：`docs/P4-3-evidence/verify-evidence.md`；手册：`docs/E2E.md` §6.1。
