...
1. **CI `test` 阶段无法整体跑完**（影响基线可信度）：`notify-hub-smoke.test.mjs` 在 main 上永久挂起，
   导致 `run-ci --only test` 无法完成，`docs/STATUS.md` 的「全量基线」长期滞后于实际套件数。
   根因与最小修法已记在 `docs/P2-7-evidence/verify-evidence.md` §7，但**一直未修**——
   目前所有套件都是逐个单跑验证的（未伪造全量基线）。**修掉它能让基线重新变成一句话可复现的结论。**
   *（注：2026-09-10 复核时该挂起仍可复现；另有一条同类隐患已修：P3-2 实测出「工作树通过、主检出失败」
   的两例环境依赖缺陷。）*
2. **插件配置面未统一**（P3-2 已登记边界）：`plugins/` 读取 `CHAT_CTX_BUDGET_CHARS` /
...
3. **白板多实例共享存储未实现**（P3-1 已登记）：当前是单实例多房间；ADR-0008 记录了被否方案与
   「转 v2」的触发条件（真正需要横向扩展时再启动）。
4. **前端仍无浏览器自动化**（P3-1 边界）：房间/角色/限流相关的前端行为靠判定层测试 +
   静态契约测试覆盖，DOM 行为不自动断言；现场行为以人工走查为准。
5. **指标与审计为进程内**（P3-1 边界）：重启归零、无长期归档；`/api/rooms/<id>/audit` 只返回当前进程事件。
6. **`serve.mjs` 对未知资源路径仍回退 SPA 200 HTML**（`docs/STATUS.md` 限制 #12）：既是 SPA 需要，
