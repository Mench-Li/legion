...
**最近一次全量基线**：2026-09-12　`run-ci --only test` **PASS**；其中 `test` **79 套件 / 2014 用例**
（**须设 `DSH_CHECKOUT`**：不设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数为 77 套件 / 1792 用例）
—— 以本文件所在提交为准；证据 `.ci/2026-09-12T05-17-29-654Z/`
...
**当前基线：79 套件 / 2014 用例，`--only test` 整体 PASS** —— 2026-09-12 实测（设 `DSH_CHECKOUT`）
...
`DSH_CHECKOUT` 未设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数为 77 套件 / 1792 用例。
上一批基线 76 套件 / 1969 用例（PRT-501 模型档案）。
