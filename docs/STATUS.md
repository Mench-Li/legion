...
**最近一次全量基线**：2026-09-12　`run-ci`（**8 个阶段全 PASS**）；其中 `test` **129 套件 / 3334 用例**
（**须设 `DSH_CHECKOUT`**：不设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数会少）
—— 以本文件所在提交为准；证据 `.ci/prt-616/`（PRT-616 allow-once 占位那一批）
...
> **本轮（PRT-616：`allow-once` 的原子占位 —— 行级 CAS 看不见的那一半）**：
>
