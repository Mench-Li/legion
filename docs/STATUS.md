...
**最近一次全量基线**：2026-09-11　`run-ci --only test` **PASS**；其中 `test` **57 套件 / 1550 用例**
...
> **本轮（阶段 2.5 产品层契约：PRT-258 第一批）**：新增 `product/`（`paths.mjs`、
> `process-manifest.mjs`、`index.mjs`、`index.d.mts`）与套件 `product-runtime`（**29 例**）。
> 56→**57** 套件、1521→**1550** 用例。交付物是**启动之前就能判定的事实**：
> 五进程清单与启动波次、per-user 目录布局不变量、配置优先级合并（带 provenance）、
> 以及「普通配置文件不得含明文密钥」的写入门禁。
>
> ⚠️ 三条**未交付**必须记住：**Secret Store 接口**、**Launcher 本身**，以及
> **五条就绪判据全部标了 `verified: false`**（清单里的 `GET /api/config` 等判据
> 尚未在真实进程上量过响应码与耗时，实测归 PRT-703）。`MANIFEST_KNOWN_GAPS`
> 把两处入口缺口（`orchestrator` 入口未创建 / `runtime` 入口未配置）写成机器可读事实，
> 并由用例与**真实仓库**对账——缺口补上时用例会**变红**，逼文档与清单一起更新。
> 详见 `docs/superpowers/prt/PRT-258-product-contracts.md`，以及本轮新增的
...
**当前基线：57 套件 / 1550 用例，`--only test` 整体 PASS** —— 2026-09-11 实测
（阶段 2.5 产品层契约：新增 `product-runtime`（**29 例**：PRT-258 第一批）。
这一组的核心断言是**启动之前就能判定的事实**——依赖顺序、端口唯一、入口存在性、
目录不变量与配置优先级。它们的共同点是：错了也**不会抛异常**，
表现分别是偶发 500、后启动者静默退出、任务一直没人做、升级吞数据、
「改了配置却不生效」。其中「入口不存在」这一类还被做成与真实仓库的**对账**：
缺口补上时用例变红，逼文档与清单一起更新。
`product/` 属 `mustBeZeroPrefixes`，本批次四个文件对执行面依赖为 **0 处**。
56→**57** 套件、1521→**1550** 用例；
