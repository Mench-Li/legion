...
**最近一次全量基线**：2026-09-11　`run-ci --only test` **PASS**；其中 `test` **58 套件 / 1559 用例**
...
> **本轮（阶段 2.5 产品层契约）**：两份交付。**① PRT-258 前三份契约**——
> 新增 `product/`（`paths.mjs`、`process-manifest.mjs`、`index.mjs`、`index.d.mts`）
> 与套件 `product-runtime`（**29 例**），交付物是**启动之前就能判定的事实**：
> 五进程清单与启动波次、per-user 目录布局不变量、配置优先级合并（带 provenance）、
> 以及「普通配置文件不得含明文密钥」的写入门禁。
> **② PRT-505 密钥库**（即 PRT-258 的第四份契约）—— 新增 `security/secrets/`
> （`ref.mjs` / `errors.mjs` / `dpapi.mjs` / `store.mjs`）与套件 `secret-store`（**9 例**，
> 含 **1 例真实 DPAPI 往返**）。这一组的断言集中在两类**不会抛异常**的失败上：
> 元数据接口（`list`/`toJSON`/审计/错误对象）把值或密文带出去，
> 以及受保护后端不可用时**退化**为明文。
> 56→**58** 套件、1521→**1559** 用例。
>
> ⚠️ **未交付**必须记住：**Launcher 本身**（`PRT-251`/`PRT-704`）、
> **五条就绪判据全部标了 `verified: false`**（清单里的 `GET /api/config` 等判据
> 尚未在真实进程上量过响应码与耗时，实测归 `PRT-703`）、
> 以及**密钥库目前没有任何生产调用方**（接线点是 `PRT-501` 与 `PRT-254`）。
> `MANIFEST_KNOWN_GAPS` 把两处入口缺口（`orchestrator` 入口未创建 /
> `runtime` 入口未配置）写成机器可读事实，并由用例与**真实仓库**对账——
> 缺口补上时用例会**变红**，逼文档与清单一起更新。
> 详见 `docs/superpowers/prt/PRT-258-product-contracts.md` 与
> `docs/superpowers/prt/PRT-505-secret-store.md`，以及本轮新增的
...
**当前基线：58 套件 / 1559 用例，`--only test` 整体 PASS** —— 2026-09-11 实测
（阶段 2.5 产品层契约：新增 `product-runtime`（**29 例**：PRT-258 前三份契约）与
`secret-store`（**9 例**：PRT-505 密钥库 = 第四份契约，含真实 DPAPI 往返）。
前者的核心断言是**启动之前就能判定的事实**——依赖顺序、端口唯一、入口存在性、
目录不变量与配置优先级。它们的共同点是：错了也**不会抛异常**，
表现分别是偶发 500、后启动者静默退出、任务一直没人做、升级吞数据、
「改了配置却不生效」。其中「入口不存在」这一类还被做成与真实仓库的**对账**：
缺口补上时用例变红，逼文档与清单一起更新。
后者的核心断言是**六条出口各自脱敏**与**不退化**——元数据接口带出密文、
明文后端被当成真实密钥库，这两件事都不会报错，只会让「密钥受保护」这句话变成空话。
`product/` 与 `security/` 均属 `mustBeZeroPrefixes`，本批次十个文件对执行面依赖为 **0 处**。
56→**58** 套件、1521→**1559** 用例；
