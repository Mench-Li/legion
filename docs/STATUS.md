...
**最近一次全量基线**：2026-09-12　`run-ci --only test` **PASS**；其中 `test` **85 套件 / 2148 用例**
（**须设 `DSH_CHECKOUT`**：不设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数为 83 套件 / 1926 用例）
—— 以本文件所在提交为准；证据 `.ci/2026-09-12T05-52-43-240Z/`
...
**当前基线：85 套件 / 2148 用例，`--only test` 整体 PASS** —— 2026-09-12 实测（设 `DSH_CHECKOUT`）
...
`DSH_CHECKOUT` 未设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数为 83 套件 / 1926 用例。
上一批基线 82 套件 / 2089 用例（PRT-503/510/511 运行预算与价目表）。
