...
**最近一次全量基线**：2026-09-12　`run-ci --only test` **PASS**；其中 `test` **95 套件 / 2361 用例**
（**须设 `DSH_CHECKOUT`**：不设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数为 93 套件 / 2313 用例）
—— 以本文件所在提交为准；证据 `.ci/2026-09-12T07-36-20-989Z/`
...
**当前基线：95 套件 / 2361 用例，`--only test` 整体 PASS** —— 2026-09-12 实测（设 `DSH_CHECKOUT`）
