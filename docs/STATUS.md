...
**最近一次全量基线**：2026-09-12　`run-ci --only test` **PASS**；其中 `test` **65 套件 / 1803 用例**
（**须设 `DSH_CHECKOUT`**：不设时 `plugins/board-plugin` 按纪律 SKIP，计数为 63 通过 + 1 跳过 / 1581 用例）
—— 以本文件所在提交为准；证据 `.ci/2026-09-12T03-48-00-449Z/`
...
> 63→**65** 套件、1801→**1803** 用例。阶段 3 由 10/16 进到 11/16。
>
...
**当前基线：65 套件 / 1803 用例，`--only test` 整体 PASS** —— 2026-09-12 实测（设 `DSH_CHECKOUT`）
（PRT-312 真实进程被强杀：新增 `run-kill-drill`（**2 例**，真 worker 进程 + 真 team-hub + `SIGKILL`）；
PRT-314 多进程并发新增 `run-concurrency`（**5 例**，竞争者是真的操作系统进程）；
`run-policy`（**14 例**：PRT-309/310/311 的重试额度、退避、Dead Letter、人工处置、幂等键）。
`DSH_CHECKOUT` 未设时 `plugins/board-plugin` 按纪律 SKIP，计数为 63 通过 + 1 跳过 / 1581 用例。
上一批基线 63 套件 / 1801 用例（PRT-314 多进程并发 + 原子迁移原语）。
