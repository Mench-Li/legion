...
**最近一次全量基线**：2026-09-12　`run-ci --only test` **PASS**；其中 `test` **93 套件 / 2313 用例**
（**须设 `DSH_CHECKOUT`**：不设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数为 92 套件 / 2295 用例）
—— 以本文件所在提交为准；证据 `.ci/2026-09-12T07-22-10-942Z/`
...
**当前基线：93 套件 / 2313 用例，`--only test` 整体 PASS** —— 2026-09-12 实测（设 `DSH_CHECKOUT`）
...
`DSH_CHECKOUT` 未设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数为 92 套件 / 2295 用例。
上一批基线 91 套件 / 2268 用例（PRT-252 模型配置校验）。
