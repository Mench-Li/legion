...
★ 而第 2、3 行要一个含构建产物的 DSH 检出才跑得起来：按本仓典藏 CI 口径
（不设 `DSH_CHECKOUT`）它们**跳过**，所以"真 DSH 进程读到了"这条证据
**不出现在 CI 摘要里**，需要单独一次带检出的运行
（读数来自 `product/launcher/run-credential-dsh-process.test.mjs`）。
