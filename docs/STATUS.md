...
**最近一次全量基线**：2026-09-12　`run-ci`（**8 个阶段全 PASS**）；其中 `test` **131 套件 / 3378 用例**
（**须设 `DSH_CHECKOUT`**：不设时 `plugins/board-plugin` 与 `plugins` 按纪律 SKIP，计数会少）
—— 以本文件所在提交为准；证据 `.ci/prt-610/`（PRT-610 工具调用持久化那一批）
...
> **本轮（PRT-610：工具调用的持久化 —— 决定来源、原始/canonical 输入、结果四态、幂等键）**：
>
