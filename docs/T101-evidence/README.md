...
| 10-m1-chatview-merge.txt | node scripts/probes/repro/m1-merge-ghost.mjs | **F1 复现**：3 FAIL（源消息 meta 不随 replied/failed 流转；chat:fail 无 SSE 消费）|
| 11-m2-grants-leak.txt | node scripts/probes/repro/m2-leak.mjs | **F2 复现**：1 FAIL（A 改版 pending 后 B 复审视图带出草稿全文）|
| 12-m3-window-ghost.txt | node scripts/probes/repro/m3-ghost.mjs | **F3 复现**：2 FAIL（窗口外 awaiting 不 failed、不进队）+ 对照组 PASS |
| 13-o2-unwrap.txt | node scripts/probes/repro/o2-unwrap.mjs | **O2 复核**：7/7 PASS（.task 消费链一致，未复现 undefined）|
