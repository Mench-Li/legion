...
## 5. 补掉"闸门永不被建起来"的最后一环：记账主体的兜底来源

§4 里那条「`LEGION_BUDGET_ACTOR` 没有生产注入点」是本轮**自己**修的。
修法不是往 Launcher 里写一个变量，而是先问：**这个主体本来该从哪儿来？**

答案是这个进程**已经被赋予**的身份：`LEGION_WORKER_ID`。
...
| # | 来源 | 说明 |
| --- | --- | --- |
| ① | `io.budgetActor` | 部署可以点名一个非 worker 身份 |
| ② | `LEGION_BUDGET_ACTOR` | 想换成一个业务身份时用它 |
| ③ | `LEGION_WORKER_ID` | 进程自身的身份，真实且可追溯 |
| ④ | 都没有 | **不建闸门**，且 `budgetState: 'not-gated'` 可见 |

### 为什么 ③ 可以有兜底，而 `budgetActor` 本身不能有默认值

这两件事不一样：

- 一个编出来的占位符（`'anonymous'`、`'system'`）会让
  「没人签名」与「某人签了名」在账本里长得一样——**所以不给**。
- `LEGION_WORKER_ID` 不是编的：它是这个进程**真实且可追溯**的身份。

而不给 ③ 的代价很具体：

> 一个默认不生效的闸门，与一个不存在的闸门，在"有没有拦住过"上是同一个答案。

变红验证 3/3：去掉兜底、优先级反转、兜底换成 `'anonymous'` 都会让用例红。

### 一条撤出的探针，以及为什么留档

原本还有一条探针瞄"身份两端的空白不被清掉"。**它咬不住**，而原因值得记：
trim 这件事在**三层**都被做了一遍——

1. `executor-binding.mjs` 的 `clean`
2. `executor.mjs:186` 的 `budgetActor.trim() !== ''` 守卫
3. `budget-gate.mjs:156/184` 的 `actor.trim()`

纵深防御是好事，代价是"**这一层**有没有 trim"在行为上不可观察。

> 一个被悄悄删掉的探针，与一个从来没写过的探针完全同形。

所以它不是被删掉，而是**留成注释记录**在探针脚本里，连同撤出的理由。
...
- ~~`LEGION_BUDGET_ACTOR` 没有生产注入点~~ **本轮已修**（见 §5）：
  退回到 `LEGION_WORKER_ID`，那是进程自身的身份，不是编出来的占位符。
  于是闸门在真实部署里**会**被建起来，不再需要等 Launcher 先写一个变量。
