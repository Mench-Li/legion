...
本批把 PRT-509 留下的三个缺口里**两个可动的**关上；第三个（win32 `0600`）
随后于 2026-09-18 裁定为**永久平台边界**并关闭（见 §11.3）。
...
### 11.3 第三（win32 `0600`）：已裁定为**永久平台边界**，本条关闭（2026-09-18）
...
**裁定（2026-09-18）**：这一条不再作为"待办缺口"挂着，改为**记录在案的平台边界**。
理由是平台语义而非本仓缺陷：POSIX 权限位在 Windows 上不存在，Node 的 `chmod` 只影响只读位、
`stat().mode` 恒为可读写形态，所以"文件模式是 0600"这个断言在 win32 上**既不能证明也不能证伪**。
"仅所有者可读写"这个**意图**在两个平台上都有读数（机制那一半由另一条用例在所有平台上数出来），
只有 win32 上的**表达**无法观测。

> 诚实边界：本条关闭的方式是**裁决**，不是**测出来**。
> 把它读成"win32 上验过了"是错的读数——正确的读数是"这个平台不提供这个观测量"。
...
### 11.5 三个缺口的当前读数（2026-09-18 实测，`main = c651135`）

| 缺口 | 读数 | 结论 |
| --- | --- | --- |
| ② 生产默认句柄工厂 | `✔ ★★★ 生产默认的句柄工厂：不注入任何东西，真的走一遍 openProductSecrets`（1514ms）；同套件 `tests 21 / pass 20 / skipped 1` | **已关**，且在 win32 上**真跑**（真 DPAPI + 真 `icacls`） |
| ③ 真 DSH 进程 | `pass 1 / fail 0 / skipped 0`，4249ms——真 `apps/cli` + 真 profile + 真 `--patch` 覆盖层 | **已关**（需一个含构建产物的 DSH 检出） |
| ③-进程内（`dsh-credentials-local`） | `﹣ skipped 1`，理由「未配置 DSH_CHECKOUT」 | 同上；缺检出时**显式跳过**，不伪造通过 |
| 第三：win32 `0600` | `skip: process.platform === 'win32'`（显式、带理由） | 已裁定为**永久平台边界**（§11.3） |

★ 由此得到**一处订正**：「生产默认句柄工厂**只有注入式覆盖**」这个说法**不成立**。
§11.1 那条用例不注入任何东西、在 win32 上真跑并通过（上表第 1 行），
所以缺口 ② 的覆盖面是"默认分支真的被走过"，不是"只有注入式"。

★ 一处口径提醒：上表第 2、3 行要一个**含构建产物的 DSH 检出**才跑得起来。
按本仓的典藏 CI 口径（不设 `DSH_CHECKOUT`）它们**跳过**，
所以"真 DSH 进程读到了"这条证据**不出现在 CI 摘要里**——
...
### 11.7 缺口 ③ 的判据在 CI 里**假红过一次**：把机器负载读成了产品缺陷（2026-09-18）

#### 读数

同一次 `node scripts/ci/run-ci.mjs --only=test`、**同一提交**、连跑两遍：

| 遍次 | 本套件 | 整个 test 阶段 |
| --- | --- | --- |
| 第 1 遍 | ✖ **120s 看门狗杀掉**——而且是那一遍 test 阶段**唯一**的一条红 | FAIL |
| 第 2 遍 | ✔ `exit=0 tests=1 pass=1 skipped=0` | **PASS**（870s） |

单独跑：**3.1s / 6.4s / 6.5s / 5.9s**；7 个真进程套件并发 **5.2s**；
27 个会起真 DSH 的套件并发**全绿**（89s）。

#### 关键的时序事实：红的那条**不是**证据断言

那条红落在**最后一条**断言（`assert.equal(result.code, 0)`）上，
而读数的写入发生在**宿主进程退出之前**（`fixtures/prt509-credentials-probe.mjs:61`
先 `writeFileSync`，再用 `setTimeout(...)` 调 `ctx.appExit(0)`）。
所以那次红的时候：**读数文件在、值读到了、`source === 'file'` 也成立**——
不成立的只是「宿主能在 120s 内干净退出」这条**另一条**主张。

> 一个把两件事（"凭证读到了没有"与"进程能不能干净退出"）说成**同一句**的断言，
> 在超时的时候会把"这台机器当时很忙"报告成"这条凭证线不成立"。

#### 为什么偏偏是它更容易红

它是本仓**唯一**把启动预算卡在 **120s** 的真进程套件。同类要么不设预算（无限等），
要么给 **180s**（`run-floor-dsh-process`、`subagents-surface-real-process`）。
于是只有它会因为机器忙而变红。

#### 真正的负载来源（实测，不是推测）

排查中直接读 `Win32_Process`：**另一个会话正在同一个共享工作树上跑
`scripts/ci/run-ci.mjs`**（其 `isolated-space.test.mjs` 正在起 `product/launcher/cli.mjs`
与两个 server 进程）。本仓 CI 是「近两百个套件 / 8 路并发」，两个 CI 同时跑
就是**双倍**进程负载压在 **8 个核**上——而本工作树是**所有会话共享**的那一份。

> 所以这次红的**不是** PRT-509 的证据线，
> 是「同一台 8 核机器上同时跑了两遍全量 CI」。

#### 处置（两件，都不放松判据的严格程度）

1. `HOST_TIMEOUT_MS` **120s → 240s**。
   ★ 这**不是**"把门槛降到能过"：真挂起仍然要红（改完当天就在一次重负载下真的红过一次）。
   它改的是——预算不再落在"单独跑所需时间"的同一个数量级上（120s 是 3s 的 40 倍，仍然不够）。
2. 超时文案改成**说清是哪一条主张不成立**，并打印"读数文件是否已经写出"，
   于是下一个人拿到这条红时不会再把它读成"凭证没读到"。

#### 诚实边界（三条）

① 我**没有**把这次假红归因到任何**产品**缺陷——我没有找到产品侧挂起的证据。
两遍 CI 同一提交、同一机器、只有一遍红，这个形状支持"负载"，不支持"产品"。
② 但我**也没有**排除"DSH 在重负载下真的偶发挂住"。要分辨它，
需要在**受控**满负载下反复跑很多遍，本轮**没做**。
③ ★ 一条**系统性**观察，超出本套件：这台机器上**会话之间共享一个工作树**，
而全量 CI 是重负载操作。两个会话同时跑全量 CI，会互相把对方的
"以时间为预算的判据"推红——**本仓所有带时间预算的真进程套件都有这个暴露面**，
不只是本套件。本轮只修了最紧的那一个。

### 11.8 ★★★ 订正：11.7 的「机器忙」解释**被算术否掉了**（同日，另一个会话）

我在 11.7 里把那次红归因为"全局负载（两个会话同时跑 CI）"。**这个解释不成立**——
有人把两次 CI 的耗时各**减掉本套件自己那一项**：

| 遍次 | test 阶段 | − 本套件 | = 其余套件 |
| --- | --- | --- | --- |
| 全绿那次 | 892474ms | − 3402ms | **889072ms** |
| FAIL 那次 | 1001899ms | − 240000ms | **761899ms** |

**其余套件快 14.3%（−127173ms）** ⇒ 那一次**全局负载没有升高，反而更低**。

> 我写下的归因用的是"我当时确实看到另一个会话在跑 CI"这个**真实观察**——
> 但一个真实的观察，与一个**能解释这条读数**的原因，不是同一件事。
> 我看到的那件事是真的，只是它**不是**那次超时的原因。

**边界（必须一起记）**：两次是**不同提交**（`32067f3` vs `3d240e2`），且比的是**总量**
⇒ 被否掉的是「**全局**负载说」，**没有**否掉突发性 / 局部争用。

### 11.9 ★★★ 由此得到的结论比"再升一次预算"重要得多

**120s 没过、240s 也没过 ⇒ 升预算这条路已证明不收敛。**（⚠️ **这句推理是错的**——两次失败推不出收敛，订正见本节末 §11.9.1）

而失败签名比"超时"精确：那条断言说读数文件**已经写出**、值**也读到了**，
「不成立的是宿主能干净退出」⇒ 卡的是**关停**，不是凭证那条路。

于是**决定性问题是唯一的一个**：

> **给足时间，那个宿主进程到底会不会退出？**
> 会（比如 260s）⇒ 关停**慢**，修的是关停；
> 永不 ⇒ 关停**挂死**，修的是挂死。
> 两者修法完全不同。

**而这个问题此前没人能问**——因为预算是硬编码的 `const HOST_TIMEOUT_MS = 240000`。
**一个硬编码的预算，与一个"没人能问出口的问题"，是同一个东西。**

本批改成可问（`PRT509_HOST_TIMEOUT_MS`），并且**坏值抛错**：

```
PRT509_HOST_TIMEOUT_MS=900000 node --test product/launcher/run-credential-dsh-process.test.mjs
```

- 退出码 0 且耗时远大于 3s ⇒ 关停**慢**；
- 仍被看门狗杀 ⇒ 关停**挂死**。

★ 坏值**不**静默退回默认：把 `600000ms` 这种笔误悄悄当成 240000，
会让提问的人**以为自己量过 600 秒**——那是一次**假测量**，比没有测量更坏。

★ 本条**不放松判据**：真挂起仍然要红（已用 1s / 3s / 3200ms 三个小预算做正对照，
看门狗照杀、断言照红）。
### 11.9.1 ★★★ 订正：上面那句"已证明不收敛"的**推理**是错的（2026-09-18）

那句话把**两次失败**当成了收敛性的证据。它推不出来：

> 两次"还没到时候"与"永远到不了"，在只有两份失败记录时长得一样。

**结论后来被独立地确立，但靠的是另一条证据**——另一个会话（`e217982`）用与本套件在
CI 里**同一种**起法（`spawn(node, ['--test', file])` + 管道）、预算 20s 连跑 8 轮：

```text
正常 4 轮：宿主 3.1s / 3.2s / 3.2s / 3.5s
挂死 4 轮：宿主 >20s（240s 预算下同样是 >240s）
⇒ ~~偶发率 ≈ 50%~~；**没有"慢"的中间态**   ← ⚠️ **那个百分比已作废，见 §11.9.2**
```

**双峰**（要么 ~3.2s，要么永不到）才是"升预算不收敛"的依据——
不是"两次不够"，而是"分布里根本没有那一段"。

⚠️ **这句的依据后来作废了，见 §11.9.2。**

⇒ 于是上面那个**决定性问题被回答了**：不是关停**慢**，是关停**挂死**。
配套读数：探针读数 **8/8 轮都写出了**（挂的不是"取数"）、挂死那几轮宿主 stdout/stderr
**全空**、挂死**不**留下累积孤儿、**与 `CI=true` 无关**（不设它也会挂，设了反而过一次）；
且**不是那一批引入的**（把改动全还原成提交版再跑 6 轮，仍是 3 轮超时 / 1 轮 3.5s）。

★ 这一节留着的理由**不是**"结论错了"（结论碰巧是对的），而是**理由错了**：

> 一个真实的观察，与一个能解释这条读数的原因，不是同一件事。

同一个形状在本条里已经出现**三次**（§11.8 那条被否掉的"全局负载说"、上面这句、
以及把双峰读成"偶发"的那次）。它是本条最贵的一课：**结论对，而支撑它的推理不成立**
——这种错最难被发现，因为反驳它要先承认"结论看起来是对的"。

### 11.9.2 ⚠️⚠️ 订正 §11.9.1 的**依据**：那个"≈50%"是**另一个会话**给的，而它**没有条件**

§11.9.1 用 `e217982` 那 8 轮的 `≈ 50%` 当作"升预算不收敛"的依据。
**那个数字已经作废**（跑那 8 轮的那个会话在 `PRT-PROGRESS.md`
「具名未修缺陷」一节做了**第二次订正**）：
...
| 串行空闲 | **0/25** |
| 串行空闲（提交版原始文件） | **0/6** |
| 并发 4 份 × 4 批（原以为负载是触发条件） | **0/16** |
| **整条 CI 正在跑**时（= 当初出红的条件） | **0/10** |

⇒ 合计 **0/57**；**触发条件至今未知**。那 8 个样本取自"另一个会话同时在跑"的窗口，
而**当时的负载没有记录** ⇒ 样本连同百分比一起废掉。

> **率是"条件 + 数字"的合取，而只抄了数字。**
> 一个不写条件的率，比不写率更糟——**它看起来是量出来的。**

⚠️ 因此 §11.9.1 那句"**双峰**才是'升预算不收敛'的依据"**也不能再用了**：
"分布里没有那一段"这个前提，正是靠那个无条件样本立起来的。
它退回到与 §11.9.1 想否掉的那句话**同级**的处境——**证据不足**。

★ 这一节本身**仍然成立**（"理由错了"这个批评是对的），只是批评的对象要再挪一格：
§11.9.1 是在一份"没有条件的样本"之上建的**另一个**同类错。
同一个形状在本条里因此是**四次**，不是三次。
...
5 个全部来自 **chokidar**，监视宿主自己的 profile 目录；创建点/关闭点在 **DSH 侧**的
`packages/credentials/credentials-local/src/index.ts:585` / `:611`（fiber disposer）。

> ★ **坐标要连着 DSH 修订读**（第 118 轮第三十八轮补记）：上面这两个数是
> **本批当时那个 DSH 检出**上的读数。2026-09-23 检出更新到
> `rel/dsh-0.1.7-rc.1`（`46a7f68`）之后，同一个文件短了 7 行 ——
> 这两处现在是 **`:578` / `:604`**（`pinned-citations-verbatim` 因此红过一次，
> 已重钉）。机制结论一个字没变，变的是坐标。

> 这解释了"双峰为什么没有中间态"：不是**慢**，而是**清理这一步成功或失败**。

⇒ **"关停挂死"这条机制结论可以留**（它是在**确实挂着的时刻**量的，不因复现不了而失效）；
**"发生得多频繁"这个数不能留**。这两件事此前被同一句话捆在一起，现在拆开。
