...
1. **引用名的写法是相容的——但要分清是哪一种相容。**（本节原先写的是
   「Legion 的 `SECRET_REF_RE` 强制的正是"恰好两段"」，**这句话是错的**，
   已由 PRT-509 路线 A′ 更正。）
   - `security/secrets/ref.mjs` 的 `SECRET_REF_RE` 是
     `/^[a-zA-Z0-9][a-zA-Z0-9._:/-]{0,127}$/`：它只要求首字符是字母数字、
     总长 ≤ 128、**不出现空段与 `.`/`..` 段**。它**从来没有**要求两段——
     `a`（一段）与 `a/b/c`（三段）都是合法的 Legion 引用名。
   - DSH 那边是**两个语法不同的键空间**：`refs` 的键是 POSIX 标识符
     （`/^[A-Za-z_][A-Za-z0-9_]*$/`，**不含 `/`**），`records` 的键是
     `<scope>/<id>` 且两段都必须是 `/^[a-z][a-z0-9-]*$/`（**恰好两段**）。
   - 于是真实关系是**单向包含**，不是"两种语法相容"：
     - 一个不含 `/` 的 Legion 引用名，若恰好是 POSIX 标识符 → 可以寻址 `refs`；
     - 一个恰好两段、且两段都是小写连字符标识符的 Legion 引用名 →
       可以寻址 `records`；
     - **其余 Legion 合法的引用名（`a/b/c`、`sk-no`、`a/B`、`a/b_c`…）
       在 DSH 里根本不可能存在**。
   - 这条差别不是学术性的：把"不可寻址"报成"文件里没有这一条"，
     会让用户去 DSH 里找一个不可能存在的记录。所以读取器给这两件事
     **两个不同的读数**（`not-addressable` vs `absent`），
     细节见 `PRT-509-dsh-credentials-read-bridge.md`。
   - 至于原句想说的"两个键空间不碰撞"：那是 DSH 自己的注释讲的设计意图
     （`credentials/src/index.ts` 里 `KEY_SEGMENT_PATTERN` 的注释：
     "the `/` between them is what keeps it out of `REF_PATTERN`"），
     与 Legion 的引用名语法**无关**。
...
### 9.1 后续：路线 A′ 已交付（本节的"最大一处未交付"已关闭）

本节记的"两个密钥库并存、没有与 DSH 对账"这一项，已由 **PRT-509 路线 A′**
补上，实现与边界见 `PRT-509-dsh-credentials-read-bridge.md`。要点：

- **权威在 Legion。** 写路径仍然唯一：只写 Legion 自己的 DPAPI 库
  （`security/secrets/store.mjs`）。DSH 的 `.credentials.yaml` **只读**。
- **没有做迁移**（路线 B 没有实施）：`.credentials.yaml` 里的值不会
  被搬进 Legion 的库，也不会被改写。
- **只认 DSH 自己写出的那个子集**，其余整份拒绝（fail closed）；
  比 DSH 自己的解析器**更严**，从严清单是实测出来的、有限的、被用例钉住的。
- **上面"反对复用"的第 2 条（要自己写 YAML 解析器）成立且没有被绕过**：
  路由 A′ 确实自己写了子集读取器（零第三方依赖）。它之所以可行，
  是因为**范围被压到"DSH 自己写出来的形状"**，而不是"YAML"——
  后者才是那条反对意见真正指向的危险。

因此本节"支持复用/反对复用"的张力**没有消失，而是被限定成了一个明确的形状**：
路线 A′ 没有把保护等级降到明文（Legion 的库仍然是第一顺位，且回退只在
"库里没有这条"时发生），代价是本读取器需要跟着 DSH 的子集一起维护。

---

## 10. 事故记录：这份文档自己泄漏了一次真实密钥

**这一节必须写下来，因为犯错的位置正是这份文档的主题。**

### 发生了什么

§9 为了论证"DSH 的 `.credentials.yaml` 是明文的"，我把本机那个文件的
`refs:` 段**抄了进来**——包括一个**真实的** DeepSeek API Key。

提交 `c30ac27` 推送时被 GitHub Push Protection 拒绝：
...
**① 具体的诱惑。** 论点完全可以用一个假值表达
（"形如 `{ DEEPSEEK_API_KEY: sk-... }`"），而我为了让它"更实"，
把一个安全反例变成了一次**真实泄漏**。
本文件通篇在讲"密钥不得进入文档、日志、诊断包"——**而它就发生在这份文件里**。

**② 更严重的是：我的推送验证给了假阳性，而我据此相信了两次失败的推送。**
...
这是本项目反复记过的那个家族里最刺眼的一个实例：

> **一个测不到东西的用例，和一个正确的实现，在输出上完全一样。**

这里的具体形态是：

> **一个永远为真的验证，和一个真的验证，在输出上完全一样。**

### 修法（两件都做了）

**① 把真实值换成假值，并重写历史。**
`c30ac27` 与紧随其后的 `1f15871` 都**尚未推送**，所以可以安全重写：
在 `c30ac27` 上就地修订、再重放 `1f15871`，得到 `60dde2f` 与 `6460552`。
验证方式是 `git rev-list --all` + `git grep` 逐提交扫描——**真实密钥已不可达**，
承载它的临时分支也已删除（**一个含密钥的本地分支是一颗地雷**：
一次 `git push --all` 就会把它带出去）。

**② 把判据换成"只有一条"的形式**：`scripts/prt/push-verify.mjs`。

```
正确的判据：远端 tip == 本地 HEAD
```

用**远端的事实**说话，不用推送命令自己的输出说话——
命令的输出是"我尝试了什么"，远端的状态才是"发生了什么"。
这两者在"命令成功但被服务端规则拒绝"时是**分离**的，而那正是这条判据存在的唯一理由。

该模块的用例里有**两条负样本**，用的是**真实的**成功输出与**真实的**拒绝输出：
两个场景必须被判成不同的结果。少了这两条，这个模块可以退回成一条永远为真的正则，
**而用例全绿**。

### 还有一件必须说的事

那个 DeepSeek API Key **没有被推送到远端**（推送被拒绝了），
但它在本机 `.git` 里存在过，并且**进入了这次对话的上下文**。
按"密钥一旦离开唯一可信位置就视为已泄漏"的通行口径，
**这个 key 应当被轮换**。我在此明确指出，而不是把它当作一件已经过去的小事。
...
node --test scripts/prt/push-verify.test.mjs       # 9 例（推送判据 + 两条真实负样本）
