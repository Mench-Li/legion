...
本轮**第一次**全量 CI（`.ci/r19b`）在跑满 **803s** 之后红了一条 ——
`p13-host-injection` ④「SSE 宿主形态：v2 `/team-hub/api/events` 信封回放+增量」
以 `SyntaxError: Unexpected end of JSON input` 失败，耗时 **260ms**；
而它通过时是 **4097ms**。同一个提交上**第二次**全量 CI（`.ci/r19c`）**九个阶段全 PASS**，
该套件 **39/39**；把它单独跑 **6 次**（1 + 5）也全绿。

判据为什么认定它是偶发：

1. **耗时形状**：260ms vs 4097ms ⇒ SSE 流几乎立刻结束 / 为空 ——
   那是**传输层**的读数（`JSON.parse` 一个空体），不是一次判定。
2. **本轮改动是环境门控的**：`LEGION_EXECUTION_SCOPE` 缺席 ⇒
   `executionScopePortFromEnv` 返回 `{state: 'absent', port: null}` ⇒
   `executionGuard` 第一句 `if (executionScope === null) return undefined`。
   同一份 env 下，这一步与改动前**逐字等价**。
3. **套件是串行的**（`run-ci.mjs:3980` 的 `for (const s of suites) { await … }`）⇒
   不是同阶段并发争 CPU；更像是 803s 长跑之后机器状态（残留进程 / 端口 / 句柄）的读数。

> 一个"偶发红、重跑就绿"的套件，与一个"一直绿"的套件，
> 在摘要里长得一样——只不过前者的**下一次**红会被当成"又是那个偶发"。

★ 而那条测试我**没有**改：**我没有观察到它的失败机理**。
把 260ms 那一次改成"失败就重试一次"或"多等一会儿"，
是把一个还没读懂的读数**掩盖**掉——*一条被重试盖住的偶发，与一条被修好的偶发，
在摘要上是同一个绿；差别在于前者会回来，而且回来时没人再看得见它。*
⇒ 正确处置是：把这两次 CI 的读数**都**留在账上（本节），并把失败形状记下来；
等第**三**次看见它（同样形状、同样套件）时，那才是有机理可查的样本。
...
### 10.50 ★★★ 第 20 轮：**判定器齐了、端口也齐了——而"怎么把 URL 拆开"决定了三道检查从不触发**

第 19 轮把 PRT-605 接上了，留下 ② 那条判据守着 PRT-606。本轮是它指的"下一件"。

#### ① 一句话结论

`external-api-scope.mjs` 的 24 例**一直**是绿的，而 `checkExternalApi` **连端口都没有**。
本轮给它建了 `external-api-scope-port.mjs`（`LEGION_EXTERNAL_API_SCOPE`），
`enforcementSurfaces()` **8 → 9**。**三道范围检查至此全部有生产位置。**

#### ② ★★★ 而本轮最值钱的读数不在"接上了"，在"怎么拆 URL"

`checkExternalApi` 要的是 `{host, method, path, headers, query, body}` 六项**分开**的
请求形状，而事实表里只有一个 `url` 字符串。所以适配器要把 URL 拆开——
**而怎么拆，决定那 24 例里的三条检查是"会触发"还是"从不触发"。**
...
query 上还有第三个同族形态：按 `&`/`=` 手切时，`?%6dethod=DELETE` 的键是 `%6dethod`，
...
（那是 PRT-605 那一份解析器，`external-api-scope.mjs` 文件头自己写着
"host 的比较规则来自 PRT-605"），`path`/`query` 用一条**只切不改**的正则。

#### ③ ★★★ 而这三个洞**红的方式很特殊**——这是本轮的判据设计要点

退回 `new URL()` 时，那三条**不会**变成放行，它们会变成
...
这一条是本轮**最该被下一个改这里的人读到**的一句：这个适配器的回归不会以
"放行了一个越权调用"的形式出现，它会以"拒绝理由换了一句话"的形式出现。
...
```text
api-scope-malformed: endpoints[0] 出现了不认识的字段 ["parsed"]
```

根因：`normalizeApiGrant` 会在每个端点上挂一个**派生字段** `parsed`
（让 `checkExternalApi` 不必每次重新解析模式），而 `parsed` **不在** `ENDPOINT_FIELDS` 里。
...
> 三个端口**长着同一副样子**——`fromEnv` 归一化一次、`create*Port` 再归一化一次——
> 而其中两个的归一化器是**幂等**的、第三个不是。
> ⇒ 同一段接线在两处是绿的、在第三处是装配期就抛，
> 而"这个归一化器幂等吗"这件事，在三个端口的代码里**一个字都没写**。

修法是对**原始**声明归一化两次（保留"不认识字段就拒"那道防打错字的检查），
并把这件事钉成套件 ⑩——它同时断言"`normalizeApiGrant` 不幂等"与
"`normalizeGrant` 幂等"，所以下一个照抄这段的人**会看到区别**。

#### ⑤ ★★★ 判据顶出来的第二个：可达性基线**只被单边守**

`reachability.test.mjs` 的 ② 只管"当前不可达的必须有分类"、
③ 只管"基线里的文件必须还在"——**没有一条判据管"基线里判成 `gap` 的模块后来被接上了"**。

实测有**两条**这样的过期条目：

| 模块 | 什么时候接上的 | 基线条目留到 |
| --- | --- | --- |
| `external-api-scope.mjs` | 第 20 轮（本轮） | —— |
| `execution-scope.mjs` | **第 19 轮** | **今天** |

第 19 轮那条更值得记：那一轮**做了所有正确的事**（从 READINGS 删条目、
按规矩补替身、更新 §5.2 的读数），**却没有任何判据要求它动基线**。

> 一个「只检查'有没有漏掉'的基线闸」，
> 与一个「可以无限积累过期条目、而每一条都长得像一条读数」的基线闸，
> 是同一个东西——只不过后者会让"死代码还有多少"这个数**只会涨不会跌**。

⇒ 新增用例 **③b**（判成 `gap` 而不许已可达），并 `--record` 清掉那两条
（基线 46 → 44 条，`git diff` 恰好是 10 行删除，一条不多）。

#### ⑥ 判据第三次"按自己的指示"换对象——而这一次它换到了一个**不同形状**的缺口

`production-scope-wiring.test.mjs` ② 在失败消息里逐字要求接线的人
"把台账、docs 与本套件一起更新"。第 19 轮它指向 `externalApiScope`，本轮照做。
而它**第三次换对象**，现在守 `whitelist`：

> 三道范围检查是"**桥里没有位置**"（连一格 `false` 都读不出来）；
> `whitelist` 是"**位置在、而生产装配从不给它值**"。
> 前者连"我该配什么"都问不出来，后者问得出来却**没有任何人**在问。

★ 三道**全部**接上之后，`whitelist`（PRT-603）是这一族里**唯一**剩下的成员：
它有一格 `false`、有一份套件、有一份台账 ✅，而生产装配里那个键从来不存在。
**没有任何一条判据会去问"谁该给它一个值"。**
...
| 闸门 | 红在哪 | 为什么红得对 |
| --- | --- | --- |
| `production-scope-wiring` ①/② | 键集 8 → 9 | 键集是**契约**，变一格必须显式改 |
| `path-scope` ⑥ | 同上 | 两处独立钉同一份契约 |
| `config.test.mjs` × 4 组基线 | 14 键 / 17 动态 / 命中 10 处 | 每接一道范围检查，这四个数都要重数一遍 |
| `boundary-facts` ①/⑫a/⑫b/⑫c | 手钉坐标 763 → 780 | **第四次**位移，四次来自四个不同功能 |
| `reachability` ④ → ③b 那条断言 | 见 ⑤ | 前一个红是**好消息**，后一个红是**新发现的洞** |

★ 其中 `boundary-facts` 的**四次位移、四个功能、同一个文件**，
把"手钉坐标的成本随这个文件被改过几次增长"从一句推测变成了**被验证四遍的读数**。

#### ⑧ 本轮踩到的第四个 CRLF/表格坑（前三轮各踩过一个）

第一版我把台账片段**直接拼在行尾** ⇒ 这一行变成
`| ...正文... 续写的一段话`，而**末尾那个 `|` 被挤到了中间**。

`check-docs.mjs` 当场红：*表格行未以 `|` 收尾（表格行被换行切断？）*——它红得对，
而且这条错的**危险之处**在于 Markdown 对此**不报错**：渲染器只是把后面几行
当成同一格的内容。⇒ 正确的写法是插在**最后一个 `|` 之前**，
让那个 `|` 继续收尾；脚本里现在有一条 `assert` 专门守这件事。

#### ⑨ 本轮的诚实边界

1. ⚠️ ★★ **本端口不看 scheme**（`checkExternalApi` 也不看）：`SCHEME_DENIED`
   归 `checkNetwork`。⇒ `ftp://api.example.com/api/items/1` 只要 host 与模式
   对得上，就会被 `externalApiScope` **放行**。本批**没有**顺手加"必须 http(s)"——
   那是**发明策略**（PRT-253 §3 明令禁止发明默认值），已列为 §5 第 26 条。
   ★ 而它今天不致命的原因**不是**"已经拦住了"，是"这一格在真实部署里仍是 `false`"
   （见下一条）——两件事不能混。
2. ⚠️ `idempotencyKey` 恒为 `null`（事实表里没有它）⇒ `IDEMPOTENCY_NOT_HONORED`
   在当前接线里**不会触发**——重试语义还没有数据来源。
3. ⚠️ `LEGION_EXTERNAL_API_SCOPE` 同样**不在** `product/process-manifest.mjs` 的
...
4. ⚠️ **MCP 那一条仍然没接**（第 19 轮的裁决未做）——§5 第 25 条。
5. ⚠️ 三道范围检查现在是"**配了才跑**"，不是"默认就拦"：三次接线同一个形状，
   没配 ⇒ `port` 是 `null` ⇒ 那一行是放行。
...
### 10.52 ★★★ 第 22 轮：给 PRT-610 的"写入方"造一条**出站车道**，并在真库上把环走通

> 决策表第 19 条已裁决（2026-09-18，**选前者**）：执行面需要的数据走**逐 Run 的载荷**，
> 不给执行面开控制面入口。§4 那张表里**第 15 项**写的是「PRT-610 `recordToolCall` 的写入方：
> 表、读面、就绪证据的产出点已接上，**写入方仍是 0**」。
> 本轮做的是那一项的**施工**。

#### 一、缝在哪：一句写在库里的需求，与一个刻意的缺席
...
★ 它**看起来不像坏了**。`toolCallLogEvidence` 返回的是一句很谨慎、很正确的话：

```text
{state:'readable', table:'tool_calls', recorded:false, total:0,
 reason:'表在，但 0 条记录里没有一条带着决定来源'}
```
...
★ `toolCallRowOf()` **本来就存在**（`tool-request.mjs:439`，PRT-610 的行形状），
此前**零生产调用方**；写入侧直接引用它，不重算、不改名。
★ 两半**分开登记为两个 CI 套件**，因为它们各自能被单独跑出一个"绿"而合起来仍然不通
...
1. 收账**之前**：`toolCallLogEvidence({db})` = `{state:'readable', total:0, recorded:false}`；
...
4. 真 SQLite 里那一行：`decisionSource='pre-execute'`、`resultStatus='ok'`、
   `dispatchedAt` 落上、`rawInput`/`canonicalInput` 原样；
5. **`toolCallLogEvidence().recorded` 由 `false` 翻成 `true`**。

并有一条**否定对照** ①b：同样的 spool、**不跑收账**，`recorded` 仍然是 `false`。
没有这条对照，"翻成 true"无法与"表自己变成那样"区分开——这正是过去若干轮反复记的那条。

#### 四、为什么这**不**等于第 15 条已关

⚠️ **生产里还没有人调这两半。** 缺的**不是代码**，是一条具名读数：
...
而"猜一个两边都同意的路径"正是 PRT-253 §3 禁止的**发明默认值**。
⇒ 立为决策表**第 28 条**，详见 `DECISION-RUNREQUEST-EXECUTION-PLANE.md` §14。

**本轮不把它记成又一次"模块全绿而零生产入口"**，理由是这一次缺的是**一个决定**，
而且那个决定有名字、有编号、有两个候选接缝（hub 进程 / `root-row.mjs:591` 的 `onDecision`）。
★ 第二个候选接缝有一处**具体的坑**已读出：`root-row.mjs` 是**进程级单例**，
在那儿绑死 runId 会让整个进程只往**第一个** Run 的账本里写
——按 Run 安装的缝在 `runtime-host-registrar-row.mjs`（PRT-214 两遍先例的形状）。

#### 五、★★★ 本轮**我自己造出来**且被用例当场抓住的三处

**① win32 会静默吃掉尾部的点/空格，而它的后果与"消毒"一模一样。**
第一版 `safeRunId` 只挡了路径分隔符。用例 ① 喂 `C:evil` 时**没红**——查下去发现
我漏了冒号（win32 的盘符/ADS 分隔符），再查又发现一整类：

```text
run-1.  与  run-1    在 win32 上是同一个目录名（尾部点被静默去掉）
run-1   与  run-1    同上（尾部空格）
```

> 一个「只挡路径分隔符」的落盘，
> 与一个「`run-1.` 与 `run-1` 仍然合流」的落盘，是同一个东西——
> 只不过后者的触发条件是"某个 Run id 末尾带一个点"，
> 而它**一个字符都没被替换**，所以连"消毒"这个能引起警觉的动作都没有。

**② 三个桶，而不是两个。** 第一版 `toolCallDrainStatus` 用 `drained` 条数分桶，
用例 ⑤ 打出 `'unwired' !== 'idle'`。查下去：**"根本没跑过"与"跑了但执行面从没写过"
是同一个处置（去查接线），而"执行面写了、一条都没落"是另一个处置（去查数据）**。
⇒ 改成用 `drain.present` 分，并在注释里写下那张三行表：
`unwired`（查接线）/ `idle`（查 `refusals` 的行号）/ `wired`。

★ 另有两处是**接口摩擦**而不是缺陷，但都补了断言把它变成可读的：
`spoolFileFor()` **只算路径不建目录**（建目录只发生在 `appendSpoolRecord` 里），
以及收账侧起初把"落账失败"报成 `line: null`——**知道有一条坏了、却找不到它在哪一行**，
于是 `readSpoolRecords` 改成同时返回带行号的 `entries`。

**③ 把指针加粗，等于把机器读指针弄瞎。** 全量 CI 在 `test` 阶段红在**可达性探针** ② 与 ⑤：

```text
② 每个不可达模块都必须在基线里被分类（新出现的必须有人定性）
actual: [ 'orchestrator/worker/toolcall-drain.mjs', 'runtime/toolcall/spool.mjs' ]
```

★ 这一条**不是缺陷、是门禁按设计工作**——本轮新建的两个模块"已交付但生产里到不了"，
门禁的作用正是逼着人说出**属于哪一类**。两条都判为 `gap`，指针指向 **§5 第 28 条**：
它们**是同一条缝的两半**，阻塞是**同一个位置决定**，不是两处要修的地方。

★★ 但**紧接着**来的这一条是缺陷，而且又是我自己造的：第 ⑧ 号用例要求每条 `gap` 的
reason 里有一个**能解析**的裁决处指针，解析器是 `/§5\s*第\s*(\d+)\s*条/`。
而我写的是：

```text
⇒ 裁决处：§5 第 **28** 条        ← 数字加了粗体，为了人读起来醒目
```

`**` 让那个正则**匹配不上**，于是它被报成"没写出裁决处指针"。

> 一个「为了给人看而加粗」的指针，
> 与一个「机器再也读不出来」的指针，是同一个字符串——
> 只不过前者的好处在**第一眼**，后者的代价在**门禁上**。

★ 这条与 ① 是同一族：都是"我为了让某件事**更好读/更稳**，而把**另一个**读者弄瞎了"
（① 里那个被弄瞎的是"路径就是 Run 的身份"，这里是"门禁读得到指针"）。
两处都是**同轮**被自己的用例抓住的。

#### 六、★ 顺带修掉一处**第 17 轮之前就在**的表格缺陷

`docs/MULTI-AGENT-FEATURE-STATUS.md` 决策表**第 23 条**的"具体决定"格里
写着一个字面竖线（`` `string|null` ``），把那一行切成 **6 列**，
于是"具体决定"与"不决定的后果"**各错位一格**。

> 一个"单元格里有一个未转义竖线"的表格行，
> 与一个"整行内容都对、只是列标题对不上"的表格行，是同一个东西——
> 只不过读它的人会把**后果**当成**决定**来读。

已改成 `` `string` 或 `null` ``（CommonMark 的代码跨度里反斜杠**不**转义，
所以 `\|` 不是选项）。修完复核：决策表 **725–752 行全部 5 列，编号连续 `1..28`**。

#### 七、诚实边界（如实）

1. ⚠️ **生产里还没有人调这两半**——所以第 15 条**没关**，本轮的口径是
   "**车道已建、环已证、位置未决**"；
2. ⚠️ **没有**给 spool 定目录（第 28 条）——不发明默认值；
3. ⚠️ **没有**动 `team-hub/server.mjs`（9097 行，且是别的会话的在制品风险区）；
4. ⚠️ **没有**做车道文件的裁剪（旋转/归档）。车道是**只追加**的，所以它会一直长——
   这是刻意换来的代价：收账必须能安全重跑，而"收完就删 + 记游标"会在崩溃后
   把没落账的几条当成"已经收过了"（套件 ③ 有一条断言确认收账侧**不出现**
   `unlinkSync`/`writeFileSync`）；
5. ⚠️ 收账侧**不校验** `decisionSource` 的语义（那是设计：词表只有
   `tool-call-log.mjs` 那一份，套件 ⑤b 有一条断言**读源码**确认这里没抄第二份），
   所以一次**语义**不合法表现为一次**落账期**具名拒绝——带行号，但不是"格式坏了"；
6. ⚠️ 环的证明是在**单个进程内**、用**注入的临时目录**走通的
   （真实部署里那两个进程是分开的，而"分开之后它们怎么找到同一个目录"正是第 28 条）；
7. ⚠️ `toolCallDrainStatus` 的三个桶**只在用例里被喂过**——生产里还没有任何一个部署自检调它。

8. ✅ **全量 CI（冻结树上）**：HEAD `466239d`、`.ci/r22a`，**9/9 PASS，exit 0**，`test` 811934ms、`skipped=1`（secret-store，已知）、**357 个 `*.test.mjs` 全部有归属**；本轮新套件 `toolcall-spool` **14/14**、`toolcall-drain` **13/13**、`doc-table` **10/10**；`doc` 读到 **140/145，未完成 5**。★ 这一次跑在一棵**冻住**的树上——`test` 阶段那 812 秒里工作树的代码与文档**一个字节都没动过**。

### 10.53 ★★ 顺带建立的一道门禁：**文档表格的列数一致性**

> 起因很小：修决策表第 23 条时发现那一行是 **6 列**（表头 5 列）。
> 往下一查，这不是一处笔误，是**一类**——于是本轮把它变成可复跑的读数。

#### 一、那一类缺陷长什么样，以及它为什么不是"渲染难看"

一行 markdown 表格里只要有一个**未转义的竖线**，那一行就会被切成多一格。
本轮在 `docs/MULTI-AGENT-FEATURE-STATUS.md` 里抓到的三处，都是**代码跨度里的竖线**：

```text
| 23 | … | `resolveConnectorId` 的值域是 `string|null`，装不下"拒" | … |   ← 6 格
| F-18 | … | … `draft → promoted | discarded`，两个终点都要人 … |          ← 6 格
```
...
第 23 条那一格尤其要紧：那张表**就是承载裁决的表**，而错位的两格正是
"**具体决定**"与"**不决定的后果**"。

★ 还有一个更隐蔽的方向：那一行**看起来完全正常**——每一格都有内容、
标点也对、读起来通顺。它只是在**列标题**上对不上。

#### 二、★ 修法不能是"把竖线转义"

第一反应是写 `\|`。**不行**，而且理由是可验证的：

> CommonMark 的代码跨度（`` ` ``）里**反斜杠不转义**——
> `` `a\|b` `` 会**原样**渲染成 `a\|b`。

所以本轮的修法全都是**换记号**，用的都是这个仓库里**已有的**写法：

★ 所以本轮的修法全都是**换记号**，用的都是这个仓库里**已有的**写法。
⚠️ 下面这份对照**刻意不写成表格**——因为"原写法"那一列里就是半角竖线，
写进表格会**当场制造同一个缺陷**（本轮的检查器确实当场抓住了我第一次的写法，
见第七节第 ② 条）：

```text
`string|null`                       →  `string` 或 `null`              （同文档的中文行文习惯）
`draft → promoted | discarded`       →  `draft → promoted` 或 `discarded`
POST /api/create|claim|transition    →  POST /api/{create,claim,transition}
F-xx | 功能名 | 章节锚点             →  F-xx ｜ 功能名 ｜ 章节锚点   （全角；这是"索引段的形状"）
`|| === null`                        →  那条判空短路分支              （行文引用，不是代码）
```

第三处用的是 `runtime/packs` 与 `LEGION_TOOL_ROUTING` 的**花括号**约定；
前两处依据同一份文档里的中文行文习惯。
...
| 文件 | 处数 | 形状 |
| --- | --- | --- |
| `docs/MULTI-AGENT-FEATURE-STATUS.md` | **8** | 第 23 条 + F-18 行的竖线；F-18 缺口①②③ 三行**只有 3 格却坐在 5 格的表里**；§4 P2 那张表**表头只有 4 格**而 F-21 行有 5 格 |
| `README.md` | **3** | 三条接口域行里的竖线（就是本轮换成 `/api/{create,claim,…}` 那三行） |
| `docs/superpowers/prt/PRT-603-…md` | **1** | 那处判空短路写法的代码跨度 |
| `docs/FEATURES.md` | **1** | 功能索引段的形状描述 |
| **合计** | **13** | 5 个文件 |

★★ 其中 **F-18 缺口①②③** 与 **P2 那张表**是**另一种形状**，值得单独记：
它们不是"多一格"，而是**行数/列数不配套**——
三行 3 格的缺口行坐在 5 格的表里（它们照抄的是 §2 那张 3 列表的形状），
而 §4 的 P2 表**表头 4 格**、里面却有一行是 5 格（照抄的是 §3 那张 5 列表的形状）。
**补法用的是本文档自己已有的约定**（表格统一 5 列；"还差什么"为空写 `—`，
第 64 行 `| F-12 | … | PRT-701～713 | — |` 就是这么写的），没有发明新记号。

修完复核：那 5 个文件的表格**逐表列数一致**（共 33 + 6 + 4 + 3 张表全绿）。

#### 四、新门禁：`scripts/prt/doc-table-integrity.mjs`（套件 `doc-table`，10 例）

两半，判据方向**相反**，这是刻意的：

| | 对象 | 判据 | 为什么 |
| --- | --- | --- | --- |
| ① | **八份权威文档** | **必须 0 处**（硬断言） | 它们承载裁决/台账/报告；错一格 = 一个读数被当成另一个读数 |
| ② | 全仓**已跟踪** md | **棘轮**（基线 87，只许降） | 剩余 87 处全在 `docs/review/*` 与 `scratch/*evidence-backup/*` 这类**时点记录**里，它们记录的是当时发生的事、不再改动 |

★★ ② 的"减少只报警、不判红"是**刻意**的，理由与 §5.4 那条**逐字相同**：

> 一个「把别人修好的好消息判成回归」的闸门，
> 与一个「逼着人把好消息重新基线一遍」的闸门，是同一个东西——
> 只不过前者会在**共享工作树上天天红**。

★ ③b 还有一条**反空洞**断言：八份文件必须**真的被读到内容**
（`missing` 计数必须为 0）——一个"所有文件都打不开、于是 0 处"的检查，
与一个"全都干净"的检查，在读数上是同一个东西，只不过前者永远绿。

#### 五、★ 假阳性护栏：`\|` 不算分隔符（这条是本轮**必需**的）

第一版检查器用朴素的 `split('|')`。它在全仓报出 **934 处**——而其中 **847 处是假红**，
全部来自 `'allow' \| 'deny'` 这种**已经正确转义**的写法。

> 一个「到处都是假红」的检查，
> 与一个「找不出真的那一处」的检查，在"它能不能挡住回归"上是同一个东西。

⇒ `splitRow()` 用 `/(?<!\\)\|/`（只有**未转义**的竖线才分格），
并有一条**双向**断言：带 `\|` 的行必须算 2 格；**拿掉那个反斜杠就必须红**
（两者只差一个字符，必须被分开）。

#### 六、⚠️ 本轮**自己做出来的**、被自己的用例抓住的第二个缺陷

第一条全仓读数是 **95**，而基线脚本读的是 **87**。差了 **8**。查下去：

```text
git ls-files '*.md'  默认按 core.quotePath 转义，非 ASCII 路径会**加上双引号**：
  "whiteboard/docs/adr/ADR-0001-\345\220\214\346\255\245\345\274\225\346\223\216.md"
```

本仓有 **8** 个 `whiteboard/docs/adr/ADR-*.md` 是中文名。那种路径**打不开**，
于是它们在 `auditFiles` 里各变成一条 `kind:'missing'`。

★ 它的症状是**总数偏高**（像个坏消息），而真正危险的方向藏在里面：
**那 8 个文件自己的真缺陷一处都数不到**。

> 一个「文件清单里带着带引号的名字」的检查，
> 与一个「对某些文件永远看不见」的检查，是同一个东西——
> 只不过前者的症状看起来像**坏消息**，而漏报的方向**没有症状**。

⇒ 改用 `git ls-files -z`（NUL 分隔、不转义），并加了一条 ④b 断言：
清单里不许出现带引号的路径、非 ASCII 路径必须真的打得开、且不许被记成 `missing`。

★★ 这一条**只有在我把"文件不存在"也算成一类缺陷时才会浮出来**。
如果 `auditFiles` 对打不开的文件**静默跳过**，那么本轮会以为基线是 87、
以为那 8 个文件是干净的——**而它们是"从没被读过"**。

#### 七、诚实边界（如实）

1. ⚠️ 剩余 **87 处 / 16 个文件**的缺陷**没有修**——它们是时点记录，做的是棘轮；
2. ⚠️ 基线**只覆盖已跟踪的 md**：新增的未跟踪文件要 `git add` 之后才进这个读数
   （与套件完备性门禁同一条纪律）；
3. ⚠️ 检查器只判**列数**，不判内容：一个 5 格全错位但恰好是 5 格的行，它**看不出来**；
4. ⚠️ `README.md` / `docs/FEATURES.md` 的修法把 `|` 换成了 `{a,b}` 与全角 `｜`，
   这**改变了渲染出来的字面文本**——功能上等价，但不再是"逐字原样"的接口列表；
5. ★★ **本节第一稿自己就踩了这个坑**：第二节那份"原写法 → 改成"的对照表里，
   "原写法"那一列写的就是**半角竖线**，于是它**当场**成了 3 处新缺陷
   ——而检查器**在它诞生的同一轮就抓住了它**（第一次跑：本文件表 88 张、问题 3 处）。
   改成把对照放进 `text` 代码块之后归零。

   > 一个"记录某一类缺陷"的文档，
   > 与一个"自己就在犯那一类缺陷"的文档，在**写的时候**是同一个东西——
   > 只不过前者的证据是那 13 处修改，后者的证据是**它自己**。
...
198. §10.49 ⑦ ★★ **一次偶发，而我没有去"修"它**：本轮第一次全量 CI（`.ci/r19b`）
199. §10.50 ★★★ **"怎么把 URL 拆开"决定了三道检查从不触发**：`checkExternalApi` 要的是
     `{host, method, path, …}` 六项分开的请求形状，而事实表里只有一个 `url` 字符串。
     本批**实测**：`u.hostname` 把 `https://api.example.com:8443/v1` 读成
     `api.example.com`（端口被丢掉）⇒ `HOST_HAS_PORT` **从不触发**；
     `u.pathname` 把 `/api/items/../admin` 读成 `/api/admin`（`..` 被折叠）
     ⇒ `PATH_ESCAPE` **从不触发**，而折叠后的路径还可能命中一条它本来不匹配的授权；
     `u.hostname` 把 `https://api.example.com@evil.com/x` 读成 `evil.com`
     ⇒ 具名的 `HOST_HAS_USERINFO` 退化成"这个 host 没被授权"。
     ⇒ 端口**从原始串上取**（`parseEgressUrl` 的 `rawAuthority` + 一条只切不改的正则）。
200. §10.50 ★★★ **而那三个洞红的方式很特殊**：退回 `new URL()` 时它们**不会**变成放行，
     而是变成 `ENDPOINT_NOT_GRANTED`（"不匹配任何端点"）——**方向仍然是拒**。
     ⇒ 一个只看 `allowed === false` 的用例**抓不到它**；套件断言的是**码**。
     *一个「只看 allowed 是不是 false」的用例，与一个「分不清"端口与 userinfo 没被检查"
     与"这个端点没被授权"」的用例，是同一个东西——只不过前者在适配器退回解析器之后
     照样是绿的。* 这个适配器的回归不会以"放行了一个越权调用"出现，
     它会以"拒绝理由换了一句话"出现。
201. §10.50 ★★ **query 上的同族形态**：按 `&`/`=` 手切时，`?%6dethod=DELETE`
     切出来的键是 `%6dethod`，而 `METHOD_OVERRIDE_KEYS` 里只有 `method`
     ⇒ 那条"能覆盖方法的东西一律拒绝"的检查**从不触发**，一次 DELETE 被按**读**判。
     ⇒ 用 `URLSearchParams`（会百分号解码）。
202. §10.50 ★★★ **判据顶出来的头号真缺陷：`normalizeApiGrant` 不幂等，而三个端口长得一样**。
     `fromEnv` 归一化一次、`create*Port` 再归一化一次——在 PRT-604/605 上是绿的，
     在 PRT-606 上是**装配期就抛**（`api-scope-malformed`：
     "出现了不认识的字段 [`parsed`]"）。根因是它在每个端点上挂了一个**派生字段**
     `parsed`，而 `parsed` **不在** `ENDPOINT_FIELDS` 里。实测：
     `normalizeGrant`（605）**幂等** ✓、`normalizeApiGrant`（606）**不幂等** ✗。
     *而最值得记的不是这个 bug，是"这个归一化器幂等吗"这件事在三个端口的代码里
     一个字都没写。* 已钉成套件 ⑩。
203. §10.50 ★★★ **可达性基线只被单边守**：`reachability.test.mjs` 的 ② 只管
     "当前不可达的必须有分类"、③ 只管"基线里的文件必须还在"——
     **没有一条判据管"基线里判成 `gap` 的模块后来被接上了"**。实测有**两条**
     这样的过期条目：`external-api-scope.mjs`（第 20 轮）与 **`execution-scope.mjs`
     （第 19 轮就接上了，一直留到今天）**。⇒ 新增用例 **③b** 补上这个方向，
     基线 46 → 44 条（`git diff` 恰好 10 行删除）。
204. §10.50 ★★★ **而第 19 轮那条过期条目不是疏忽**：那一轮**做了所有正确的事**
     （从 READINGS 删条目、按规矩补替身、更新 §5.2 的读数），
     **却没有任何判据要求它动基线**。*一个「只检查'有没有漏掉'的基线闸」，
     与一个「可以无限积累过期条目、而每一条都长得像一条读数」的基线闸，
     是同一个东西——只不过后者会让"死代码还有多少"这个数**只会涨不会跌**。*
205. §10.50 ★★★ **判据第三次"按自己的指示"换对象，而这次换到了一个不同形状的缺口**：
     `production-scope-wiring.test.mjs` ② 现在守 `whitelist`。
     *三道范围检查是"**桥里没有位置**"（连一格 `false` 都读不出来）；
     `whitelist` 是"**位置在、而生产装配从不给它值**"——前者连"我该配什么"都问不出来，
     后者问得出来却**没有任何人**在问。* ★ 三道接齐后，`whitelist`（PRT-603）
     是这一族里**唯一**剩下的成员：有一格 `false`、有一份套件、有一份台账 ✅，
     而生产装配里那个键从来不存在。
206. §10.50 ★★ **闸门红了五处，五处拦的都不是同一件事**：`production-scope-wiring` ①/②
     （键集 8 → 9）、`path-scope` ⑥（两处独立钉同一份契约）、`config.test.mjs` × 4 组基线
     （14 键 / 17 动态 / 命中 10 处）、`boundary-facts` ①/⑫a/⑫b/⑫c（手钉坐标 763 → 780）、
     `reachability` ④ → ③b（前一个红是**好消息**，后一个红是**新发现的洞**）。
207. §10.50 ★★ **手钉坐标第四次漂了**（`tool-request.mjs` 763 → 780）。
     四次位移来自**四个不同的功能、同一个文件**——这把"手钉坐标的成本随这个文件
     被改过几次增长"从一句推测变成了**被验证四遍的读数**。
     *一条钉在"行号 + 逐字文本"上的判据，锚的其实是**文本**；行号只是"去哪里找"的提示。*
208. §10.50 ★★ **本轮踩到的第四个 CRLF/表格坑**（前三轮各踩过一个）：
     第一版把台账片段**直接拼在行尾** ⇒ 末尾那个 `|` 被挤到了中间，
     `check-docs.mjs` 当场红（*表格行未以 `|` 收尾*）。★ 这条错的危险之处在于
     Markdown 对此**不报错**——渲染器只是把后面几行当成同一格的内容。
     ⇒ 正确写法是插在**最后一个 `|` 之前**；脚本里现在有一条 `assert` 专守这件事。
209. §10.50 ⚠️ **诚实边界 ①**：端口**不看 scheme**（`checkExternalApi` 也不看），
     `SCHEME_DENIED` 归 `checkNetwork` ⇒ `ftp://api.example.com/api/items/1`
     只要 host 与模式对得上就会被 `externalApiScope`**放行**。本批**没有**顺手加
     "必须 http(s)"——那是**发明策略**（PRT-253 §3 明令禁止发明默认值），见 §5 第 26 条。
     ★ 它今天不致命的原因**不是**"已经拦住了"，是"这一格在真实部署里仍是 `false`"
     ——两件事不能混。
210. §10.50 ⚠️ **诚实边界 ②**：`idempotencyKey` 恒为 `null`（事实表里没有它）
     ⇒ `IDEMPOTENCY_NOT_HONORED` 在当前接线里**不会触发**——重试语义还没有数据来源。
211. §10.50 ⚠️ **诚实边界 ③**：`LEGION_EXTERNAL_API_SCOPE` 同样**不在**
     `product/process-manifest.mjs` 的 runtime `envNames` 里 ⇒ 真实部署里
     `externalApiScope` 仍是 `false`。★ 它是**同一个缺口（一个数组）的第四个受害者**。
     ⚠️ 而这个数**连续两轮都在涨**：每接一道范围检查，就有一把新键落进同一个没修的数组
     ——"四把键一起通"这句话仍然成立，但它成立的前提是**有人去修那个数组**。
212. §10.50 ⚠️ **诚实边界 ④**：三道范围检查现在是"**配了才跑**"，不是"默认就拦"：
     三次接线同一个形状，没配 ⇒ `port` 是 `null` ⇒ 那一行是放行。
213. §10.50 ⚠️ **诚实边界 ⑤**：**MCP 那一条仍然没接**（第 19 轮的裁决未做，§5 第 25 条）；
     这一轮**同样**没有产生任何"真 DSH 进程里观察到的读数"——
     全部证据来自真实组合根 + 假 Context。
     跑满 803s 后红了 `p13-host-injection` ④（SSE 宿主形态），
     `SyntaxError: Unexpected end of JSON input`，耗时 **260ms**；它通过时是 **4097ms**。
     同一提交第二次全量 CI（`.ci/r19c`）**九阶段全 PASS**，该套件 39/39；
     单独跑 6 次也全绿。三条判据认定它偶发：①耗时形状（260ms ⇒ 流几乎立刻结束，
     是**传输层**读数不是判定）；②本轮改动是**环境门控**的（`LEGION_EXECUTION_SCOPE`
     缺席 ⇒ `executionGuard` 第一句就 `return undefined`，与改动前逐字等价）；
     ③套件**串行**（`run-ci.mjs:3980`）⇒ 不是并发争 CPU。
     *一个"偶发红、重跑就绿"的套件，与一个"一直绿"的套件，在摘要里长得一样——
     只不过前者的**下一次**红会被当成"又是那个偶发"。*
...
220. §10.52 ⚠️ **生产里还没有人调**那条出站车道的两半——所以决策表第 15 条（PRT-610 的写入方）**没关**。本轮的口径是"**车道已建、环已证、位置未决**"，而不是"模块全绿"。缺的是**一个决定**（第 28 条），不是代码。
221. §10.52 ⚠️ **没有给 spool 定目录**：收账方拿不到那个路径（hub 的库是 `team-hub/team.db`，它**不读** `LEGION_DATA_DIR`），而猜一个两边都同意的路径正是 PRT-253 §3 禁止的"发明默认值"。
222. §10.52 ⚠️ **没有动 `team-hub/server.mjs`**（9097 行，且是别的会话的在制品风险区）；生产接线的两个候选接缝只**读出来**了，没有采用。
223. §10.52 ⚠️ **没有做车道文件的裁剪**（旋转/归档）。车道是只追加的，所以它会一直长。这是刻意换来的代价：收账必须能安全重跑，而"收完就删 + 记游标"会在崩溃后把没落账的几条当成"已经收过了"。
224. §10.52 ⚠️ 环的证明是在**单个进程内**、用**注入的临时目录**走通的；真实部署里写与收是两个进程，而"分开之后它们怎么找到同一个目录"正是第 28 条。⇒ 这一条证明的是**车道与收账逻辑**，不是**跨进程部署**。
225. §10.52 ⚠️ `toolCallDrainStatus` 的三个桶（`unwired`/`idle`/`wired`）**只在用例里被喂过**——生产里还没有任何一个部署自检调它。
226. §10.52 ⚠️ 本轮**自己造出来、又被用例当场抓住**的一处：第一版 `safeRunId` 只挡了路径分隔符，漏了冒号与**win32 会静默去掉的尾部点/空格**——而后者与"消毒"是同一个后果（`run-1.` 与 `run-1` 落进同一个目录，两个 Run 合流成一本账），且它**一个字符都没被替换**，所以连一个能引起警觉的动作都没有。另有一处接口摩擦：收账侧起初把"落账失败"报成 `line: null`（知道有一条坏了、却找不到它在哪一行），已改为带行号。

227. §10.53 ⚠️ 剩余 **87 处 / 16 个文件**的表格缺陷**没有修**——它们全在
     `docs/review/*` 与 `scratch/*evidence-backup/*` 这类**时点记录**里（记录的是当时
     发生的事、不再改动），所以本轮建的是**棘轮**（只许降）而不是清零。
228. §10.53 ⚠️ 基线**只覆盖已跟踪的 md**：一个新增但未 `git add` 的文件不进这个读数
     （与套件完备性门禁同一条纪律）。
229. §10.53 ⚠️ 检查器只判**列数**，不判内容：一行若有 5 格而**恰好**是 5 格却是错的，
     它看不出来——它挡的是"错位"，不是"写错"。
230. §10.53 ⚠️ `README.md` / `docs/FEATURES.md` 的修法把竖线换成了 `{a,b}` 与全角 `｜`，
     这**改变了渲染出来的字面文本**——语义等价，但不再是"逐字原样"的接口列表。另有一处：`PRT-603` 那份文档里的 `` `|| === null` `` 改成了行文引用（"那条判空短路分支"），因为转义在代码跨度里**无效**。

231. §10.52 ★★ **可达性门禁按设计工作了**：全量 CI 红在它的 ②⑤ ——本轮新建的
     `runtime/toolcall/spool.mjs` 与 `orchestrator/worker/toolcall-drain.mjs`
     **必须有人定性**。两条都判为 `gap`，指针指向 **§5 第 28 条**（它们**是同一条缝的
     两半**，阻塞是同一个位置决定）。⇒ 基线 44 → 46 条。
     ★ 这条**不是回归**：对一个"已交付但生产里到不了"的模块，门禁的全部作用
     就是逼着人说清它属于 `by-design` / `deliberate` / `gap` 里的哪一类。
232. §10.52 ⚠️ **同一处还有一层摩擦**：我为了让人读起来醒目，把指针写成
     `§5 第 **28** 条`（数字加粗），而门禁 ⑧ 的解析器是 `/§5\s*第\s*(\d+)\s*条/`
     ——`**` 让它**匹配不上**，于是被报成"没写出裁决处指针"。
     *一个「为了给人看而加粗」的指针，与一个「机器再也读不出来」的指针，
     是同一个字符串——只不过前者的好处在第一眼，后者的代价在门禁上。*

### 10.54 第 23 轮：F-15 的「告警 / 降级」——**spec 的一半此前没有任何实现，而文件头声称它已实现**

#### 一、缝在哪：一行写着"已实现"的覆盖声明

`docs/MULTI-AGENT-FEATURE-OPTIMIZATION.md` §4.4 对 F-15 逐字要求：

> 按 scope、goal、task、employee、model 记录 token、调用次数、耗时和成本，
> **支持告警、降级、暂停和硬阻止**。
...
| spec 要的 | 落地 | 读数 |
| --- | --- | --- |
| 暂停 / 硬阻止 | `budget-ledger.mjs` | `reserve → observe → settle`，超限 ⇒ `cancel-requested` + `cancel: true` |
| 按五维记录并读出 | `usage-rollup.mjs` | 五维 × 四量，且每一个数字都带"不知道"的邻居 |
| **告警** | —— | `budget-ledger.mjs` 里 `告警` / `alert` / `notify` 出现 **0** 次 |
| **降级** | —— | `budget-ledger.mjs` 里 `降级` / `degrade` / `downgrade` 出现 **0** 次 |

而 `usage-rollup.mjs` 的文件头当时写着：

```text
· **硬阻止 / 暂停 / 降级** —— `budget-ledger.mjs` 已实现（PRT-503/510/511）
```

它把 spec 四件事里的**降级**记在了账本名下，而**账本里根本没有降级**。

> 一行写着"已实现"的覆盖声明，与一行真的已实现，
> 在排期表上是同一个东西——只不过前者会让下一轮的人**不去看它**。

★ 这与决策表 F-04 那一格被手抄的四列合计、以及 `RUNTIME-CONTRACT` 那族被错标
`in-flight` 是**同一族**：**一句没有门禁核对的断言**。区别只在于它写在哪个文件里。

#### 二、为什么这块不能靠"读一下报表，看数字大不大"糊过去

因为 `usage-rollup.mjs` 用整篇文件头立下的那条纪律（"每一个数字都有一个
'不知道'的邻居"），在告警这条路上会变成一个**具体的、危险的**输出：
...
#### 三、交付物：两个正交的量分开报，再用**一条**规则接起来

`team-hub/budget-alert.mjs`（新，20 例）：

| 字段 | 由什么算 | 取值 |
| --- | --- | --- |
| `level` | **只由已知金额** | `ok` < `warn` < `degrade` < `block` |
| `confidence` | **由"不知道有几条"** | `exact` / `partial` / `unknown` |
| `action` | 两者**取更严** | `none` / `review` / `degrade-model` / `block-new-runs` |
| `allClear` | 三件事**同时**成立 | `configured && level === 'ok' && confidence === 'exact'` |
...
```text
confidence !== 'exact'  ⇒  action 永远不许是 'none'
```

它保证了"数不可信"这个事实**不可能**被读成一个放行。

`GET /api/usage/alert`（新，9 例真 HTTP）——把这条求值接到真库上。

#### 四、★★★ 刻意的接口设计：**不**拿 `perRunBudget` 当上限

`bindingStore` 里确实有一个 `perRunBudget: { maxCost, maxTokens, currency }`，
...
但它没有任何意义：窗里有 30 次运行，那么"累计花费超过单次上限"是**必然**的，
与"超支了"无关；反过来窗里只有一次运行没花完，它就报绿，
而那个员工可能已经把**月度**预算花光了。
...
本接口不发明上限、不发明阈值、不发明降级目标。

用例 ④c 把这条钉住了：夹具里**故意**放了一条 `perRunBudget.maxCost = 0.01`
的绑定，然后断言响应**与它无关**（`limit` 仍是查询串给的那个值）。
若哪天有人"顺手"把它接上去当上限，那条用例会红。

#### 五、★★★ 本轮抓到接缝上的一个真缺陷：**守卫在，但它看不见那个键**

路由第一版是这样的：

```text
从查询串里挑出**认识的那三个** rung 名 → 交给 evaluateBudgetAlert()
```

于是 `?warning=0.5`（把 `warn` 拼成了 `warning`）这个键
**根本没进到** `normalizeThresholds()`——那条"不认识的阈值名 ⇒ 抛"的守卫
**在，但它看不见这个键**，接口照旧返回 **200**。

> 一个把不合格的输入**过滤掉**、再交给守卫的适配层，
> 与一个根本没有守卫的实现，是同一个东西——
> 只不过前者的守卫**在源码里看起来是有的**。

★ 这是**用例 ④b 抓到的**，不是复核出来的。修法是把参数集变成**封闭**的
（`ALLOWED_PARAMS`），认不出的参数一律拒并列出可选值——
它同时覆盖"阈值名拼错"与"其他参数拼错"，而两者的后果是同一个：
**用户以为配了，实际没配**。

★ 注意这条缺陷的形状与第三节那条**方向相反**：第三节是"守卫太松"，
这一条是"守卫没看到输入"。**两种都不会让任何一条既有用例变红。**

#### 六、变异验证：7 + 2 处守卫逐条被咬住

`budget-alert.mjs` 的 7 处：

```text
✓ M1 规则 1 拆掉（confidence 不 exact 时的下限改成 none）      → 红 3 条
✓ M2 allClear 少一个合取项（去掉 confidence === exact）        → 红 6 条
✓ M3 不认识的阈值名静默忽略                                    → 红 1 条
✓ M4 阈值判定从 >= 改成 >                                     → 红 1 条
✓ M5 没给上限时落回默认值                                      → 红 1 条
✓ M6 没配阈值时也算 allClear                                   → 红 2 条
✓ M7 阈值不递增时放行                                          → 红 1 条
```
...
```text
✓ M1 参数集不再封闭（即上面那个真缺陷）  → 红 1 条
✓ M2 缺 limit 时落回默认上限            → 红 1 条
```

★ 变异脚本本身也踩过一次坑，值得记：我第一版用 `/# fail (\d+)/` 读结果，
而 node 的 spec reporter 打的是 `ℹ fail 3` ⇒ **每一条都读成 -1，
于是七条全被判成"漏网"**——而它们其实每条都咬住了（红的那几条名字都对）。

> 一个读不出"红了几条"的解析器，
> 会让一次**全部咬住**的变异验证看起来像一次**全部漏网**。
> 两种结论的处置完全相反。

#### 七、诚实边界（如实）

1. ⚠️ **`degrade-model` 只是一个动作名，不是一次真的降级。** 本模块算出
   "该降级了"并给出目标 id，而**真的去换模型**是执行面的事——
   而执行面今天**没有**读这个接口的地方（它没有 hub token，见第 19 条裁决）。
   ⇒ 与第 22 轮那条车道**卡在同一处**：**位置/通道未决**。
   ★ 所以这一批的口径是"**告警这一半建好了、且真 HTTP 上跑得通**"，
   **不是**"生产里已经在降级"。
2. ⚠️ **上限仍然是一个部署配置里不存在的东西。** 本接口要求调用方给，
   而"每个 scope / employee 的预算是多少"今天**没有任何地方存着**——
   那是第 19 条人工项 A 的同一个缺口（配置键进不了 runtime）。
3. ⚠️ **没有做防抖。** `direction` 只报读数（升高/降低/持平/不知道），
   不在这一层做滞回：滞回是有状态的，而本模块刻意是纯函数。
   ★ 代价是**在阈值上下反复跨线时会来回报**——这个代价是明说的，不是漏掉的。
4. ⚠️ **`records === 0` 被当作 `exact`。** 窗里一条用量记录都没有时，
   花费确实是 0。但"账本里没有行"与"记账坏了"在**这个函数的知识范围外**——
   它只看得到 `totals`。真要区分，得比对 `run_attempts` 的条数，那是另一个读数。
5. ⚠️ 两条 ⏸（PRT-009 峰值资源、PRT-253 真实自动执行）与第 22 轮相同，未动。

#### 八、本轮提交面

```text
team-hub/budget-alert.mjs           新（445 行）
team-hub/budget-alert.test.mjs      新（20 例）
team-hub/budget-alert-http.test.mjs 新（9 例，真 hub 进程 + 真 HTTP）
team-hub/server.mjs                 +1 条只读路由（约 120 行，含两段设计说明）
team-hub/usage-rollup.mjs           订正文件头那句错误的归属声明
scripts/ci/run-ci.mjs               登记新套件（含 22 行理由）
docs/MULTI-AGENT-FEATURE-STATUS.md  F-15 主行补齐这一批的读数
```
...
★★ 这一轮**两处红都是真门禁拦下的真遗漏**（`config scan` 的 14 个未登记具名码；
`prt-baseline` ④ 的平台契约漂移），**都不是既有问题**。
前者证明那道扫描器对**新写的**码同样生效；
后者那次 diff **只有 2 行**（1 条新路由 + 1 个源文件哈希）⇒
顺手证明了"这次的契约变化**只有**这一条"——**门禁的价值正是这个读数**。

★★★ 并且由此抓到一处「设计说明与事实不符」（与本节第一条**同族**）：
`scripts/prt/baseline-snapshot.mjs` 第 20–23 行写着
「为什么**不**接进 CI 作为阻断门禁……最后必然被人用 `--record` 无脑刷掉」，
**而它已经是一门阻断门禁**（`baseline-snapshot.test.mjs` ④ 就在 `test` 阶段里）。

> 那两行**逐字预言了**这个结局。它没有防住——因为写下那段话的人以为
> "没接进 CI"是**已经完成的事实**，而实际上它是以另一个文件名接进去的。

⇒ 本轮**就是**那句预言的复现：加 1 条只读路由 ⇒ CI 红在 ④ 上 ⇒
唯一的解除方式是 `--record` ⇒ 照做。订正只改注释、不改任何行为。
