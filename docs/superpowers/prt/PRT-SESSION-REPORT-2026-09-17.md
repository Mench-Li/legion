...
51. §10.26 只订正了 §5 第 13 条**一处**。我**没有**系统检查 §5 其余 20 条里
    是否还有同类"机械阻塞盖住决定阻塞"的表述——而这一批的教训恰恰是
    这种表述会出货（"等并发"过期了没人改）。同理我**没有**检查 §5 之外的
    文档（`STATUS.md`、各 PRT 明细）里有没有同形的过期阻塞。
    ⇒ "发现了一处"与"只有一处"是两件事，而我只做到了前者。
