...
大量依赖坐标（`tool-request.mjs:639`「缺表=放行」、
...
> 而它与 `tool-request.mjs:639` 那句 `if (pathScope === null) return undefined`
...
| 第 **19** 条（执行面四份数据的载荷） | **5** | `execution-plane-config`、`runtime/packs/*` |
| 第 **20** 条（契约服务端那一行进不进补丁层） | **4** | 契约服务端 + 组合根注册行 + `run-floor` |
| 第 **18** 条（F-18/F-19 执行面一半） | **3** | 摩擦分、关系图、岗位包 |
| 第 **13** 条（F-21 判定面） | **2** | `connectors/registry`、`target-binding` |
| 第 **14** 条（三道范围表） | **2** | `execution-scope`、`external-api-scope` |
| 第 **17** 条（PRT-707 两份实现） | **1** | 死的那份 `first-run.mjs` |
| **合计** | **26** | |

⇒ **"140/145"与"26 个模块在生产里永远跑不到"可以同时是真的**——
它们量的是两件事：前者量"判据交付了没有"，后者量"这条链从真入口走得到吗"。
本轮**两件都写出来，不合并成一个数字**。

★ 排序后，真正需要项目方点头的**主要就 3 件**：
第 **16** 条（9 个模块）、第 **15** 条（**同时**解开第 13 条的反馈面）、
第 **14** 条（2 个）。其余要么已在别人的施工里（第 19/20 条），
要么是外部环境/真人（第 1～7、11、21 条）。

#### 八、诚实边界

1. 我**没有**去核 DSH `tools/` 那一侧有没有"执行后"的钩子
   （`pre-execute` 是**前置**瀑布，名字就说清了它的位置）。
   所以"反馈面没有接缝"精确的意思是**"本仓的强制面桥没有这个接缝"**，
   **不是**"DSH 不提供任何钩子"。这一条会改变 §九 第 2 项的成本估计，
   但**不改**"要有这个决定"这件事。
...
109. §10.41 ★ 交付全是文档＋一处状态订正：裁决文档新增 §10、状态文档 §5 第 13 条与
    ...
    ★★ §九把 26 个 `gap` 与决策表**逐条对上账**（不重不漏：16→9、19→5、20→4、18→3、
    13→2、14→2、17→1 = **26**）⇒ **"140/145"与"26 个模块在生产里永远跑不到"
    可以同时是真的**，它们量的是两件事。
