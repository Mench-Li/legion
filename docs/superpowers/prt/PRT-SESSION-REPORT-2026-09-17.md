...
- ★ 我**没有**做系统普查：**仓库里还有多少生成物在手写状态/结论？**
  这一批只处理了手上撞见的这一处。这与我上一批记下的空洞（过期数字没有普查）
  是同一类，而我又一次只做到了"顺路发现"。
...
58. §10.30 的结论是"生成物去复述状态，就一定会漂"。这是**从一个实例**推出来的一般化：
    我这一批只撞见**一处**（`legion-host.patch.yml`）。
    ⇒ 我**没有**扫仓库里还有多少生成物在手写状态或结论。
    "发现了一处"与"只有一处"仍然是两件事——而这是本会话**第三次**记下同一个空洞
    （过期数字没普查、§5 之外没扫、生成物没普查）。
    ★ 一个反复出现的**没做的事**，比一次做错的事更该被记下来。
