...
### 10.21 ★★ 本轮 CI 是**红的**，而红的那一条不属于本轮——证据、成因与边界
...
| 复现点 | 结果 |
|---|---|
| 主工作树（含另一个 agent 的未提交改动） | 失败 |
| 隔离 worktree @ `e347462`（我的 HEAD，工作树**干净**） | 失败 |
| 隔离 worktree @ `69fed2c`（**本轮之前**的提交） | 失败 |
...
**这说明什么、不说明什么：**
- 说明：它是**环境侧**的（DSH profile 启动在本机当前状态下挂住），
  且**可稳定复现**——不是一次幸运的抖动；
- 不说明：我没有查到**为什么** profile 启动挂住。pnpm 存储/网络/临时目录
  三个方向我都**没有**验到底（临时目录被用例的 `finally` 清掉了，
  而"清掉"正是它清理得对的样子）。
- ★ 这一条属 **PRT-509**，是另一个 agent 进程的域，且他们手上正有
  `product/launcher/*` 与 `process-manifest.mjs` 的未提交改动。
  **我没有去改他们的文件**——改它等于替他们做一个我没查清的判断。

**给下一次跑 CI 的人**：这一条红了**不代表本轮改坏了什么**，
也不代表 PRT-509 坏了——它代表**本机当前起不动 DSH profile**。
先跑 `node --test product/launcher/run-credential-dsh-process.test.mjs` 单独确认，
再看 DSH 检出能不能 boot 任意 profile。
...
37. §10.21 那个 CI 失败，我**没有**查到根因。已排除的三项（检出未改、
    ...
    下一个人若要定位，`--verbose` 或抓子进程的堆栈是没做过的方向。
38. 本轮结束时**整个 CI 是红的**（`[test]` FAIL）。我的四套相关判据
    （`reachability` 9/9、`intervention-coverage` 4/4、`check-docs`、
    `spec-progress`）都各自通过，但**没有任何一次"全绿 CI"覆盖本轮的提交**。
    ★ 本轮**没有**把 §10.21 那条失败绕开或标记跳过——
      它仍然红着，这是有意的：一条我不知道根因的红，比一条被我压掉的红有价值。
