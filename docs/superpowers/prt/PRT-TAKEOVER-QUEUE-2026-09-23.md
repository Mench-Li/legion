...
`cd plugins && npm test`（先 build 再跑 38 个套件）**在本机跑不了**，三条一起挡着： ★★ 订正 2026-09-24（T10）：**这句已不成立** —— 实测只有**一条**挡着，且它不是"缺"而是**上游拆包后路径过期**（见本表 T10 行）。
...
`runtime/adapters/dsh/port.mjs:46` 引的 `plugins/src/index.ts:1827` 随那次搬家失效（净 −66 ⇒ **当时**实际落在 **1764**；★ **2026-09-24 T2 片 3 又让它下移一次** ⇒ 现落在 **1766** —— 旧数字留此不改写）。
...
| **W-23** | 新增 `connectorShape` 谓词端口 | 端口 + `createEnforcementBridge()` | 反例：`mcp__evil__x`（命名空间无已知连接器）⇒ **具名拒绝**（理由必须是登记表，不是政策门） | ★ **✅ 已收** —— 谓词端口落地（`connector-port.mjs`：`connectorShape`，与 `resolveConnectorId` **同生共死**，缺席时一并 `null`）；`decision-port.mjs` 具名拒绝 + 新计数器 `namespaceUnknown`（**不算** `unattributed`，两个读数必须分得开）；四层接线同批（root-row / root / assemble）；判据 ⑩～⑮ + ④g/④h + root-row ④（**旧读数 `unattributed + 1` 按裁决取代**）；变异 `_probe-ruling23-connector-shape.mjs` 两个方向都咬住（"谓词被忽略" ⇒ 未知命名空间又落回政策门；"认不出就拒掉一切" ⇒ 非 MCP 形状的名字也被拒） |
...
| **W-12** | 七类版本面写实 | `role-pack.mjs` + 文档 | 判据：岗位包的版本节集合 == 七类（prompt/skills/tools/permissions/model/connectors/budget） | ★ **✅ 已收** —— 与"内容哈希面"（已有 ⑧）分开钉：⑩ 每类字段闭包含 `version` + id 字段唯一 + 三种形态**恰好**盖满七类；⑪ **逐一漂移**（每类单独换版本 ⇒ **恰好**一处 `VERSION_DRIFT` 且是那一类；"恰好"这半边证明另外六类没被误报）。判据 role-pack.test ⑩/⑪（42/42）；变异 `_probe-w12-version-surface.mjs` 两个方向都咬住 |
| **W-3** | host-plane 冻结的**边界** | 文档 + 清单 | 判据：新增 host 插件须过一道显式清单（否则红） | ★ **✅ 已收** —— 新增 `runtime/dsh-composition/host-plane.manifest.json`（只存 `rows` 一份，**推导值不落盘**）；判据 composition.test ⑯～⑲（52/52）：⑯ 清单与 `PATCH_LAYER_ROWS` 逐字同序、⑰ 命名空间边界、⑱ 清单里不许有推导键、⑲ YAML 行集合 ⊆ 清单；变异 `_probe-w3-host-plane-freeze.mjs` **三个方向**都咬住（含主方向：代码加一行而清单没人动） |
| **W-4** | win32 `sandbox-enforcement` 的"完整 vs 部分"口径 | 文档表 | 口径落表后与 `#11-①` 一次收口（两条都是已裁定的平台边界） | ★ **✅ 已收** —— 口径 = `probeSandbox` **八条**拒绝理由的闭集（四组），表在 §5.29 ①；判据 `composition.test.mjs` ⑳（逐项归因 + 闭集断言：源码每条拒绝理由都必须被某个反例触发过）；变异 `_probe-w4-sandbox-caliber.mjs` 两方向都咬住。★ **我自己第一版写成"四项"**，被闭集断言抓出漏掉的 3 条早退分支 —— 已订正并留痕。与 `#11-①` 同批收口为永久平台边界 |
| **W-5** | G2/G3/G4 口径落地；黄金跑**挂 #7** | 文档 + 用例 | 口径与"黄金跑待外部输入"分开记，不许混成一句 | ★ **✅ 已收** —— §5.29 ②：能力来源判据（= §5 第 24 条，已施工）/ win32 沙箱口径（= §5 第 4 条八条表）两条**标准**，与"真实模型黄金跑**缺外部输入**、仍挂 §5 第 7 条"**分行记**；另如实记：本行 4 个编号对 3 个条目、整仓无 G 编号定义表 ⇒ **按条目读，编号对应不发明** |
| **W-2** | F-15 行补一句"**永久环境边界**"（#2 已裁决） | 权威表 F-15 行 | 与本次裁决同批，读数不变（保持 ✅） | ★ **✅ 已收（`255b7bd`）** |

★ **同批要求**：**W-24 与 W-25 必须同批**（两条改的是同一个 `preExecute` 上的"配了却没生效"读数，分开做会在中间态互相打架）。
★ **本次已收**：`W-2`、`T1`（见 §13.1 / §13.2 表内标记）。
...
| **T2** | PRT-1007 片 3+：给 `spaceWorker` 立缝再搬一族 | `plugins/src/index.ts:486-2668` | `probe-slice-verbatim` 登记片 3（`from` 用**不可变提交**）；★ 回引写**删除点原位置** ★ **◐ 片 3 已落（isSupervisor → plugins/src/proc.ts，逐字 + 409 例复跑）2026-09-24；spaceWorker 立缝未做** —— 本轮量到本行的一个**结构性事实**：`plugins/src/index.ts` 里 `spaceWorker` 那一族（例：`recallCorpus`）**全是缩进 2 格的闭包**，捕获外层的 `expDraftDir` / `parseDraftState` / `pendingRecallRefs` / `config` …⇒ **四问①（逐字相同）与闭包互斥**：搬走一个闭包，就必须把它捕获的那些东西一并变成参数，而**那一步必然改文本**。于是"立缝"本质上是**改行为边界**的活 —— 逐字工具搬不动它。★ 这条边界此前**没写在工具的脸上**：使用者会拿到 ①/②/③ 三条**看起来像自己写错了**的报错，而真相是"这一族不在这个工具的适用范围里"。已做成棘轮：`probe-slice-verbatim.mjs` 新增 `symbolTopLevel` 与规则 **⑤**，把闭包当片登记会**明确报 ⑤**并指出正确路线（立缝 + 行为级）。判据：`node scripts/probes/probe-slice-verbatim.mjs` ⇒ **exit 0**（两片仍绿 ⇒ 既有片确实是顶层，⑤ 不与现实冲突）。变异验证 `scripts/probes/_probe-t2-top-level.mjs`（**内存 fixture，不碰磁盘**）：⓪ 工具读数 `recallCorpus` top=false indent=2 / `hashStr` top=true ① 闭包当片 ⇒ 报 ⑤ 且消息含"闭包""立缝" ② 对称控制：顶层符号**不许**被 ⑤ 误报 ③ 真登记表 2 片全绿且无 ⑤。★ 诚实边界：⑤ 是**保守**规则**不是定理** —— 闭包在新模块恰好同作用域时，逐字相同理论上也能成立；本仓选"顶层优先 + 说清理由"，代价是拒绝一种理论上的合法形状（已写进代码注释）。★ ⇒ **本行的片 3 对象需重定**：逐字对拍适合的是**顶层纯函数族**（既有的片 1/2 就是这形状）；而 `spaceWorker` 的立缝是**另一件活**（改行为边界），它的兜底判据现在是 `cd plugins && npm test`（**409 例 / 10 套件 / 0 失败**，T10 之后本机可跑）。两条路走哪条、以及片 3 挑哪一族，需要业主一句话（或下一轮先挑顶层族把片 3 落下）。 |
...
| **T5** | 16 条 ✅ 行做**声称级**核对 | 权威表 ✅ 行 | 每行一条可复跑判据，否则如实降级"未核" ★ **◐ 已勘（读数已量、降级待裁）2026-09-24** —— 落点：新增普查 `scripts/probes/feature-row-judges.mjs`（✅ 行 × 三种判据来源：**点名 F-id 的套件** （`run-ci.mjs` 的 `label: '…（F-18：…）'`，最强）· 行内**引用**的套件名 · 行内引用的 `scripts/**` 判据脚本）。**实测读数**：✅ 行共 **21** 条（不是 16）；**13 条**有点名该 F-id 的套件（F-01 F-02 F-05×2 F-15 F-16 F-17 F-18×4 F-19 F-20）；**2 条**只有引用式判据（F-07 F-13）；★ **6 条没有任何可复跑判据**：**F-03 F-06 F-08 F-10 F-12 F-14** —— 它们就是本行说的"否则如实降级未核"的对象。★★ **我自己第一版读到的是"16 条未核"，那是错数**：`named-suite` 那类我没有给 `note` 字段，而筛选写的是 `note === null`，`undefined !== null` ⇒ **最强的一类判据被静默排除**；另有一个区间标签（`（F-01～F-25 …）`）被误配成 F-01 的判据。两处都修了，并把这两条写进代码注释与变异验证（见下）。> 一个"16 条缺失"的读数与一个"我的筛选把最强的判据漏掉了"的读数，> 在只看那一行输出的屏幕上，是同一个东西 —— 而它恰好与本行原来那个数字吻合，> 差点被我当成"找到了那句老话的真身"。★ 本普查**不判红**（理由写在文件头）：把"✅ 行必须有判据"做成门禁会立刻红，而红的意思是"有 6 行需要**如实降级未核**"——那是一次**对业主可见的表态**，不是机械动作。（本仓处理"41 个不可达模块"用的是同一节奏：先清点、逐条挂裁决处，而不是按红灯让人猜。）★ 判据：`node scripts/probes/feature-row-judges.mjs` ⇒ exit 0 并印出名单；找不到文档 ⇒ exit 3（**未观察**）。变异验证 `scripts/probes/_probe-t5-row-judges.mjs`（内存 fixture）：① 点名 F-id ⇒ 最强且**不许**进未核 ② 引用不存在的套件名 ⇒ 判未核 ③ 引用存在的判据脚本 ⇒ 可用（非最强）④ 零锚 ⇒ 未核 ⑤ 区间标签不误配。 |
| **T6** | 引用 **0 条路径**的 4 行（F-02/F-12/F-23/F-25）另找核法 | — | 一种不依赖"引用路径"的核法 ★ **◐ 已勘（未做完）2026-09-24**：本行的前提「引用 0 路径」**实测不成立** —— 按 `docs/MULTI-AGENT-FEATURE-STATUS.md` 今天的四行原文：F-02（L53）引了 `runtime/adapters/dsh/*` **且**指名套件 `dsh-adapter`（103 例）；F-12（L64）引了 `product/launcher/*`；无路径的只有 F-23（L189）与 F-25（L191）两行，而这两行是 **⏸ 按设计归档**（依据格写的是 §2 / §9 的**设计决定**，并在决策台账 L912/L949 有业主裁决「维持」）。⇒ 「另找核法」的对象因此**不是四条**，而是两条，且它们的核法本来就不是路径，是**条款 + 裁决**：即"⏸ 行必须指得到一条设计决定（§N）且该裁决在台账里有登记"。★ 而 F-02/F-12 属于另一类问题（有路径，但要问**路径是否够**），本行不再覆盖它们。★ 下一轮的落点：给 F 表加一条"**每行必须有可核的锚**（路径 / 套件 / 条款§N / 裁决）"的判据，并先核 `scripts/prt/feature-landing-paths.mjs` 今天到底已经管到哪一层，避免重复造。 ★ **✅ 已收 2026-09-24** —— 落点：`scripts/prt/feature-landing-paths.mjs` 新增 `checkRowAnchors`，挂在**既有**那条门禁上（复用它的行解析器 `progress-check.featureTableRow`，**不另造第二份解析器**）。两条规则：① 每行必须有可核的锚 —— 文件路径 / **目录通配且该目录存在** / 套件名 / §条款；② ⏸ 行另加两条：必须指得到条款（设计决定），**且**该条在决策台账里有「已裁决」的登记。实测：功能表 29 行 = 按文件路径 23 + 按目录通配 4 + 按 §条款 2；⏸ 2 行（F-23/F-25）两条都齐，台账里 7 条「已裁决」。★ **判据自己咬出一条**：F-12 的 `product/launcher/*` 是**目录通配**，我第一版把它读成"没有锚"。由此立起一层此前没有的区分：**通配锚的强度低于文件路径** —— 落点门禁本来就跳过通配（它核不了"这个目录下每一个文件"），所以通配要算锚，就得先证明**它指的那个目录还在**（`git ls-files` 前缀核）。> 把两种强度混成一个"有锚"，等于用后面那种的诚实去替前面那种背书。判据：`node scripts/prt/feature-landing-paths.mjs` ⇒ **exit 0**（并印出四个分类计数）。变异验证 `scripts/probes/_probe-t6-row-anchors.mjs` —— ★ **在内存里改文档、不碰磁盘**（该判据收的就是文本）：⓪ 基准绿 29 行/2 ⏸ ① 抽掉 F-23 的 §2 ⇒ `feature-paused-without-clause` ② 台账那条裁决不再点名 F-23/F-25 ⇒ **两条** `feature-paused-without-ruling` ③ 抽掉 F-12 唯一的锚 ⇒ `feature-row-no-anchor` 且点名 F-12 ④ 通配指向不存在的目录 ⇒ `feature-row-no-anchor` 并说明"目录不在被跟踪的文件里" ⑤ 对称控制：原文档零违规（不许误报合规行）。★ 第 ④ 条第一版挑的是 **F-02**，**不咬** —— 它除通配外还有套件锚 `dsh-adapter`；"一行的锚不止一个"这件事已写进探针注释。★ 另一处口径订正：打印的四个数是"**每行按优先级归一类**"（路径 > 通配 > 套件 > 条款），**不是锚的总数** —— F-02 通配与套件两样都有，只进"通配"那一格；混读会让"按套件 0"看起来像"没有行靠套件支撑"。 |
...
| **T8** | 两分支各一提交未并入 main：`w/T-065` `5c0ce26`、`w/dual-write-race` `3c78cce`（含"main 里没有"的 `proc-utils.mjs` + 用例） | — | ★ **✅ 已收（逐件处置 + 可复跑账）** —— 新增 `scripts/probes/branch-disposition.mjs`：每个 `w/*` 分支要么已并入 main、要么登记为"不并入 + 理由"，新出现未登记分支即红。**① `w/T-065` `5c0ce26` ⇒ 已并入**（`6aa4cae`）：`docs/T065-evidence/*` 10 份全并（327 行）；同一提交对 `docs/TEST_REPORT.md` 的改动**不采纳** —— 该文件是滚动覆盖的单槽报告（自己写着"本报告取代上版"），main 上已是**更晚**的 T-109 ⇒ 采纳即回退（解冲突时保留 main 版，`git diff main HEAD -- docs/TEST_REPORT.md` 为空）。**② `w/dual-write-race` `3c78cce` ⇒ 明确不并入**（分支保留作存档），三条各自可复跑的理由：(a) 它改的 `docs/DUAL-WRITE-RACE-evidence/verify-evidence.md` 相对 main **删掉 §10**（2026-09-12 的"同一竞态第二处实例"，实测 main 有 1 处 / 分支 0 处）；(b) 它改的 `scripts/ci/run-ci.mjs` 基于 main **抽出 `parseSuiteCounts` 之前**的版本，试合在同一区域冲突，采纳即回退第 35 轮的解析抽取与机读读数行；(c) 它新增的 `scripts/ci/proc-utils.test.mjs` **在本机 3 例 1 红**（孙进程采集 `[]`），而 main 无消费方 ⇒ 并入即把红带进 main。★ 复活条件已写进处置账（先修 (c) 用例 ⇒ 再按 main 现结构改超时判定 ⇒ §6 以追加并入、保留 §10）。变异 `_probe-t8-branch-disposition.mjs` **三个方向**都咬住（fate 记错 / 分支漏登记 / 提交号指错） |
...
| **T10** | 让 CI 的 build/test **本机可跑**（缺 `packages/preset/agent-presets`、无 typescript、`plugins/lib` 被 ignore） | 环境 | `cd plugins && npm test` 能跑 ⇒ 插件改动从"文本级证明"升到"行为级" ★ **◐ 已勘（未迁）2026-09-24：本行说的"缺三样"实测**只剩一样**，而它不是"缺"，是**改了名**：(a) `packages/preset/agent-presets`（复数）**不存在**，DSH 今天叫 `packages/preset/agent-preset`（单数），`package.json` 的 `name` = **`@deepseek-ai/dsh-agent-preset`**（0.1.7-rc.1）——而 Legion 三处仍引复数：`plugins/src/index.ts:59`（`import type AgentPresets from ...`）、`plugins/package.json:16`（依赖声明）、`scripts/ci/build-external-package.mjs:16`（symlink 的目标路径）⇒ 实测 `node scripts/ci/build-external-package.mjs plugins` 就死在 `build: dependency target missing: …\packages\preset\agent-presets`（该脚本 L34）；(b) **typescript 不缺**：`<DSH 检出>/node_modules/typescript/bin/tsc` 在（该脚本 L30-31 那一关过得去）；(c) `plugins/lib` 本地**在**（ignore 不等于不存在）。★ 这不是未知缺口：`plugins/src/docContract.ts:36-42` 的注释早就写着"`:30` 的 `@deepseek-ai/dsh-agent-presets` 缺包 —— **先于本片存在**"。⇒ **下一轮的机械动作**（三处同名改单数 + `plugins/tsconfig.json` 的 paths 若有），然后 `cd plugins && npm test`；★ 它也是 **T2 的前置**：`probe-slice-verbatim` 的诚实边界（该文件 L29-33）明说它只证"文本搬运保真"，"跑起来一样"要的就是这条命令 ⇒ T2 的验证级别挂在 T10 上。⚠️ 迁移前必须确认单数包的**类型面兼容**（改名可能同时改了导出形状），不然会从"文本级绿"变成"行为级红"。 ★ **✅ 已收 2026-09-24** —— ★ 而我上面那句"改了名单数"**是错的**，实测是**拆包**：DSH 把原包拆成 `dsh-agent-preset`（被挂载的类，`private config`）与 `dsh-agent-preset-registry`（带 `mount` 的注册表，`export default AgentPresetRegistry`）——**本插件依赖的是后者**。按单数包迁会得到 `TS2339 Property 'mount' does not exist on type 'never'`（`AgentPresetRegistry & AgentPreset` 因两边都有 private `config` 退化成 `never`）⇒ 这是"类型面兼容"那句话的实物。落地：`plugins/src/index.ts`、`plugins/package.json`（实为 peerDependencies）、`scripts/ci/build-external-package.mjs`、`scripts/ci/dsh-boundary.mjs` 与其基线，共 5 处，另在 `docContract.ts` 与 `probe-slice-verbatim.mjs` 就地订正三处陈旧读数（原文保留）。判据：`node scripts/ci/build-external-package.mjs plugins` **exit 0**；工程根跑 `npm test` ⇒ **409 例 / 10 套件 / 0 失败**；新增 `scripts/probes/build-targets-exist.mjs`（核"构建自己用的那份名单"：3 个外部包 29 个目标全在 + peer 依赖全部有条目，exit 0；找不到检出时退 3 = **未观察**，不打通过）。变异验证 `scripts/probes/_probe-t10-build-targets.mjs`：① 路径写回旧值 ⇒ 判据红 **且**构建红（打印 `dependency target missing`）② 删掉一个 peer 依赖条目 ⇒ 判据红（方向②）⇒ 三条全咬并逐字还原。★ 顺带解开 **T2 的前置**：`probe-slice-verbatim` 的行为级证明现在本机可跑。 |
| **T11** | `scripts/legion-up.ps1` 归属 + **跨会话并发写入**协调 | 工作区 | 明确谁在写、是否纳管 ★ **⏸ 待业主（非阻塞）2026-09-24**：`scripts/legion-up.ps1` 是**外部未跟踪文件**（"Legion 一键独立启动"包装脚本，不是本会话写的；`git status` 里长期为 `??`）。★ 本会话的纪律是**不动它、也不纳入**（不替别人认领所有权）。需要业主回答一句：**它是谁写的、要不要纳管**（纳管 ⇒ 加 license/编码与 banner 门禁、并写进文档索引；不纳管 ⇒ 记进 `.gitignore`，免得每个会话都在 `git status` 里重新猜一次）。★ 跨会话并发写入：本会话按"每轮只动自己明确 `git add` 的文件"执行（从不 `git add -A`），推送前核对 `git rev-list --left-right --count origin/main...main`；若确有两个会话同时在写，需要业主指定**谁负责推送**——本会话不假设自己是唯一的写者。 |
