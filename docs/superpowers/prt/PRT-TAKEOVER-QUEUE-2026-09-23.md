...
| **P3-2** | ★★ **`scratch/` 是"证据库"却没有被跟踪**（第十轮量到） | 记账 / 待业主定 | `git status` 里**未跟踪 777 个**：189 个根目录 `_*` 遗留（0 个被文档引用，已清）、其余约 580 个在 `scratch/`。而**文档与脚本引用了 105 个不同的 `scratch/…` 路径**当"可复跑的量具"（例：`scratch/_mutate-whitelist-port.mjs` 就写在 `run-ci.mjs` 的注释里）。`scratch/` **不在 `.gitignore`、也从未提交** ⇒ 那些引用指向**只存在于这个工作区**的文件。> 一个"证据可复跑"的读数，与一个"证据在别人的检出里根本不存在"的读数，在**本机**是同一个东西。⇒ 待定：把量具收进 `scripts/probes/` 并跟踪，还是明确 `scratch/` 是一次性工作区（那就该把引用改成"当时量具已随批次丢弃"） | 待业主定 |
