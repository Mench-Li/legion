...
| **T6** | 引用 **0 条路径**的 4 行（F-02/F-12/F-23/F-25）另找核法 | — | 一种不依赖"引用路径"的核法 ★ **◐ 已勘（未做完）2026-09-24**：本行的前提「引用 0 路径」**实测不成立** —— 按 `docs/MULTI-AGENT-FEATURE-STATUS.md` 今天的四行原文：F-02（L53）引了 `runtime/adapters/dsh/*` **且**指名套件 `dsh-adapter`（103 例）；F-12（L64）引了 `product/launcher/*`；无路径的只有 F-23（L189）与 F-25（L191）两行，而这两行是 **⏸ 按设计归档**（依据格写的是 §2 / §9 的**设计决定**，并在决策台账 L912/L949 有业主裁决「维持」）。⇒ 「另找核法」的对象因此**不是四条**，而是两条，且它们的核法本来就不是路径，是**条款 + 裁决**：即"⏸ 行必须指得到一条设计决定（§N）且该裁决在台账里有登记"。★ 而 F-02/F-12 属于另一类问题（有路径，但要问**路径是否够**），本行不再覆盖它们。★ 下一轮的落点：给 F 表加一条"**每行必须有可核的锚**（路径 / 套件 / 条款§N / 裁决）"的判据，并先核 `scripts/prt/feature-landing-paths.mjs` 今天到底已经管到哪一层，避免重复造。 |
