...
| **P3-2** | 19 个陈旧 worktree（`w/T-043`…`w/T-117`、`codex/prt-phase0-1`、`codex/prt-runtime`） | 卫生 | 分支领先 `main` 1–2 个提交未合并，最后活动 9/5–9/12；其中 4 个还有未提交改动 | **🟡 但"未合并"这个词要改**（第二十一轮实测，见下）：**没有一个是"该合而没合"的** ⇒ 处置是**清理**，不是合并 |
...
★ 目标里那句"需要业主裁决的单独汇总"就是本节。**只有下面这 11 格真需要您开口**，
其余的都已在别处裁过或已由本会话做完 —— 按 §3.4 那条纪律，答过的**不重复摆上来**
（"一条已经答过的问题留在待办里，会让'还剩几件事要您拍'这个读数虚高"）。

### 8.1 需要您说一句的（**11 格**，A–K）
...
| **K** | P3-2 的另一半：**19 个陈旧 worktree/分支**（16 个 `w/T-*` + `wip/*` + 3 个 `codex/*`）怎么处置？★ 第二十一轮已量清：**没有一个是"该合而没合"的**（2 个已在 main、1 个被 main 取代、16 个合了会回退 850–1000 个提交）⇒ 这一格问的其实是"**删不删**" | §9bis.2 | 不裁 ⇒ 19 棵工作树继续占着盘、并让"分支领先 main"这个读数继续看起来像欠账 |
| **J** | **守护要不要恢复**（`scrum/control.json` 停在 `{"paused":true}`、`legion-guardian` / `legion-worker` 两个插件停用）？ | §0.1 | 不裁 ⇒ 由本会话独占这棵工作树；这一条**只有您能开**（本会话故意不自作主张） |

★ 表格里是 **11 格（A–K）**，但它们分两组、**都是"一句话"**：

- **A–E** 是 §3.4 已经量好的 **P2-2 那五条**（第 10 / 12 / 8 / 21 / 6 条）——
  每条都只需要一个字或一个选项（其中第 6 条**只需要"包不包含多档案"**）；
- **F–K** 是本会话这二十一轮里**做不下去时才浮出来**的六件（工具结果通道 / `attemptId` 定义 /
  metrics 的只读库句柄 / `scratch/` 的处置 / 19 个陈旧工作树的处置 / 守护是否恢复）。

⇒ 真正**独立**的决定是 **11 格**；其中"D"（要您给一个外部事实）与"J"（要您开关守护）
这两格**本来就是您那边的动作**，其余九格是设计口径或清理口径。
