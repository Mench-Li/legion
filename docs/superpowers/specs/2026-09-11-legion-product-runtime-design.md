...
> **实施进度：141 / 146 已完成（96.6%），尚未完成 5 个。**见文末 **附录 A.7 任务进度总览**；逐条现状见 `docs/superpowers/prt/PRT-PROGRESS.md`。
...
> `test` **47 套件 / 1230 用例**（复现见 `docs/STATUS.md`）。
>
> ⚠️ **版本说明**：本附录首次撰写时依据的是 `3a38d19`（133 个任务）；
> 其时 P0/P1 修订（含 §6.4 审批 TTL、§6.8 强制面可用性、§6.9 组合补丁层与 12 个新任务）
> **尚未提交**，只存在于工作区。该修订已由 `6666b3b` 落定并并入，本附录随之按
> **145 个任务**重算——A.4 中「12 个新任务均未实现」一节即此次补入。
...
| 项 | 状态 |
| --- | --- |
| `PRT-005` | **已收口**：GF-001 在受控隔离空间 `gf001` 真实执行并成功（目标 `G-mtwxx7an-2`，3/3 完成），任务状态、执行事件、产物与审计证据已保存，见 `docs/PRT-005-evidence/verify-evidence.md` |
| `PRT-009` 的 `pending` 段 | **六项结清四项**（token 用量 / 端到端耗时 / 旧路径实测状态序列 / 人工介入率），靠**读取一次已经发生的真实执行**，不是重跑一次。**剩余两项各有不同阻塞**：费用（无有出处的单价——自建网关 `custom-jt` 的单价未定，工具在单价缺失时返回 `null` 而非 `0`，因为 `0` 会让预算检查静默失效）、峰值资源（转录记录不含进程资源，依赖 `PRT-011` 分发形态裁决）。见 `docs/PRT-009-evidence/verify-evidence.md` §4 |

> 这两项曾共用同一句「差一次真实执行」。实际收口后发现**剩余缺口的原因并不相同**——
> 把它们继续并成一条会掩盖「费用卡在单价出处、资源卡在分发形态」这个事实，
> 而这两件事的解锁人不是同一个。故在此拆开记录。
>
> 那次真实执行同时暴露两件需长期跟踪的事：**模型漂移**（三个岗位实际都跑
> `deepseek-v4-flash-openai`，而 planner/reviewer 声明的是 `deepseek-v4-pro-openai`；
> 派工走全局默认模型服务，已登记为机器可检的 `modelDrift`，修复归阶段 3）
> 与 **`advanceTask` 旁路**（`team-hub/server.mjs:2705` 绕过 `TRANSITIONS` 且不要求
> `by === 'general'`，76 个任务里 42 个跳过 `in_review`；已登记为
> `GOLDEN_TASK.transitionBypass` 并有 2 条用例守着，裁决归 `PRT-316`）。

**阶段 2 已完成；阶段 2.5～10 全部未启动**

| 阶段 | 任务 | 数量 |
| --- | --- | --- |
| 2 DshRuntimeAdapter | `PRT-201`～`PRT-215` **全部已落地**（`PRT-201`~`209` 见 A.1；`PRT-210`~`215` 见 **A.6**） | 15 |
| 2.5 商业薄垂直切片 | `PRT-251`～`PRT-258` | 8 |
| 3 Orchestrator Core | `PRT-301`～`PRT-316` | 16 |
| 4 上下文边界 | `PRT-401`～`PRT-413` | 13 |
| 5 模型和密钥配置 | `PRT-501`～`PRT-511` | 11 |
| 6 工具、权限和审批 | `PRT-601`～`PRT-620` | 20 |
| 7 Product Launcher | `PRT-701`～`PRT-713` | 13 |
| 8 安装、升级和回滚 | `PRT-801`～`PRT-813` | 13 |
| 9 商业 Alpha 发布保障 | `PRT-901`～`PRT-910` | 10 |
| 10 能力包协议 | `PRT-1001`～`PRT-1006` | 6 |

**完成度：42 / 145 任务（约 29%）**；1 项部分（`PRT-009`：费用与峰值资源两项未采集，各有不同阻塞）。
里程碑达成：**M0 达成**（GF-001 已在受控隔离空间 `gf001` 真实执行并机器验收）、
**M1 达成**（阶段 1+2 边界完成，含强制面声明与启动自检）。
⚠️ 阶段 2 有一项**刻意的未落盘**：组合补丁层已声明、已受测、已生成产物，但**尚未写入运行 profile**
（该层 `patchReload: 'live'`，写入会立刻改变当前进程的强制面）。
因此强制点原语目前**只被自己的用例驱动，没有生产调用方**——见 `docs/PRT-212-evidence/verify-evidence.md` §2/§5。

> **按 §13 的程序级停止条件，下一批是阶段 2.5（商业薄切片），不是阶段 3。**

**本 A 节首次并入的 12 个任务均未实现**（`PRT-010`、`PRT-011` 除外，它们已由阶段 0～1 完成）：

| 任务 | 依赖的已固定不变量 |
| --- | --- |
| `PRT-214`、`PRT-215` | §6.9 组合补丁层 + 启动自检（未生效按 `incompatible` 处理） |
| `PRT-257`、`PRT-258` | `PRT-011` 路线 C 的落地；阶段 2.5 契约冻结 |
| `PRT-615` | 审批 TTL 到期自动拒绝，与 lease/heartbeat 交互（§6.4） |
| `PRT-616` | `allow-once` 的 team-hub 原子 CAS 消费（§6.8） |
| `PRT-617` | 策略门与 answerer 的连接/响应**双段**超时 + `unavailable` fail-closed（§6.8） |
| `PRT-618` | `legion-attended` / `legion-unattended` preset 表，禁止复用 DSH 默认表 |
| `PRT-619` | Run 期间 approval policy 与 preset 冻结 + 改写审计 |
| `PRT-620` | pre-execute 放行且获 `allowed-once` 的调用不得被 ToolGuard 拒绝（一致性不变量） |
...
| `PRT-315`、`PRT-316`（阶段 3） | 未启动。评审闸门**已通过**：两个热点文件最近 40 个提交仅被触及 1 / 2 次（历史峰值 9 / 7），日更节奏已降温，可用 `scripts/prt/hot-file-churn.mjs` 复算 ——— ⚠️ **2026-09-21 复核订正：上面这一格是一份快照，已不能照字面读。** 该探针量的是「最近 40 个提交」这个**移动窗口**，每落一个提交就变一次；本行自己写着的"可用 `scripts/prt/hot-file-churn.mjs` 复算"就是让它被重量一次的意思。重量读数（2026-09-21，`HEAD 5cd44ab`）：`node scripts/prt/hot-file-churn.mjs --strict` ⇒ `recentMax` **10/40**（`plugins/src/index.ts` 0/40、`team-hub/server.mjs` 10/40）、历史峰值 17/40，判定 **未降温 ⇒ 阶段 3 应推迟**、退出码 **2**。★ 另注：该工具的退出码默认**不**因判定而变（读得到 `git` 历史就是 0），所以"`exit 0`"**不等于**"闸门已过"——要当门禁用请加 `--strict`（见该文件头"退出码"一节）。⇒ 阶段 3 是否可开工，**以下面 A.5 第 3 条为准**（§13 的 Program 级停止条件优先于排期）。 |

### A.5 阶段 3 启动前置（提醒）

- `PRT-316` 仍须排在 team-hub 启动期并发迁移加固（`0db37af`）**沉淀一个完整发布周期**之后。
- 阶段 3 的切片每个都要能独立对拍与回滚，不允许一次性重写两个大文件。〔§2.1、§11 迁移规则 3〕
- **§13 的 Program 级停止条件优先于排期**：M1.5 评审未达标则暂停阶段 3～4。
  因此下一批应是**阶段 2 + 2.5**，不是阶段 3。

### A.6 阶段 2 已落地指针（**回填，不改设计**）

阶段 2（`PRT-201`～`PRT-215`）已全部落地。产物与对应套件：
...
1. **`ctx.sandbox.confine()` 返回 `enforcement: 'full' | 'partial'`** —— 这是 §6.8「必须探测实际后端和
   enforcement；**仅有配置名不算生效**」的可判定落点。判据取 `full`：`partial` 的字面含义是
   「这条 argv 只被部分管制」，即**存在不被管制的路径**。另两条同族判据：`confine` 原样返回
   输入 argv（没做任何包装）、`denialSignatures` 为空（沙箱拒绝会退化成普通失败）。
2. **`ctx.sandbox` 是抽象进程沙箱，不是文件沙箱。** 它只有 `confine(argv, policy)`；
   文件读写走 `fs-sandbox` 行 + `ctx.fs` 的 `sandboxPolicy` 参数。因此「沙箱是否生效」
   **不能**用一次文件读来判定。
3. **`sandbox` 与 `approval` 是独立旋钮，由 `permission` 行捆绑。** DSH 默认表把
   `workspace-write↔ask`、`danger-full-access↔never` 绑定 —— 这正是 §6.9 那条警告成立的**机制**：
   按默认表实现「无人值守 = `never`」会**同时**把沙箱升级为 `danger-full-access`。

**★ 第 4 条实测结论（本批新增）：在 Windows 上，本 spec 的目标平台与"沙箱必须达到 `full`"这条
**实现侧判据**互斥；**当前实现下生产宿主端口在 Windows 上注册不上**。**

> **⚠️ 归属更正（下一批核实后加）**：本条初记时把这件事写成"两条 **spec 条款**互斥、需 spec 所有者
> 裁决"。**那是错的，必须更正**：`full` 这条**判据不是 spec 正文**，它出自本附录第 1 条——即
> **实现方自己**写下的记录。spec 正文只说两件事：§6.8/`line 69`「启动时探测实际 backend 与
> enforcement；**未启用或能力不足时禁止受保护执行**」，以及 `line 854`「未生效时 Runtime Manager
> 按 `incompatible` 处理并**禁止自动执行**」。**正文没有规定判据取 `full`，也没有要求整个进程不许启动**。
>
>   > 把"我们自己选的判据"写成"spec 的要求"，会让一个**可以由实现方自己解决**的取舍
>   > 看起来像一道必须由别人来批的题——而"等别人批"与"没人做"在进度上是同一件事。
>
> 因此下面这段读数（**证据部分全部成立，未改动**）应当读成：平台有真实上界（DSH 的属性）+
> 我们选了 `full` 作判据（我们的选择）+ 宿主行把拒绝升级成**整棵树加载失败**（我们的选择）。
> 三个都是**仓库内可改的**，不存在"spec 互斥"。

- 读数（在一次性 `DSH_HOME` + `bundles: ['@deepseek-ai/dsh-base']` 的真 bundle 启动里量到）：
...
- 后果链（用**生产自己的**判定器读出来的，不是推的）：真 bundle + 真补丁层 + 把
...
- 这里其实有**两个独立的取舍**，别把它们压成一个：
  1. **判据**：沙箱这一项是否取 `full`。取 `full` 有实质理由——`partial` 的定义就是"存在不被
     管制的路径"，而硬链接别名那条路径**是真的存在的**。放松它会把一个**真的**缺口变成
     **看不见的**缺口，所以本批**没有**放松。
  2. **拒绝的后果**：自检拒绝之后，是(a)**整棵树加载失败、进程退出**，还是(b)**不注册宿主
     端口、把运行时标记为 `incompatible`、产品照常运行但禁止自动执行**。**spec 正文 `line 854`
     的字面写法是 (b)**，而当前 `runtime/dsh-composition/plugins/runtime-host-row.mjs`
     （`apply()` 里 `bootstrap` 拒绝那一跳，约 `:645-663`，`throw rowError(... BIND_REFUSED ...)`）
     实现的是 (a)——**比正文更严**。
     本批**没有**翻这个开关：它是一条 fail-closed 的安全取舍，改它要同时改写多条钉住
     "拒绝即退出"的用例，那种改动应当是一次**说出来的**决定，而不是顺手。已核实的唯一
     风险点（改 (b) 是否会让依赖方无限期挂起）**已排除**：没有任何行 `inject` 宿主绑定服务
     （只有 `index.mjs` 导出它），所以不发布它不会造成 Cordis pending。
- 结论：**这不是被外部条件卡住的阻塞**。在 Windows 上让产品可用，只需要在上述取舍 (1)/(2)
  里做一个**仓库内**的选择（最省的是 (2)(b)：判据不动、安全保证不动——自动执行仍然被禁止——
  只是不再让整棵树死掉，于是产品以"只能有人值守"的形态继续可用）。在做出那个选择之前，
  `PRT-257`（生产组合宿主 / 一键启动）在 Windows 上装不上宿主行**这件事成立**，但原因是
  **我们自己的**这条取舍，不是 spec。
...
其余差异按 intended / improvements / bounded 分开归因（各带 PRT 归属）。

<!-- LEGION-PROGRESS:BEGIN —— 本区由 `node scripts/prt/spec-progress.mjs` 从 `docs/superpowers/prt/PRT-PROGRESS.md` **生成**；手改会让 `--check` 变红 -->

### A.7 任务进度总览（**由台账生成，勿手改**）

**已完成 141 / 146 = 96.6%**　·　尚未完成 **5** 个；还有 0 个从未开始。

| 阶段 | ✅ 已完成 | 🟡 部分 | ⏸ 需外部输入 | ⬜ 未开始 | 合计 | 完成率 |
| --- | --- | --- | --- | --- | --- | --- |
| 阶段 0 冻结基线 | 10 | 0 | 1 | 0 | 11 | 90.9% |
| 阶段 1 Runtime Contract | 9 | 0 | 0 | 0 | 9 | 100.0% |
| 阶段 2 DshRuntimeAdapter | 15 | 0 | 0 | 0 | 15 | 100.0% |
| 阶段 2.5 商业薄垂直切片 | 6 | 0 | 2 | 0 | 8 | 75.0% |
| 阶段 3 Orchestrator Core | 16 | 1 | 0 | 0 | 17 | 94.1% |
| 阶段 4 上下文边界 | 13 | 0 | 0 | 0 | 13 | 100.0% |
| 阶段 5 模型和密钥配置 | 11 | 0 | 0 | 0 | 11 | 100.0% |
| 阶段 6 工具、权限和审批 | 20 | 0 | 0 | 0 | 20 | 100.0% |
| 阶段 7 Product Launcher | 13 | 0 | 0 | 0 | 13 | 100.0% |
| 阶段 8 安装、升级和回滚 | 13 | 0 | 0 | 0 | 13 | 100.0% |
| 阶段 9 商业 Alpha 发布保障 | 9 | 0 | 1 | 0 | 10 | 90.0% |
| 阶段 10 能力包协议 | 6 | 0 | 0 | 0 | 6 | 100.0% |
| **合计** | **141** | **1** | **4** | **0** | **146** | **96.6%** |

**尚未完成的 5 个任务**（逐条现状见 `docs/superpowers/prt/PRT-PROGRESS.md` 对应行）：

| 任务 | 名称 | 状态 | 阶段 |
| --- | --- | --- | --- |
| PRT-009 | 成功率/人工介入/token/费用/耗时/资源基线 | ⏸ 需外部输入 | 0 |
| PRT-253 | 单员工黄金任务迁移到 RuntimeAdapter | ⏸ 需外部输入 | 2.5 |
| PRT-256 | 设计伙伴独立完成真实低风险任务 | ⏸ 需外部输入 | 2.5 |
| PRT-910 | 内部与金丝雀真实项目验证 | ⏸ 需外部输入 | 9 |
| PRT-1007 | 编排提取（逐片迁移，业主 2026-09-24 裁决「另行立项」） | 🟡 部分 | 3 |

> **这个百分比的口径（不要读成「产品已完成 91%」）**：
> 
> 本表的 ✅ 判的是**该任务自己的交付物与用例是否已交付**，
> 不是"该阶段的完成标准（exit criteria）已满足"。两者不是一回事：
> 阶段 2.5 及其后各阶段的完成标准涉及**真实端到端跑通**（真引擎、真任务、
> 真审批往返），那些标准目前**尚未达标**，而其中很多任务自己的交付物是齐的。
> 
> 因此：**141 / 146 是"任务交付率"，不是"产品完成度"。**
> 
> 另有 1 个 🟡 是"已有交付物但完成标准未全部满足"。
> 把 🟡 也计入"已开工"，则已开工 142 / 146 = 97.3%。
