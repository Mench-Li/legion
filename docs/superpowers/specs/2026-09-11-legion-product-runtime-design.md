...
**阶段 0 的两处缺口（差一次真实执行即可收口）**

| 项 | 状态 |
| --- | --- |
| `PRT-005` | 未启动：需保存**旧路径真实执行**的任务状态、执行事件、产物与审计证据 |
| `PRT-009` 的 `pending` 段 | 部分完成：静态/机器测量已落地，但 token / 费用 / 端到端耗时 / 峰值资源 / 人工介入率 需一次**真实模型执行**；工具拒绝编造，宁可留空 |

> 这两项与**阶段 0 的完成标准**（「在受控环境稳定复现一次端到端软件交付」）是同一件事：
> 黄金流程 GF-001 与固定夹具已冻结（`PRT-004`），但**从未真正跑过一次**。
> 一次真实执行同时补齐 `PRT-005` 与 `PRT-009` 的 `pending`。
> `PRT-009` 的旧路径状态序列（`old-path-task-state-sequence`）也依赖同一次执行。
