...
test('★★★★★ 缺口 ③（真进程）：一个真 DSH 进程在启动期从 Legion 那份文件里读到了值', async (t) => {
  ...
  try {
    ...
    //
    //   2026-09-18 那次 CI 红**恰好**是这一条：读数文件在、值也读到了、
    //   `source === 'file'` 也成立，只是宿主进程没在预算内退出。
    ...
    assert.equal(result.code, 0,
      result.code === 'TIMEOUT'
        ? `宿主进程在 ${HOST_TIMEOUT_MS / 1000}s 内没有退出（看门狗杀了它；实测 ${Math.round(result.ms / 1000)}s）。\n`
          ...
          + '★★★ **不要靠调大预算来"修"它**：120s 没过、240s 也没过 ⇒ 升预算这条路'
          + '**已证明不收敛**（且"机器忙"这个解释被算术否掉了——那一次其余套件反而快 14.3%）。\n'
          + '★ 决定性问题只有一个：**给足时间，这个宿主到底会不会退出？**'
          + '会（比如 260s）⇒ 关停慢；永不 ⇒ 关停挂死。两者修法完全不同。\n'
          + '  去问它的办法（本套件已支持）：'
          + `\`PRT509_HOST_TIMEOUT_MS=900000 node --test product/launcher/run-credential-dsh-process.test.mjs\`\n`
          + '  退出码 0 且实测耗时远大于 3s ⇒ 关停慢；仍被看门狗杀 ⇒ 关停挂死。**先量再定。**\n'
          + `日志尾部：\n${tail}`
