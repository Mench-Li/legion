...
// ══════════════════════════════════════════════════════════════════════════
// ② ★★★ 文件头已按**活着的那条链**对齐（两段），而三段的形状**仍然**不可寻址
// ══════════════════════════════════════════════════════════════════════════

test('② ★★★ 文件头说的是活着的两段名，而三段的形状**仍然**不可寻址（第 17 条收尾）', () => {
  // ── ★★★ 第 118 轮第十二轮（业主第五轮确认）：上一版这条判据钉的是
  //    "文件头**仍然**声称 Legion 的模型引用是三段"。业主选了"只改陈旧注释与文档、
  //    与活的两段名对齐" ⇒ 那句话已经改成 `model/api-key`，于是这个**载具**要跟着翻过来：
  //    从"仍然这么写着"变成"不再这么写着，而且点了活着的那个名"。
  //
  //    ★ 而三段的"不可寻址"那几条**一条都不动**：那句话描述的是**键空间的形状**，
  //    不是"当前用哪个名"。形状没变，边界就没变。
  const headerOwner = read('security/secrets/credential-materializer.mjs')
  //   ⚠️ 检查的写法本身就是一条教训：订正说明**必须引用**那句旧话（"本文件头**曾经**
  //   写着'Legion 的模型引用是…'"），所以 `includes('Legion 的模型引用是')` 会**永远为真**——
  //   它分不清"这句话是**主张**"与"这句话是被**引用**"。判据只能钉**主张的写法**：
  assert.equal(headerOwner.includes('// 而 Legion 的模型引用是'), false,
    '★ 文件头又把三段引用当成 Legion 的模型引用形状了（`// 而 …` 那句**主张**）—— ' +
    '那句话描述的是已删除的 `first-run.mjs`')
  assert.equal(headerOwner.includes('model/api-key'), true,
    '★ 文件头没有点名**活着**的那个引用（`model/api-key`）⇒ 读者仍然不知道该用哪个名')
