...
export function createDshRuntimeAdapter(host, options = {}) {
  ...
  async function* execute(request) {
    ...
    //                   ⚠️ 2026-09-18 订正：上面"今天没有任何生产调用方生产下限
    //                   （`deriveRunFloor()` 零调用点）"**已过期**。它现在**有**生产调用路径：
    //                   `product/orchestrator/worker.mjs:18`（生产装配点）→
    //                   `orchestrator/worker/executor.mjs:386`（对**每一个**请求）→
    //                   `:868` 的 `deriveRunFloor()`。
    //                   原文保留。**该段结论（这一档不拒收整个 Run）没有被本轮改动**——
    //                   前提没了不等于结论就错，但"结论是否仍然成立"是一次新的裁决，
    //                   属于这条政策的所有者；本轮只把前提过期这件事记下来。
   ...
   * 组装 prompt 文本。
   *
   * ## `request.prompt` 优先（PRT-253 接线）
   *
   * `RunRequest.prompt` 装的是**冻结上下文快照的正文**（§6.5 Context Assembler，
   * PRT-401~413）。给了它就必须**原样**发出去。
   *
   * 这一条不是优化，是那个模块存在的全部理由：快照被冻结、被哈希、被审计、
   * 被持久化，为的就是能回答"模型当时看到了什么"。若这一层另拼一份文本，
   * 那个问题的答案就又消失了——而库里那份快照看起来完全正常，
   * 装配器的套件也全绿（它们验的是装配器，不是这一次执行）。
   *
   *   > 一份被冻结、被哈希、被审计、然后**没有被用上**的上下文，
   *   > 与一份从未被冻结的上下文，在"模型看到了什么"这个问题上是同一个答案。
   *
   * 所以这里**不**在快照正文外再包一层标签：包了就改了 `finalText`，
   * 而快照里存的是没包的那一份，两者又对不上了。
   *
   * ## 回落分支
   *
   * `request.prompt` 缺失时退回阶段 2 的**最小**包装（逐字保持不变——
   * 生产对拍测试 `parity.mjs` 以它为基准）。这条路径是**没有装配器**的调用方
   * （阶段 1/2 的对拍与契约用例）在用，不是生产路径；措辞仍刻意不塞任何
   * 「聪明」的格式，免得后来者以为它有语义。
   */
  function requestPrompt(request) {
    const frozen = request?.prompt
