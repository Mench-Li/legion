...
// plugins/src/index.ts:1766 现场注释原文：「subagent 可能挂死且 run.result 永不结算
// （abort 不保证杀死子代理）」。这不是理论风险，是已发生的生产故障。
// ★ 坐标随 PRT-1007「编排提取」片 1 下移（1827 → **1764** → **1766**）：那次搬家从 `index.ts` 挪出 86 行；★ **2026-09-24 T2 片 3**（把 `isSupervisor` 逐字搬到 `plugins/src/proc.ts`）**又下移一次**：1764 → **1766**（上文那个 1764 是**被取代**的读数，留在原处不改写）
//   （净 −66），而**引文本身一个字没变** —— 坐标是"对某个产物的读数"，产物动了就要重量一次。
//   ★ 这正是上面第 44-45 行说的那条判据（㉓）抓到的：它不是靠人碰巧读到，而是当场红。
//   （同一条失效模式，本轮在 DSH 侧也发生过一次：检出换代 ⇒ 手钉坐标漂 7 行。）
