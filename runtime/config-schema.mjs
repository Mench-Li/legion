...
export const SCHEMA = defineSchema({
  ...
  dynamicEnvReads: [
    {
      file: 'runtime/dsh-composition/scope-port.mjs',
      expr: 'env[SCOPE_PORT_ENV_KEY]',
      reason: '范围表按导出的常量键名下标读取（`SCOPE_PORT_ENV_KEY` = LEGION_PATH_SCOPE，已在上面 fields 声明）。'
        + '**故意用常量而不是字面量**：读取点、用例、以及未来 Launcher 那一侧要指同一处，'
        + '两处各写一遍字面量就会漂移；扫描器看不见这一处正是本登记存在的理由。',
    },
    {
      file: 'runtime/dsh-composition/scope-port.mjs',
      expr: 'env[SCOPE_PORT_CWD_ENV_KEY]',
