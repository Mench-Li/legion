 ...
 *
 * ★ 生产者已经接上了（`orchestrator/worker/executor.mjs` 的
 * `deriveRunFloorCarrier()`：派得出来就写 `installed`，派不出来就地**具名拒绝**），
 * 于是 `absent` 的含义跟着变了：它不再"每个人都是这样"，而是
 * **有人绕过了那个生产者**（自己拼了一个 RunRequest，或走了另一条装配路）。
 * 那仍然是合法形状（契约不能禁止它），但它是一个**要被读出来的**信号——
 * 安装点对它 fail closed 的处置因此不是在补一个默认值，而是在挡一次绕过。
 *
