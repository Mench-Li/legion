...
//
// ---------------------------------------------------------------------------
// ⑨ ★★★ 本批**新查出**的一处：声明里的工具名该写**公开名**，而今天都写的是裸名
//
...
//     因为 DSH 送来的名字带了命名空间，而声明里那一条没有。
//
//   > 一组"声明写裸名、用例写裸名"的夹具，与一组"声明与线上名字对得上"
//   > 的夹具，在**套件读数**上是同一片 ✔——
//   > 只不过前者从来没验过"DSH 真的会送来的那个名字"。
//
// ★ 修法**不是**在归属时把命名空间剥掉：DSH 的公开名在归一化/截断时会被替换成
//   12 位 SHA-256 后缀（见 `public-name.mjs` ②），那时**剥不出** rawName。
//   `tools.ts:9-10` 逐字写着 "the public name is never parsed to recover it"。
//   ⇒ 正确做法是**声明侧**用 `publicToolName(connectorId, rawName)` 算出公开名
//     再写进声明（那个函数本模块已经依赖并导出）。
//
// ⚠️ 本批**没有**改声明语义（那是一次契约变更 + 既有夹具全要动），
//    也没有加"声明里出现裸名就拒"的守卫（今天的夹具与 `git-status`
//    那一类合法用法会被它一起打掉）。它作为**一条裁决项**记在
//    `docs/MULTI-AGENT-FEATURE-STATUS.md` §5 与最终报告里。
//
