...
// 注册方留在依赖方向允许的那一侧。**注册方本批没有交付**（诚实边界见
// `docs/superpowers/prt/PRT-214-enforcement-composition-root.md`）：没有工厂时本行
// **拒绝装配**，而不是装一个"没有审批口"的半根——一个批不准问人的强制面，
// 与一个"审批永远问不到人"的强制面，是同一个东西。
//
...
// 「注册方还没交付」这件事在 PRT-214 文档的诚实边界里写明了——不留一个
// "看起来已经接上"的默认实现，是本文件唯一能做的事。
 ...
 *
 *   · 本行在 `legion-host.patch.yml` 里（`module: './plugins/root-row.mjs'`），
