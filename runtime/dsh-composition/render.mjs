...
const HEADER = Object.freeze([
  ...
  '#   （fail closed）。',
  '#',
  '#   ★ **本文件不声明任何任务的完成状态。** 状态只有一份权威：',
  '#     `docs/superpowers/prt/PRT-PROGRESS.md` 的台账。',
  '#     这段话原来写着一条关于 PRT-214 的状态判断；台账后来改判了那一条，',
  '#     于是它就变成了**下一段自己要警告的那种东西**：一句手写的、与权威清单',
  '#     不再一致的判断；而读到哪一句，取决于读的人。',
  '#     ⇒ 问"这一层生效了没有"，读 `reconcilePatchLayer()` 的对账；',
  '#       问"这个任务做完了没有"，读台账。本文件两者都不替它们回答。',
  '#     （`scripts/prt/boundary-facts.test.mjs` 钉着这一条：本文件的正文里',
  '#      不许出现任何状态词——包括"引用了那个词"的情形。）',
  '#',
