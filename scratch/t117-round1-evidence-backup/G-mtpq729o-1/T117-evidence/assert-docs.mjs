...
const W = 'D:/project/DSH/legion/.legion-worktrees/T-117/'
...
const spanBody = (fromLine, fromLevel) => { // non-empty body lines until next heading level <= fromLevel
  const out = []
...
ok(feats.length > 0 && headingRe.test(fl[0]), 'TC-S1-01 存在非空且首行 # 标题')
...
ok(rev >= 1, 'TC-S2-13 FEATURES→README 反向关键链接 >=1（实际=' + rev + '）')
