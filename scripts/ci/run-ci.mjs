...
async function stageTest() {
  const suites = [
    ...
    {
      // 第 16 条第二刀的那一份**读法**：把产品自己的数据落点从磁盘上读出来。
      //
      // ★ 三个模块（`planExport` / `planRetention` / `planUninstall`）的主入口都要这份入参，
      //   而在这之前**仓里没有任何一处产生它** —— 它们因此长期停在"用例齐备、零生产调用方"。
      //
      // 这一套盯的不是"能不能列出一串文件名"，而是**五个看起来像"扫完了"的形状**：
      // 根不存在（"不知道"不许读成"是空的"）/ 目录读不了（少了一棵子树而清单看着完整）/
      // 到上限（"没扫完"与"扫完了"不许同形）/ 认不出的落点被**猜一个类别**
      // （那会让上层的具名拒绝永远不出现）/ 跟着符号链接走出了根（扫到别人的文件）。
      label: 'store-scan（第 16 条第二刀：三份计划的**共同前置**——把落点从磁盘读出来）',
      files: ['product/lifecycle/store-scan.test.mjs'],
      cwd: ROOT,
    },
    {
      // 第 16 条第二刀的三份计划入口。
      //
      // ★ 三条口径：模式**必须显式给**（给错 ⇒ 计划整份不可用 + 退出码 2，
      //   因为 `planUninstall` 对未知模式什么都不删——猜 `purge` 会删掉用户的数据）；
      //   有发现**不改退出码**（那些发现是内容，脚本丢掉它们就再也看不见了）；
      //   正文必须自己声明"这是计划、零副作用"（与"已经删了"不许同形）。
      label: 'plan-cli（第 16 条第二刀：`--uninstall-plan` / `--export-plan` / `--retention-plan` 的入口判据）',
      files: ['product/lifecycle/plan-cli.test.mjs'],
      cwd: ROOT,
    },
    {
  ...
  // ★★ 第十七套（第 39 轮）：目标文档 **§7 第 6 条**那六个「持续观察」指标的**落地**。
  //
  //   第 37 轮把 §7 投影了一遍，结论是"5 条可核对要求已满足，第 6 条不是退出条件"。
  //   但"不是退出条件"**不等于**"不该实现"——它是目标文档的明文要求，
  //   而第 39 轮实测：6 个指标里 **4 个零命中**，也没有 `/api/metrics`。
  //   而原始数据**已经在库里**（`event_deliveries` / `run_attempts` /
  //   `permission_requests` / `budget_reservations`）——缺的是**汇总读出口**。
  //
  //   所以这一套是**功能**，不是判据：6 条要求 → 8 格指标 + 真 SQL 生产者。
  //
  //   它防的形状（与 §6.6 那九格同一个族）：
  //     ① **"零"与"没有"同形**：分母为 0 是"还没观察过"，**不是** 0%；
  //     ② **分子分母不是同一批人**：用"全部预留行"当超限率的分母，
  //        会把还挂着、还没花过钱的预留算进去，系统性地压低超限率；
  //     ③ **"没这张表"与"表是空的"得到同一个 0**；
  //     ④ **"没有生产者"与"这次读不出来"混为一谈**（一个要去建记录器，一个把库连上就行）。
