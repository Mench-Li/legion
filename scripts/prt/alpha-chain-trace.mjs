...
export const ALPHA_CHAIN = Object.freeze([
  ...
  Object.freeze({
    ...
    coreWhy: '切换与迁移是"回滚"这一节的两半（两者都属 `deliberate` 不可达——'
      ...
      + '不是"回滚"本身。'
      + '★★ 第 118 轮第十六轮：`retention.mjs` 已被 `--retention-plan`（第 16 条第二刀）'
      + '接上 ⇒ 本节的支撑缺口**消失**，`owner: 16` 随之被 `owner-stale` 判掉并删除'
      + '（与第 118 轮第七轮 L7 的 `owner: 28` 同一种处置）。'
      + '⚠️ 核心那两个仍是 `deliberate` 不可达 —— 那**不是**断点，但也不是"通了"：'
      + '它由 CLI 按路径调用，不由模块 import，判据永远判不出它跑没跑。',
  }),
])
