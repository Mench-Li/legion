...
test('① ★★ 真仓库：九节、模块文件全在、**没有硬断**、链现在**全绿**了', () => {
  const t = traceChain()
  assert.equal(t.sections.length, 9, `§9 应当是九节，实际 ${t.sections.length}`)
  // ★ `allGreen` 这一行的值**改了四次**，而四次都必须把"为什么"写下来——
  //   否则"还有一个硬断"、"还有两处软缺口"、"只剩一处"、"全绿"在这一行上是同一个读数。
  //
  //   第 44 轮之前是 L5 硬断；第 44 轮之后是 **L7/L9 两处软缺口**；
  //   第 118 轮第七轮 L7 的支撑模块（`spool`/`toolcall-drain`）被 hub 的收账 tick 接上，
  //   于是只剩 **L9** 一处；
  //   ★★ 第 118 轮第十六轮 L9 的支撑模块 `product/lifecycle/retention.mjs`
  //   被 `--retention-plan`（第 16 条第二刀）接上 ⇒ **整条链全绿**。
  //
  //   ⚠️ 全绿**不等于**"这条链被证明跑通了"：L9 的核心两个模块
  //   （`switchover.mjs` / `migration.mjs`）仍是 `deliberate` 不可达 ——
  //   判据判不出"有没有人真的按路径调用过它"。⑥ 那条用例钉的正是这一点：
  //   全绿必须是**算出来**的，不是"没有硬断就算全绿"。
  assert.equal(t.allGreen, true, '链本该全绿（L9 的支撑缺口已在本轮被接上）——见下面 ⑥ 的反面控制')
   ...
   *     而支撑模块再次掉回去，这一行也会红。
   */
  // ★★ 第 118 轮第十六轮：这里原来是 `['L9']`。L9 的支撑模块
  //   `product/lifecycle/retention.mjs` 被 `--retention-plan`（第 16 条第二刀）接上
  //   ⇒ 软缺口**一处不剩** ⇒ 这一行改成 `[]`。
  //
  //   ⚠️ 空集**不等于**"这条判据没用了"：⑩/⑪/⑫ 三条控制用的都是**人造节**，
  //   它们证明的是规则函数本身有牙齿（缺归属红、指到空处红、过期指针红）。
  //   而这一行仍然会在**下一次**有节掉回缺口时报出来（那时它会不相等）。
  assert.deepEqual(t.softGaps.map((s) => s.id), [],
...
test('⑨ ★★ 正对照：真仓库里**全部断点都有归属**，且恰有**一**节声明了归属', () => {
  ...
  // ★★★ 第 118 轮第十六轮再从一节变**零节**：L9 的支撑模块被 `--retention-plan` 接上，
  //   它不再有断点，`owner: 16` 同样被 `owner-stale` 逼掉（同上：判据没被放松）。
  //
  //   ⇒ 链上**没有任何一节**还剩断点，所以**归属集必须是空的**。
  //     这一行现在是空集，而它的价值没变：**只要有节重新出现断点而没人声明归属，
  //     它立刻不相等**（下面那条反向对照把两个方向都钉住）。
  const t = traceChain()
  ...
  assert.deepEqual(withOwner, [],
    ...
    + '链上已无断点，还留着的归属指针就是过期的（⑫ 那条控制钉的是同一件事）')
