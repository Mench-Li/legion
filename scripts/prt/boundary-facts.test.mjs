...
const PROBE_BATTERY = [
  ...
  {
    no: '㉑d',
    file: 'scripts/probes/feature-row-judges.mjs', what: '✅ 行的可复跑判据普查（T5）',
    allowUnobserved: true,
    // 它存在的理由："这一条 ✅ 了"与"这一条有东西在核"在表里是同一个 ✅。
    // ★ 它是**普查**（永远 exit 0，名单印在输出里），不是门禁 —— 把"✅ 行必须有判据"
    //   做成门禁会立刻红，而红的意思是"有若干行要**如实降级未核**"（对业主可见的表态）。
    //   本节只保证它**真的被跑**（并因此被读到），不替那个决定表态。
  },
]
