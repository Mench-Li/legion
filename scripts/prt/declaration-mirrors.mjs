 ...
 *
 * ★★ 三条的共同形状：**它们是"名字"的来源，不是"行为"的来源**。
 *   判据分不出"这张表被人手写重建了一遍"（真缺陷）与"另一个东西的键恰好
 *   与这张表同名"（巧合）——那是**人**的判断，所以由人在这里写明。
 *   这正是豁免机制存在的意义：不是把判据改松，是把判断留痕。
 */
export const DECLARED_MIRRORS = Object.freeze([
  Object.freeze({
    name: 'WRITABLE_ROLES',
    why: '它是 `WritableRole` **类型**的来源（`product/index.d.mts` 引用它），不是运行期行为的来源。'
      + '`product/paths.mjs` 那处 `writable` 对象把角色映射到 `layout` 的各个目录字段，'
      + '而字段名（`dataDir`/`workspaceDir`/`cacheDir`/`logDir`）**无法由角色名机械派生**，'
      + '所以那份手写映射是必要的。加一个角色必须同时改两处——这**是**一处已知的耦合，'
      + '但它属于"改 `paths.mjs` 时要记得的事"，不属于本判据能替人做的重构。',
    owner: '已知耦合；若要机械化，属 §5 第 16 条那批"产品级动作"之外的独立重构，需另行裁决',
  }),
  Object.freeze({
    name: 'PREFLIGHT_VERDICTS',
    why: '**命名巧合**：`ok`/`blocked`/`unknown` 既是判定词，**也**是预检结果对象的字段名'
      + '（那个对象还有 `stage`/`checks`/`toleratedUnknown`/`reasons`/`remedies`）。'
      + '它**不是**判定词表的第二份实现——判据分不出"重建"与"同名"，故在此说明。',
    owner: '判据的已知假阳性；记在 §5 第 16 条同族的"口径边界"里',
  }),
  Object.freeze({
    name: 'BPE_ARTIFACT_FIELDS',
    why: '它声明的是产物的**字段清单**，而 `runtime/context/bpe.mjs` 那个返回对象是产物的**形状本身**'
      + '（比清单多一个 `ranks`）。清单被用来校验/描述产物，不是用来构造它——'
      + '「构造」与「描述」在这里是两个方向的问题。',
    owner: '同 `WRITABLE_ROLES`：已知耦合，需另行裁决是否机械化',
  }),
])
