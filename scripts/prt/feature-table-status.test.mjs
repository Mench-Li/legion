...
// ══════════════════════════════════════════════════════════════════════════
// ★★★ 第 48 轮：本模块是同一张功能表在本仓的**第五份**手写解析
//
// 而它是这五份里**唯一做对了一件事**的：它把跳过**报出来**（`skipped`，
// 第 23 轮 F-15 那一课的成果）。所以它的缺陷不是"静默少读"，
// 而是"把 4 格 / 7 格的功能行**归错类**"——报成"另一张表"，
// 于是那一行的状态与「还差什么」都不再被这条规则管。
//
//   > 一份"跳过并报出来"的清单，比一份"静默跳过"好得多——
//   > 但它仍然可能把**自己看不懂的东西**说成**别人的东西**。
//
// ⇒ 现在"哪一行算功能行"由所有者（`progress-check.featureTableRow`）说了算。
// ══════════════════════════════════════════════════════════════════════════

test('⑧ ★★★ 4 格 / 7 格的功能行**算行**，不许归进"另一张表"', () => {
  // ★ 旧规则 `≠5 且 ≠6 ⇒ 跳过` 会把这两种宽度的**真功能行**报成"别的表"。
  const withCols = (n) => {
    const cells = ['F-99', '名称', '🟡', '`a.mjs`', '证据', '—', '附注'].slice(0, n)
    return ['| 编号 | 名称 | 状态 | 代码落点 | 判据 | 还差什么 |',
      '|---|---|---|---|---|---|', `| ${cells.join(' | ')} |`, ''].join('\n')
  }
  for (const n of [4, 5, 6, 7]) {
    const r = check(withCols(n))
    assert.equal(r.skipped.length, 0,
      `${n} 格的功能行被归进了"另一张表" ⇒ 它的状态与「还差什么」都不再被管`)
    assert.equal(r.rows, 1, `${n} 格的功能行没被当成行（rows=${r.rows}）`)
  }
  // ★ 反面控制：**3 格**确实是**另一张表**，必须仍然报成跳过（否则会把别的表当功能表、假红）。
  const other = ['| 项 | 已做到 | 还差 |', '| --- | --- | --- |',
    '| F-21 连接器 | 未声明即拒绝 | 没连过真的 MCP server |', ''].join('\n')
  const ro = check(other)
  assert.equal(ro.rows, 0, '把另一张 3 格表当成了功能表')
  assert.equal(ro.skipped.length, 1, '3 格的另一张表没有被报成"跳过" ⇒ 它从报告里消失了')
})
