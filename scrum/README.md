...
4. **依赖解除的 blocked 任务**（属于本角色）→ 解阻认领 → 续做。**「❓ 待将军确认」的疑问型 blocked 除外**：士兵遇必须将军拍板的疑问会转 blocked 并留 ❓ 评论（任务详情/清单显示醒目「❓ 待将军确认」徽标），守护不自动重跑；将军在评论答复后，下一轮自动带答复续做。
...
- **🖥 军团总指挥部（`console.html`，看板右上角/左下角入口 + GUI **侧栏常驻块**）**：借鉴 dsh-worktable 控制室的「节点 + 任务实时总览」——顶部守护状态条（角色/模型/流水线/并发/inbox/uptime/最近扫单），三列卡片（⚡ 工作中 / ⏳ 待你决定 = in_review+blocked / ✅ 已完成），每卡带运行时长、进度条、最近评论预览，底部滚动最近动态。数据源 = `daemon.json` + `board.json` + `activity.jsonl`，经 `board/events` SSE + 轮询驱动，**纯 UI、零 Token**。board-plugin 挂在 `/scrum-board/console`，serve.mjs 挂在 `/console.html`；入口两处：看板页 header 按钮、看板页左下角浮动胶囊（独立页面上无设置按钮，不受遮挡影响）——圆点均为守护心跳（绿在线/红未自述/灰未知，每 30s 探测 `/scrum-board/api/daemon`），点击在新标签页打开总指挥部独立页面。**DSH GUI 侧的入口已移出**（会话内看板面板与侧栏常驻块均已移除），指挥总览改由 Legion 指挥台承载。
