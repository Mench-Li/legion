...
test('⑦ ★★★★★ 下界：`limit=-5` 现在夹到 **1**（原来会变成"不限制"）', async () => {
  reset(); seedN(60)
  const r = await get('scope=S&limit=-5')
  assert.equal(r.status, 200)
  // ★★★★★ 2026-09-20 修（原为"钉住缺陷"）：以前 `Math.min(-5,500) = -5` 原样进 SQL，
  //   而 SQLite 里**负**的 `LIMIT` 意思是不限制 ⇒ "我只要 5 条"变成"给我全部"。
  //   现在夹到 `[1, 500]` ⇒ 负数落到 **1**。
  assert.equal(r.body.length, 1,
    '★★★★★ 负数夹到 1 —— 一个"限制条数"的参数**不能再被用来取消限制**')
  // ★ 各种负数都落到同一个下界
  for (const v of ['-1', '-500', '-99999']) {
    assert.equal((await get(`scope=S&limit=${v}`)).body.length, 1, `★ limit=${v} ⇒ 1`)
  }
})
...
test('⑨ ★★★★★ `limit=2.7` 现在回 **200**（原来把只读接口打成 500）', async () => {
  reset(); seedN(10)
  // ★★★★★ 2026-09-20 修（原为"钉住缺陷"）：
  //   以前 `Math.min(2.7, 500)` 是 **2.7**（没有取整），原样绑定进 `LIMIT ?`
  //   ⇒ SQLite 抛错 ⇒ `handle()` 的兜底把它变成 **500** ——
  //   **一个查询参数能把只读接口打成 500**，而只读接口本该"给什么参数都回 200"。
  //   现在先 `Math.trunc` 再夹范围 ⇒ 浮点也回 200。
  //
  ...
  for (const v of ['2.7', '1.5', '0.5']) {
    ...
    assert.equal(r.status, 200, `★★★★★ limit=${v} ⇒ **200**（浮点不再崩）`)
    assert.ok(r.body.length <= 2, `★ 而且是"取整后"的条数，不是全部：拿到 ${r.body.length}`)
  }
  // ★ `2.7` 取整是 **2**（不是 3、不是 50）
  assert.equal((await get('scope=S&limit=2.7')).body.length, 2, '★★★ `2.7` ⇒ 取整成 **2**')
  // ★ 对照：整数一直都没事
  ...
  // ★★ 反自检：算式**确实**取整了 —— 若有人把它改回去，这条会红
  const srv = await import('node:fs').then((m) => m.readFileSync('team-hub/routes/activity.mjs', 'utf8'))
  // ★★★★★ 反自检：算式**确实**取整并夹了范围。
  //   ★ 这条以前钉的是"**没有**取整"（那时缺陷还在）。现在缺陷修了，
  //     它反过来钉"**已经**取整" —— 谁把修复回退掉，这里就红。
  //
  //   ★★★★★ **必须先剥掉注释再判**：我在这段注释里把**旧写法当反例抄了一遍**，
  //     而那些"不许退回去"的断言原来扫的是**整个文件** ⇒ 它们会命中**我自己的注释**、
  //     在我刚写好的修复上报红。
  //     > 一个「我加了一条"不许退回去"的断言，它会守住这个修复」的印象，
  //     > 与一个「那条断言扫的是**整个文件**，而我在同一段里把旧写法抄了一遍，
  //     > 于是它守的是我自己的注释」的事实，在它在我刚写好的修复上报红之前是同一个东西。
  const code = srv.replace(/\/\*[\s\S]*?\*\//g, '').split(/\r?\n/).filter((l) => !/^\s*\/\//.test(l)).join('\n')
  assert.match(code, /Math\.trunc\(Number\(/,
    '★★★★★ 缺陷 21 的修复：必须先 `Math.trunc` 取整（否则浮点进 SQLite 的 LIMIT ⇒ 500）')
  assert.match(code, /Math\.min\(Math\.max\([^)]*,\s*1\),\s*500\)/,
    '★★★★★ 缺陷 21 的修复：必须夹到 `[1, 500]`（下界 1 消灭"负数 ⇒ 不限制"）')
  assert.ok(!/Math\.min\(Number\([\s\S]{0,80}?\b500\)/.test(code),
    '★★★ 不许退回"只夹上界"的老形状（★ 已在**剥掉注释**之后判）')
})
