...
async function handle(req, res, stripPrefix) {
  ...
  try {
    ...
    // 本族这 2 条已全部搬进模块，`server.mjs` 里不再有它们。
    // 归属 /api/agents , /api/spaces 的那 2 条都在这里了。
    if (await router.dispatch(req, res, { path, url })) return
    ...
    // ══════════════════════════════════════════════════════════════════════
    // ★★★★★ 裁定：下面这 2 条**不提取**，就留在 `server.mjs` 里。
    //
    //   业主 2026-09-20 裁决（读法乙）：`GET /api/events` 与
    //   `GET /api/event-delivery` **判定不该被提取**，而不是"还没做"。
    //
    //   理由（不是"做不完"，是"搬了更糟"）：
    //
    //   ① 它们与前 49 族**共享同一个模块级可变计数器** `deliveryBookkeepingFailures`
    //      （`let` 在 L4544）：
    //        · 本段之内：`events` 里 `+= 1`；`event-delivery` 里读两次。
    //        · 本段之外：`audit()` 那条路与广播那条路也 `+= 1`（L4575 / L4732）；
    //          切片 19 的 `config.mjs` 装配处用 `live: () => ({ … })` 读它（L5082）。
    //      ⇒ 一旦搬出去，这个**可变**计数器就要**跨越模块边界**。
    //
    //   ② 而 `createRouter([...])` 在**模块加载时只发生一次**。于是"按值注入"会
    //      把计数**永久冻结**在装配那一刻（实测：真实值 3，路由读到 0）。
    //      ★★★ 危险在于它**完全静默**：body 逐字节相同、逐字对拍会通过、
    //      全量回归会全绿，而那个读数**从此永远是错的**。
    //
    //   ③ 唯一忠实的搬法（按访问器注入，照切片 19 的 precedent）要求 **body 改动**，
    //      而 spec `:1282` 写的是「阶段 3 的切片每个都要能**独立对拍与回滚**」。
    //      ★ `events` 还要更重一层：它里面是**写**（`+= 1`），只读访问器不够，
    //      得再开口子给"写"。
    //
    //   ④ 形态上也不同：`GET /api/events` 不是一个"算个结果然后 `json()`"的路由 ——
    //      它 `res.writeHead(200, text/event-stream)` **自己接管响应**、把连接登记进
    //      `eventClients`、装一个 `setInterval` 心跳、在 `req.on('close')` 里清。
    //      它和 `handle()` 的**生命周期**绑在一起。
    //
    //   ⇒ 硬搬只是把耦合换个地方，还会把"谁拥有连接 / 谁清心跳 / 谁数这个失败"
    //     这三件事拆到两个文件里。**留着才是更小的耦合。**
    //
    //   ★ 这道裁定是**可核的**：`scripts/prt/sse-tail-guard.mjs` 钉住
    //     「剩下的恰好是这 2 条」+「那个绑定仍是模块级 `let` 且真的被写过」，
    //     并且在那对**被搬走之后**要求新模块**不许按值注入**它。
    //   ★ 有出处的理由与量测见 `docs/PRT-316-evidence/sse-tail-decision-required.md`。
    // ══════════════════════════════════════════════════════════════════════
