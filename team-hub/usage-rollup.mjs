...
//
// 五个维度 × 四个量。落地层面：
//   · **硬阻止 / 暂停** —— `budget-ledger.mjs` 已实现（PRT-503/510/511），
//     `reserve → observe → settle` 那条链是闸门，不是报表。
//   · **告警 / 降级** —— `budget-alert.mjs`（本批新增）。
//   · **本文补的是剩下那一半：把已经记下来的事实按五个维度读出来。**
//
// ★ **订正一处曾写错的归属（2026-09-18 第 23 轮）。** 本文件头此前写的是
//   「**硬阻止 / 暂停 / 降级** —— `budget-ledger.mjs` 已实现」，于是把
//   spec 四件事里的「降级」记在了账本名下。**账本里根本没有降级。**
//   实测：`budget-ledger.mjs` 里 `降级` / `告警` / `degrade` / `alert` /
//   `notify` 的出现次数**全部是 0**。
//
//   这不是"少写了一块"那么简单——它是一句**在文档里宣告已完成**的覆盖声明，
//   而**没有任何门禁会去核对它**（与决策表 F-04 那一格手抄计数是同一类）。
//   它的后果是下一轮的人在排期时把"降级"划掉。
//
