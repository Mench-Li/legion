...
- **可观测性**：`/healthz`（浅）、`/readyz`（深：逐房间存储 + 心跳新鲜度）、`/metrics`、
  `/api/rooms`、`/api/rooms/<id>/audit`（JSONL 落盘 + 内存环形缓冲；`?source=archive|all` 可**跨重启**回溯，
  响应附 `retention` 说明能回溯多久）；控制面默认仅回环。
- 实例形态**仍是单实例、单进程**；被否的多实例共享存储方案与转 v2 触发条件见 ADR-0008。
  **该约束已是可执行守卫**（P4-6）：房间目录加独占锁 `<WB_ROOMS_DIR>/.whiteboard.lock`，
  第二个实例指向同一目录会**启动即失败**并提示怎么办（旧行为是两边都起来、**静默分裂数据**：
  实测两边视图各自停在 2 与 3 而真值已是 4，两侧日志一个字都没有）。
  进程被强杀留下的陈旧锁可**自动接管**（会留启动告警 + 审计 `lock_takeover`），不会把服务锁死；
  守卫状态见 `/healthz`、`/readyz`、`/metrics` 的 `dirLock`。
